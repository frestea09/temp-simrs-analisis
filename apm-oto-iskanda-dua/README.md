...
```json
{
  ...
  "API_REGISTRATION_BY_DATE_ENDPOINT": "/apm-oto/v1/registrations/{identifier}/date/{date}",
...
- `API_REGISTRATION_BY_DATE_ENDPOINT`: template endpoint registrasi berdasarkan tanggal (opsional, wajib ada `{identifier}` dan `{date}`).
