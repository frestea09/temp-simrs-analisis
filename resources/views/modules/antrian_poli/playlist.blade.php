...
<html lang="">
  ...
  <body class="hold-transition login-page bg-image">
    <h3 class="besar">Suara Antrian Poli</h3>
    <h2>{{ config('app.nama') }} <br><small>{{ configrs()->alamat }}</small></h2>
    <!-- <img src="{{ asset('images/'.configrs()->logo) }}" class="logo" > -->
    <div class="notif">
      *) Pastikan halaman ini selalu terbuka! sini kah
    </div>
    <h3 class="layarantrian">
      <a href="{{ url('/antrian_poli/layarlcd') }}" target="_blank">Klik disini untuk membuka Layar urutan Antrian!</a>
...
@endforeach
  ...
  <META HTTP-EQUIV="REFRESH" CONTENT="{{ $delay }}; URL={{ url('antrian_poli/suara') }}">
