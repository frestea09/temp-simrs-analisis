        ...
        <div class="col-md-12">
          <div class="blockloket">
            <div class="col-md-5 text-center">
              <div class="no_antrian">NO ANTRIAN</div>
              <div class="panggil_antrianku" id="panggilan_terakhir">-</div>
            </div>
            <div class="col-md-2 text-center">
              <div class="panah">>></div>
            </div>
            <div class="col-md-5 text-center">
              <div class="no_antrian">RUANG</div>
              <div class="panggil_antrianku" style="font-size: 40px;" id="nama_poli"></div>
            </div>
          </div>
        </div> 
        <div class="col-md-12">
          <div class="blockloket">
            <table id="dataku" class='table table-bordered table-condensed' style="margin-bottom: 0px;">
              {{-- @foreach($data['grid']['grid1'] as $key => $item)
              <tr>
                <td width="150" class="textku text-center" id="layarlcd{{ $item['id'] }}"> </td>
                <td class="textku">{{ $item['nama'] }}</td>
              </tr>
              @endforeach --}}
            </table>
          </div>
        ...
        <div class="col-md-12">
          <div class="blockloket">
            <table id="dataku" class='table table-bordered table-condensed' style="margin-bottom: 0px;">
              @foreach($data['grid']['grid1'] as $key => $item)
              <tr>
                <td width="150" class="textku text-center" style="font-size: 17pt" id="layarlcd{{ $item['id'] }}"> </td>
                <td class="textku" style="font-size: 17pt">{{ $item['nama'] }}</td>
              </tr>
              @endforeach
              {{-- @foreach($data['grid']['grid2'] as $key => $item)
              <tr>
                <td width="150" class="textku text-center" style="font-size: 17pt" id="layarlcd{{ $item['id'] }}"> </td>
                <td class="textku" style="font-size: 17pt">{{ $item['nama'] }}</td>
              </tr>
              @endforeach --}}
              {{-- @foreach($data['grid']['grid3'] as $key => $item)
              <tr>
                <td width="150" class="textku text-center" id="layarlcd{{ $item['id'] }}"> </td>
                <td class="textku">{{ $item['nama'] }}</td>
              </tr>
              @endforeach
              @foreach($data['grid']['grid4'] as $key => $item)
              <tr>
                <td width="150" class="textku text-center" id="layarlcd{{ $item['id'] }}"> </td>
                <td class="textku">{{ $item['nama'] }}</td>
              </tr>
...
{{-- <div class="footer">
  ...
  <div><marquee>{{ configrs()->antrianfooter }}</marquee></div>
    ...
    <script>
      ...
      //   // alert("a")
      //   $.ajax({
      //     url: "{{ url('antrian_poli/ajax_suara') }}",
      //     type: "GET",
      //     cache: false,
      //     dataType: "json",
      //     success: function(res){
