...
{!! Form::hidden('bayar', '1') !!}
{!! Form::hidden('nomorantrian', $pasien->nomorantrian) !!}
        ...
        <div class="col-sm-9">
          <select class="form-control select2" style="width:100%" name="dokter_id">
            ...
            @foreach ($dokter as $key => $d)
              <option value="{{ $d->id }}" {{$pasien->dokter == $d->kode_bpjs ? 'selected' :'' }} >{{ $d->nama }}</option>
            @endforeach
          </select>
  ...
  <section class="form-add-dokter">
    ...
    @if (request()->segment(1) == 'regperjanjian')
      <div class="form-group{{ $errors->has('created_at') ? ' has-error' : '' }}">
          ...
          <div class="col-sm-9">
              {!! Form::text('created_at', date('d-m-Y',strtotime(@$pasien->tglperiksa)), ['class' => 'form-control', 'id'=>'regperjanjian', 'required' => 'required']) !!}
