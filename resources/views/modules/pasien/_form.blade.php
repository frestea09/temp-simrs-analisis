...
<div class="row">
  ...
  <div class="col-md-6">
    ...
    @if (request()->segment(count(request()->segments())) == 'create')
      <button type="button" onclick="nomorNik()" class="btn btn-danger btn-flat">CARI NIK</button>
    @endif
    {{-- {{    }} --}}
    ...
    <div class="form-group{{ $errors->has('nama') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-9">
            {!! Form::text('nama', null, ['class' => 'form-control', 'onkeyup'=>'this.value = this.value.toUpperCase()', 'readonly' => Request::segment(4) ? 'readonly' : null ]) !!}
    ...
    <div class="form-group{{ $errors->has('nik') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-9">
            {!! Form::input('number','nik', null, ['class' => 'form-control', 'required' => 'required']) !!}
            <small class="text-danger">{{ $errors->first('nik') }}</small>
        </div>
    </div>
    <div class="{{ request()->is('registrasi/create_umum') || request()->is('registrasi/igd/umum')  ? 'hidden' : '' }} form-group{{ $errors->has('no_jkn') ? ' has-error' : '' }}">
      {!! Form::label('no_jkn', 'No.JKN', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
    ...
    <div class="form-group{{ $errors->has('tmplahir') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-9">
            <div class="row">
              <div class="col-md-5">
                {!! Form::text('tmplahir', null, ['class' => 'form-control', 'onkeyup'=>'this.value = this.value.toUpperCase()', 'readonly' => Request::segment(4) ? 'readonly' : null ]) !!}
                <small class="text-danger">{{ $errors->first('tmplahir') }}</small>
              </div>
              <div class="col-md-5">
                {!! Form::text('tgllahir', (!empty($pasien->tgllahir)) ? tgl_indo($pasien->tgllahir) : null, ['class' => 'form-control', 'id'=>'tgllahir', 'style' => Request::segment(4) ? 'pointer-events: none;' : null, 'readonly' => Request::segment(4) ? 'readonly' : null ]) !!}
    ...
    <div class="form-group{{ $errors->has('alamat') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-5">
            {!! Form::text('alamat', null, ['class' => 'form-control', 'onkeyup'=>'this.value = this.value.toUpperCase()','required' => 'required']) !!}
            <small class="text-danger">{{ $errors->first('alamat') }}</small>
        </div>
        <div class="col-sm-2">
            {!! Form::text('rt', null, ['class' => 'form-control', 'placeholder'=>'RT','required' => 'required']) !!}
		...
		<div class="form-group{{ $errors->has('province_id') ? ' has-error' : '' }}">
		    ...
		    <div class="col-sm-9">
		        {!! Form::select('province_id', $provinsi, null,['class' => 'form-control select2', 'style'=>'width:100%', 'placeholder'=>' ']) !!}
        ...
        <div class="col-sm-9">
          <select class="form-control select2" name="village_id" id="village_id">
            @if (!empty ($pasien->village_id))
              <option value="{{ @$pasien->village_id }}"></option>
            @endif
          </select>
  ...
  @if (Request::is('pasien/'.@$id.'/edit'))
    <div class="form-group{{ $errors->has('triage') ? ' has-error' : '' }}">
      {!! Form::label('triage', 'Triage A/n', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
        <select class="form-control select2" id="triage" name="triage">
          <option value="">-- Pilih --</option>
          @foreach ($triage as $tria)
            @php
              $fisik = json_decode($tria->fisik, true);
            @endphp
            <option value="{{ $tria->id }}">{{ @$fisik['namaPasien'] }}</option>
          @endforeach
        </select>
        <small class="text-danger">{{ $errors->first('triage') }}</small>
      </div>
    </div>
    <div class="form-group{{ $errors->has('tgl_reg') ? ' has-error' : '' }}">
      {!! Form::label('tgl_reg', 'Tanggal reg', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
          <select class="form-control select2" id="tgl_reg" name="tgl_reg">
              <option value="">-- Pilih --</option>
              @foreach ($tgl_reg as $tgl)
                  <option value="{{ $tgl->id }}">{{ $tgl->created_at->format('d-m-Y H:i:s') }}</option>
              @endforeach
          </select>
          <small class="text-danger">{{ $errors->first('tgl_reg') }}</small>
      </div>
    </div>  
  @endif
  </div>
  ...
  <div class="col-md-6">
    <div class="form-group{{ $errors->has('kelamin') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-9">
            {!! Form::select('kelamin', ['L'=>'Laki-laki', 'P'=>'Perempuan'], null, ['class' => 'form-control select2', 'style'=>'width:100%']) !!}
    ...
    <div class="form-group{{ $errors->has('nohp') ? ' has-error' : '' }}">
      ...
      <div class="col-sm-9">
        {!! Form::text('nohp', null, ['class' => 'form-control','required' => 'required']) !!}
    ...
    <div class="form-group{{ $errors->has('ibu_kandung') ? ' has-error' : '' }}">
        ...
        <div class="col-sm-9">
            {!! Form::text('ibu_kandung', @$pasien->ibu_kandung ? @$pasien->ibu_kandung :'-' , ['class' => 'form-control', 'onkeyup'=>'this.value = this.value.toUpperCase()']) !!}
    ...
    <div class="form-group{{ $errors->has('namakeluarga') ? ' has-error' : '' }}">
      {!! Form::label('namakeluarga', 'Penanggung Jawab', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
        {!! Form::text('namakeluarga', null, ['class' => 'form-control']) !!}
        <small class="text-danger">{{ $errors->first('namakeluarga') }}</small>
      </div>
    </div>

    <div class="form-group{{ $errors->has('hubungan') ? ' has-error' : '' }}">
      {!! Form::label('hubungan', 'Hub Keluarga', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
    ...
    <div class="form-group{{ $errors->has('nodarurat') ? ' has-error' : '' }}">
      {!! Form::label('nodarurat', 'No. HP Darurat', ['class' => 'col-sm-3 control-label']) !!}
      <div class="col-sm-9">
        {!! Form::text('nodarurat', null, ['class' => 'form-control']) !!}
        <small class="text-danger">{{ $errors->first('nodarurat') }}</small>
      </div>
    </div>
    ...
    <div class="form-group{{ $errors->has('ttd') ? ' has-error' : '' }}">
      ...
      <div class="col-sm-9 d-flex align-items-center">
          {!! Form::file('ttd', ['class' => 'form-control w-50']) !!}
  
          @if (!empty($pasien->tanda_tangan))
              <span class="ms-2 text-success">Pasien sudah memiliki tanda tangan</span>
              <input type="hidden" name="ttd_old" value="{{ $pasien->tanda_tangan }}">
          @endif
      </div>
