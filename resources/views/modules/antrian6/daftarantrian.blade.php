...
@section('header')
  <h1 style="font-size: 20pt;color:black;font-weight: bold;">Front Office - Daftar Antrian Loket 6</h1>
@endsection
                        ...
                        @foreach ($terpanggil as $key => $d)
                          <tr>
                            ...
                            <td>
                              ...
                              @if (cek_registrasi($d->id, 6) < 1)
                                @if ($d->status <= 2)
                                <div class="btn-group" style="min-width:0px !important">
                                  <a href="{{ route('antrian6.panggilkembali',$d->id) }}" class="btn btn-info btn-sm btn-flat"><i class="fa fa-microphone"></i></a>
...
<script type="text/javascript">
  $(document).ready(function() {
    setInterval(function () {
      $('#daftarantrian').load("{{ route('antrian6.daftarpanggil') }}");
    },5000);
  });
  ...
  function searchPasien(antrian_id){
    ...
    table = $('#tablePasien').DataTable({
      ...
      processing  : true,
      serverSide  : true,
      ajax: '/pasien/search-pasien/'+antrian_id+'/'+6,
      columns: [
          {data: 'nama'},
          {data: 'no_rm'},
          {data: 'no_rm_lama'},
