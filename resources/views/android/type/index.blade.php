            ...
            <div class="modal-footer" style="margin:0px; border-top:0px; text-align:center;">
                ...
                <form method="POST" action="" id="form-update" style="display:none;">
                    ...
                    <div style="display: grid;margin-bottom: 30px;">
                        <div class="form-group{{ $errors->has('type_nama') ? ' has-error' : '' }}">
                            {!! Form::label('Nama Halaman', 'Nama Halaman', ['class' => 'col-sm-3 control-label']) !!}
                            <div class="col-sm-9">
                                {!! Form::text('type_nama', null , ['class' => 'form-control']) !!}
                                <small class="text-danger">{{ $errors->first('type_nama') }}</small>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function(){
        $(document).on('click','.edit-data', function(){
            ...
            let url = window.location.origin+'/android/type/'+id;
            let source = eval($(this).attr('data-source'));
            jQuery('#modal-data').modal('show', {
                backdrop: 'static'
            });
            $('#modal-data-title').html('Edit Halaman Baru');
            $('input[name="id"]').val(id);
            $('input[name="type_nama"]').val(source[0].type_nama);
            $('#modal-data-title').html('Update Halaman Baru');
        ...
        $(document).on('click','.add-data', function(){
            let id = $(this).attr('data-id');
            let url = window.location.origin+'/android/type';
            let source = eval($(this).attr('data-source'));
            jQuery('#modal-data').modal('show', {
                backdrop: 'static'
            });
            $('#modal-data-title').html('Tambah Halaman Baru');
