@extends('master')
@section('header')
  <h1>Setting Loket Antrian Dokter <small></small></h1>
@endsection
          ...
          <tbody>
            @foreach ($dokter as $key => $d)
              <tr>
                ...
                <td>
                  @if (@$d->kd->praktik == 'Y')
                    <a href="{{ url('frontoffice/tutup-praktik-dokter/'.@$d->kd->id) }}" onclick="return confirm('Non Aktifkan Antrian <b>{{ strtoupper($d->nama) }}</b> ?')" class="btn btn-success btn-sm btn-flat"><i class="fa fa-check"></i></a>
                  @else
                    <a href="{{ url('frontoffice/buka-praktik-dokter/'.@$d->kd->id) }}" onclick="return confirm('Aktifkan Antrian {{ strtoupper($d->nama) }}?')" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-remove"></i></a>
                  @endif
                </td>
                <td>{{ substr(@$d->kd->buka,0,-3) }}</td>
                <td>{{ substr(@$d->kd->tutup,0,-3) }}</td>
                <td><span style="color:red">{{@$d->kd->monday}}</span> | <span style="color:blue">{{@$d->kd->jkn_monday}}</span></td>
                <td><span style="color:red">{{@$d->kd->tuesday}}</span> | <span style="color:blue">{{@$d->kd->jkn_tuesday}}</span></td>
                <td><span style="color:red">{{@$d->kd->wednesday}}</span> | <span style="color:blue">{{@$d->kd->jkn_wednesday}}</span></td>
                <td><span style="color:red">{{@$d->kd->thursday}}</span> | <span style="color:blue">{{@$d->kd->jkn_thursday}}</span></td>
                <td><span style="color:red">{{@$d->kd->friday}}</span> | <span style="color:blue">{{@$d->kd->jkn_friday}}</span></td>
                <td><span style="color:red">{{@$d->kd->saturday}}</span> | <span style="color:blue">{{@$d->kd->jkn_saturday}}</span></td>
                <td>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label for="namadokter" class="col-md-3 control-label" readonly >Nama Dokter</label>
                  <div class="col-md-9">
...
@section('script')
  ...
  $(".skin-blue").addClass( "sidebar-collapse" );
    function editKuota(id) {
      ...
      $('.modal-title').text('Ubah Kuota Dokter');

      $.ajax({
        url: '/frontoffice/get-dokter/' + id,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
          ...
          // ambil data dari relasi kuota_dokter
          let q = data.kd || {};

          $('input[name="kuota"]').val(q.kuota ?? '');
          $('input[name="kuota_online"]').val(q.kuota_online ?? '');
          $('input[name="buka"]').val(q.buka ?? '');
          $('input[name="tutup"]').val(q.tutup ?? '');
          $('select[name="loket"]').val(q.loket ?? '');
          $('input[name="bagian"]').val(q.kode_loket ?? '');

          $('input[name="senin"]').val(q.monday ?? '');
          $('input[name="selasa"]').val(q.tuesday ?? '');
          $('input[name="rabu"]').val(q.wednesday ?? '');
          $('input[name="kamis"]').val(q.thursday ?? '');
          $('input[name="jumat"]').val(q.friday ?? '');
          $('input[name="sabtu"]').val(q.saturday ?? '');
          $('input[name="minggu"]').val(q.sunday ?? '');

          $('input[name="jkn_senin"]').val(q.jkn_monday ?? '');
          $('input[name="jkn_selasa"]').val(q.jkn_tuesday ?? '');
          $('input[name="jkn_rabu"]').val(q.jkn_wednesday ?? '');
          $('input[name="jkn_kamis"]').val(q.jkn_thursday ?? '');
          $('input[name="jkn_jumat"]').val(q.jkn_friday ?? '');
          $('input[name="jkn_sabtu"]').val(q.jkn_saturday ?? '');
          $('input[name="jkn_minggu"]').val(q.jkn_sunday ?? '');
        }
      });
    }
    ...
    function saveKuota() {
      $.ajax({
        url: '/frontoffice/save-kuota-dokter',
        type: 'POST',
        dataType: 'json',
        ...
        success: function (data) {
          if(data.sukses == true) {
            ...
            // location.reload();
            if(!alert('Berhasil Update Loket Antrian Dokter')){location.reload()}
          }
        }
      });
    }
    ...
    function savePraktik() {
      $.ajax({
        url: '/frontoffice/save-edit-praktik-dokter',
        type: 'POST',
        dataType: 'json',
