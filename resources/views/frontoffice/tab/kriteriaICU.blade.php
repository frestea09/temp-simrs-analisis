          ...
          <div class="table-responsive">
            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                <tr>
                  ...
                  <th>Lihat Hasil Kriteria Pasien Masuk</th>
                </tr>
                @if (count($masuk) > 0)
                  @foreach ($masuk as $key=>$m)
                  <tr>
                    <td>{{$key+1}}</td>
                    <td><a target="_blank" href="{{url('/emr-print/cetak-kriteria-masuk-icu/'.$unit.'/'.$m->registrasi_id.'/'.$m->id)}}">{{ $m->created_at }}</a></td>
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Kriteria Pasien Masuk</i></td>
                </tr>
                @endif
            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                <tr>
                  <th style="width: 80px;">No</th>
                  <th>Lihat Hasil Kriteria Pasien Keluar</th>
                </tr>
                @if (count($keluar) > 0)
                  @foreach ($keluar as $key=>$k)
                  <tr>
                    <td>{{$key+1}}</td>
                    <td><a target="_blank" href="{{url('/emr-print/cetak-kriteria-keluar-icu/'.$unit.'/'.$k->registrasi_id.'/'.$k->id)}}">{{ $k->created_at }}</a></td>
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Kriteria Pasien Keluar</i></td>
                </tr>
                @endif
