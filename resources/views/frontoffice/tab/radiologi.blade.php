          ...
          <div class="table-responsive">
            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                ...
                @if (count($ekspertise) > 0)
                  @foreach ($ekspertise as $key=>$p)
                  <tr>
                    <td>{{$key+1}}</td>
                    @if ($p->poli_id == 1)
                      {{-- <td><a target="_blank" href="{{url('/radiologi/cetak-ekpertise/'.$p->id.'/'.$reg->id.'/'.@$p->folio_id)}}">{{ $p->uuid }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}} - Radiologi</small></td> --}}
                      <td><a target="_blank" href="{{ url("/dokumen_tte/".@$p->tte)}}">{{ $p->uuid }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}} - Radiologi</small></td>
                    @elseif ($p->poli_id == 38)
                      <td><a target="_blank" href="{{url('/radiologi-gigi/cetak-ekpertise/'.$p->id.'/'.$reg->id)}}">{{ $p->uuid }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}} - Radiologi Gigi</small></td>
                    @endif
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Hasil Radiologi Ekspertise</i></td>
                </tr>
                @endif
