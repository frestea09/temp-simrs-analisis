...
@section('content')
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">Periode Grouping</h3>
    </div>
    <div class="box-body">
            ...
            <tbody>
              ...
              @endphp
                <tr>
                  ...
                  <td>{{ totalEklaim($d->id) }}</td>
                  <td>{{ kodeInacbgs($d->id) }}</td>
                  {{-- <td class="text-center">
                    <div class="btn-group">
                      <button type="button" class="btn btn-md btn-primary">Detail</button>
                      <button type="button" class="btn btn-md btn-primary dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                          <span class="caret"></span>
                      </button>
                      <ul class="dropdown-menu dropdown-menu-right" role="menu">
                        <li><a class="btn btn-md" onclick="viewDetailTindakan({{ $d->id }}, 'Tindakan')">Tindakan</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailObat({{ $d->id }}, 'Resep')">Obat</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailDiagnosa({{ $d->id }}, 'Diagnosa')">Diagnosa</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailRagiologi({{ $d->id }}, 'Radiologi')">Ekspertise</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailRagiologi({{ $d->id }}, 'Radiologi')">Hasil LAB</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailRagiologi({{ $d->id }}, 'Radiologi')">SPRI</a></li>
                        <li><a class="btn btn-md" onclick="viewDetailRagiologi({{ $d->id }}, 'Radiologi')">ECHO</a></li>
                      </ul>
                    </div>
                  </td> --}}
                  ...
                  <td class="text-center">
                    <div class="btn-group">
                      ...
                      <ul class="dropdown-menu dropdown-menu-right" role="menu">
                        {{-- <li><a class="btn btn-md" onclick="viewPDF({{ $d->id }}, 'ekspertise')">Ekspertise</a></li>
                        <li><a class="btn btn-md" onclick="viewPDF({{ $d->id }}, 'laboratorium')">Hasil LAB</a></li>
                        <li><a class="btn btn-md" onclick="viewPDF({{ $d->id }}, 'echocardiogram')">Echocardiogram</a></li>
                        <li><a class="btn btn-md" onclick="viewPDF({{ $d->id }}, 'spri')">SPRI</a></li>
                        <li><a class="btn btn-md" onclick="viewPDF({{ $d->id }}, 'biaya')">Detail Biaya</a></li>
                        <li><a class="btn btn-md" onclick="viewPDF({{ cekPenjualan($d->id) }},'obat')">Detail Obat</a></li> --}}
                        @if (cekEkpertise($d->id))
                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/radiologi/cetak-ekpertise-vedika/'.$d->id) }}">Ekspertise</a></li>
                        @else
                        <li><a class="btn btn-md" href="#"><del>Ekspertise</del></a></li>
                        @endif
                        
                        @if (cekLab($d->id))
                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/pemeriksaanlab/cetakAll-vedika/'.$d->id) }}">Hasil LAB</a></li>
                        @else
                        <li><a class="btn btn-md" href="#"><del>Hasil LAB</del></a></li>
                        @endif
                        
                        @if (cekEcho($d->id))
                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/echocardiogram/cetak-echocardiogram/'.$d->id) }}"">Echocardiogram</a></li>
                        @else
                        <li><a class="btn btn-md" href="#"><del>Echocardiogram</del></a></li>
                        @endif
                        {{-- <li><a class="btn btn-md"  target="_blank" href="{{ url('/kasir/cetak-verifikasi/'.$d->id) }}">Detail Biaya</a></li>
                        <li><a class="btn btn-md" href="#"><del>Detail Biaya</del></a></li> --}}
                        
                        @if (substr($d->status_reg,0,1) == 'I')
                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/spri/'.$d->id) }}"> SPRI</a></li>
                        @else
                        <li><a class="btn btn-md" href="#"><del>SPRI</del></a></li>
                        @endif

                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/kasir/cetak-verifikasi/'.$d->id) }}">Detail Biaya</a></li>
                        <li><a class="btn btn-md"  target="_blank" href="{{ url('/farmasi/cetak-detail-vedika/'.$d->id) }}">Detail Obat</a></li>
                      </ul>
                    </div>
                  </td>
                  <td>
                      <div class="input-group-btn">
                        <a class="btn btn-sm bg-orange btn-flat" onclick="dokumenView({{ $d->id }})" >View</a>
                      </div>
                  </td>
                  <td>
                    @if ($d->no_sep <> null)
                  ...
                  {{--  <td>
                    @if ($d->no_sep <> null)
                    <a href="{{ url('frontoffice/e-claim/cetak-full/'.$d->id) }}" class="btn btn-md btn-danger btn-flat" target="_blank"><i class="fa fa-print"></i></a>
                    @endif
                  </td>  --}}
                </tr>
              @endforeach
