@extends('master')
@section('header')
  <h1>Bridging INACBG Rawat Inap<small></small></h1>
@endsection
...
@section('content')
  ...
  <div class="box box-primary">
    ...
    <div class="box-body">
      ...
      {!! Form::open(['method' => 'POST', 'url' => 'frontoffice/e-claim/dataRawatInap', 'class'=>'form-hosizontal']) !!}
      <div class="row">
          <div class="col-md-6">
          ...
          <tbody>
            @foreach ($irna as $key => $d)
              <tr>
                ...
                <td>
                  @if ($d->no_sep <> '')
                    @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->count() == 1)
                      <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id.'/?edit=1') }}" class="btn btn-default btn-sm btn-flat" title="EDIT KLAIM"><b>{{ $d->no_sep }}</b></a>
                    @else
                      <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id) }}" class="btn btn-default btn-sm btn-flat"><b>{{ $d->no_sep }}</b></a>
                    @endif
                  @else
                    @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->count() == 1)
                      <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id.'/?edit=1') }}" class="btn btn-default btn-sm btn-flat" title="EDIT KLAIM"><b>EDIT KLAIM</b></a>
                    @else
                      <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id) }}" class="btn btn-default btn-sm btn-flat"><b>PROSES</b></a>
                    @endif
                  @endif
                </td>
                <td>
                  @if ($d->no_sep <> '')
                    @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->count() == 1)
                      <a href="{{ url('frontoffice/e-claim/bridging-irna-idrg/'.$d->registrasi_id.'/?edit=1') }}" class="btn btn-default btn-sm btn-flat" title="EDIT KLAIM"><b>{{ $d->no_sep }}</b></a>
                    @else
                      <a href="{{ url('frontoffice/e-claim/bridging-irna-idrg/'.$d->registrasi_id) }}" class="btn btn-default btn-sm btn-flat"><b>{{ $d->no_sep }}</b></a>
                    @endif
                  @else
                    @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->count() == 1)
                      <a href="{{ url('frontoffice/e-claim/bridging-irna-idrg/'.$d->registrasi_id.'/?edit=1') }}" class="btn btn-default btn-sm btn-flat" title="EDIT KLAIM"><b>EDIT KLAIM</b></a>
                    @else
                      <a href="{{ url('frontoffice/e-claim/bridging-irna-idrg/'.$d->registrasi_id) }}" class="btn btn-default btn-sm btn-flat"><b>PROSES</b></a>
                    @endif
                  @endif
                </td>
                <td>{{ $d->tgl_keluar }}</td>
                <td>
                  <a href="{{ url('/emr-soap-rawatinap/anamnesis/umum/'.$d->registrasi_id) }}" class="btn btn-success btn-flat btn-sm"><i class="fa fa-paste"></i></a>
                </td>
                @role('administrator')
                  <td>
                    <button class="btn btn-flat btn-sm btn-success" onclick="statusInaCbg({{ $d->registrasi_id }})"><i class="fa fa-folder-open"></i></button>
                  </td>
                @endrole
                {{--  <td class="text-center">
                   @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->where('kirim_dc', 'N')->count() == 1)
                      <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id) }}" class="btn btn-default btn-sm btn-flat"><i class="fa fa-check"></i></a>
                   @else
                    <a href="{{ url('frontoffice/e-claim/bridging-irna/'.$d->registrasi_id) }}" class="btn btn-primary btn-sm btn-flat"><i class="fa fa-flash"></i></a>
                  @endif
                </td>  --}}
                <td class="text-center">
                  @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->where('kirim_dc', 'N')->count() == 1)
                    <a href="{{ url('/eklaim-detail-bridging/'.$d->registrasi_id) }}" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print"></i></a>
                  @endif
                </td>
                <td class="text-center">
                  @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->where('kirim_dc', 'N')->count() == 1)
                    <button type="button" onclick="finalKlaim({{ $d->registrasi_id }})" class="btn btn-sm btn-success btn-flat"><i class="fa  fa-paper-plane-o"></i></button>
                  @endif
                </td>
                <td class="text-center">
                    @if (\App\Inacbg::where('registrasi_id', $d->registrasi_id)->count() == 1)
                ...
                <td class="text-center">
                  <a href="{{ url('frontoffice/uploadDokument/'.$d->registrasi_id) }}" class="btn btn-sm btn-flat btn-warning"><i class="fa fa-upload"></i></a>
                </td>
              </tr>
            @endforeach
  ...
  <script type="text/javascript">
    ...
    function saveFinalKlaim(no_sep, coder_nik) {
      ...
      .done(function(resp) {
        if (resp.sukses == true) {
          ...
          alert('No. SEP '+resp.no_sep+' sdh di Final dan Kirim DC')
        }
      });

    }

    function saveFinalKlaimInacbg(no_sep, coder_nik) {
      $.ajax({
    ...
    function hapusClaim(no_sep, coder){
      if(confirm('Yakin akan di hapus?')){
        $.get('/inacbg/hapus-klaim-irna/'+no_sep+'/'+coder, function(resp){
          if(resp.proses == true){
            location.reload()
          }
        })
