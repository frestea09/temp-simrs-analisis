@extends('master')
@section('header')
<h1>Table Mapping CONF.RL310</h1>
@endsection
...
<div class="box box-primary">
    <div class="box-header with-border">
        ...
        <!-- <a href="{{route('mastermapping_confrl310.index')}}" class="btn btn-default btn-flat">
            <i class="fa fa-plus"></i> List Mapping CONF.RL310 
        </a>         -->
    </div>
    <div class="box-body">
        <div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class='table-responsive'>
                                <table id='table_conf_rl310' class='table table-striped table-bordered table-hover table-condensed'>
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Kegiatan</th>
...
<script type="text/javascript">
    $(document).ready(function(e) {
        var table = $('#viewMapping').DataTable({
            ...
            ajax: {
                url: '{{ route("data-mastermapping_confrl310") }}',
                data: {
                    jenis: 'laporan'
                }
            },
    ...
    function editForm(id) {
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log(data);
                $('input[name="id"]').val(data.id_conf_rl310)
    ...
    function saveForm() {
        ...
        if (id == '') {
            url = '{{ route("mastermapping_confrl310.store") }}';
            type = 'POST';
        } else {
    ...
    function detailconfrl310(id) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                $('.modal-title').text('Conf.RL310  ' + data.kegiatan)
            }
        });


        $('#table_conf_rl310').DataTable({
            'language': {
            ...
            processing: true,
            serverSide: true,
            ajax: '/list-detail-conf-rl310/' + id,
            columns: [{
                    data: 'DT_RowIndex',
                    searchable: false
                },
    ...
    function hapusMapping(conf_rl310_id, tarif_id) {
        if (confirm('Yakin akan dihapus') == true) {
            $.ajax({
                    url: '/hapus-conf-rl310/' + tarif_id,
                    type: 'GET',
                    dataType: 'json',
                })
                .done(function(data) {
                    detailconfrl310(conf_rl310_id)
                });
