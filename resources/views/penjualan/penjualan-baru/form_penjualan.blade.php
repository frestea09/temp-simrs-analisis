...
@section('content')
    <div class="box box-primary">
        ...
        <div class="box-body">
            ...
            <table class='table-striped table-bordered table-hover table-condensed table'>
                <tbody>
                    <tr>
                        ...
                        <td>
                            {{ strtoupper($pasien->nama) }} / {{ $pasien->no_rm }} /
                            {{ !empty($pasien->tgllahir) ? hitung_umur($pasien->tgllahir) : 'tgl lahir kosonglo' }}
                            <button type="button" id="historipenjualan" data-registrasiID="{{ $idreg }}"
                    ...
                    <tr>
                        @if (@$reg->status_reg == 'I2')
                            <td>Ruangan</td>
                            <td>{{ @$ranap->kamar_id != '' ? @baca_kamar(@$ranap->kamar_id) : '' }}</td>
                        @else
                            <td>Klinik</td>
                            <td>{{ strtoupper(baca_poli($reg->poli_id)) }}</td>
                        @endif
                    </tr>
                    ...
                    <tr>
                        <td>Alamat</td>
                        <td>{{ strtoupper($pasien->alamat) }} RT. {{ $pasien->rt }} / RW. {{ $pasien->rw }}
                            {{ baca_kelurahan($pasien->village_id) }} {{ baca_kecamatan($pasien->district_id) }}</td>
                    </tr>
                </tbody>
            </table>
            @if (isset($resep->id))
            ...
            <form method="POST" id="formPenjualan" class="form-horizontal">
                ...
                {!! Form::hidden('pasien_id', $pasien->id) !!}
                {!! Form::hidden('idreg', $idreg) !!}
                {!! Form::hidden('penjualan_id', session('penjualanid')) !!}
                    ...
                    <div class="col-sm-4">
                        <select class="form-control select2" name="uang_racik" style="width: 100%;">
                            ...
                            @foreach ($tipe_uang_racik as $d)
                                ...
                                <option value="{{ $d->id }}" {{ $d->id == 2 ? 'selected' : '' }}>
                                    {{ $d->nama }}</option>
                            @endforeach
                        </select>
              ...
              <div class="input-group">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-3">
                        <div class="input-group">
                            ...
                            <span class="input-group-btn">
                                <a href="{{ url('/penjualan/master-cara-minum') }}" class="btn btn-primary"
                    ...
                    <div class="form-group{{ $errors->has('cetak') ? ' has-error' : '' }}">
                        <div class="col-sm-3">
                            <button type="button" class="btn btn-primary btn-flat"
                                onclick="addItem(this)">Tambahkan</button>
                        </div>
                    </div>
                </div>
                <div class="form-group{{ $errors->has('informasi1') ? ' has-error' : '' }}">
                ...
                <div>
                    <div class='table-responsive'>
                        <table class='table-striped table-bordered table-hover table-condensed table'>
                            ...
                            <tbody id="dataObatPenjualan">
                                
                                @foreach ($detail as $key => $d)
                                    <tr>
                                        <td class="text-center">{{ $no++ }}</td>
                                        <td>{{ !empty($d->logistik_batch_id) ? baca_batches($d->logistik_batch_id) : '' }}
                                        </td>
                                        ...
                                        <td class="text-right">
                                            {{ number_format(($d->hargajual + $d->hargajual_kronis) / ($d->jumlah + $d->jml_kronis)) }}
                                        </td>
                                        <td class="text-right">{{ number_format($d->uang_racik) }}</td>
                                        <td class="text-right">{{ number_format($d->hargajual_kronis + $d->hargajual) }}
                                        </td>
                                        <td>{{ $d->etiket }}</td>
                                        
                                        <td>{{ $d->cetak }}</td>
                                        <td>
                                            <a href="{{ url('penjualan/deleteDetailBaru/' . @$d->id . '/' . @$pasien->id . '/' . @$idreg . '/' . @$penjualan->id) }}"
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <th class="text-right">
                                        {{ number_format($detail->sum('hargajual') + count($detail) * $d->uang_racik) }}
                                    </th>
                                </tr>
                            </tfoot>
                        </table>
                ...
                <form method="POST" id="formTotalPenjualan" class="form-horizontal">
                    {{ csrf_field() }}
                    <input class="form-control" type="hidden" name="penjualan_id"
                        value="{{ !empty($penjualanid) ? $penjualanid : null }}">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group{{ $errors->has('pembuat_resep') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-7">
                                {!! Form::select('pembuat_resep', $apoteker, null, ['class' => 'form-control select2', 'style' => 'width: 100%']) !!}
                    ...
                    <div class="col-sm-6">
                        <div class="form-group{{ $errors->has('response_time') ? ' has-error' : '' }}">
                            {!! Form::label('response_time', 'Response Time', ['class' => 'col-sm-5 control-label']) !!}
                            <div class="col-sm-7">
                                <div style="display: flex; flex-flow: row; align-items:center; gap: 10px">
                                    <div style="width: 100%">
                                        <input type="number" class="form-control" placeholder="Menit" name="response_time" id="responseTime" value="1">
                                    </div>
                                </div>
                                <small class="text-danger">{{ $errors->first('response_time') }}</small>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6">
                        <div class="form-group">
                            {!! Form::label('tanggal', 'Tanggal ', ['class' => 'col-sm-5 control-label']) !!}
                            <div class="col-sm-7">
                    ...
                    <div class="col-sm-12 ">
                        ...
                        <button type="button" onclick="savePenjualan()" class="btn btn-success btn-flat pull-right">SIMPAN</button>
                    </div>
                    {!! Form::close() !!}
        </div>
    ...
    <div class="modal fade" id="showHistoriPenjualan" tabindex="-1" role="dialog" aria-labelledby=""
        aria-hidden="true">
        ...
        <div class="modal-dialog modal-md">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    <a href="" class="btn btn-primary btn-flat" id="non_kronis" target="_blank"> <i
                            class="fa fa-file-pdf-o"></i> CETAK FAKTUR NON KRONIS </a>
                    <a href="" class="btn btn-danger btn-flat" id="kronis" target="_blank"> <i
                            class="fa fa-file-pdf-o"></i> CETAK FAKTUR KRONIS </a>
                </div>
                <div class="modal-footer">
            ...
            $.ajax({
                ...
                .done(function(resp) {
                    ...
                    if (resp.sukses == true) {
                        ...
                        let idreg = $("input[name='idreg']").val();
                        ...
                        console.log(resp)
                        let tableStr = '';
                        resp.details.forEach(function(detail, index){
                            tableStr +=
                            `<tr>
                                <td class="text-center">${index+1}</td>
                                <td>${detail.nama}</td>
                                <td class="text-center">
                                    <input type="number" id="jumlahObat${detail.id}" class="form-control" onchange="editJumlah(${detail.id})" value="${detail.jumlah}">
                                </td>
                                <td class="text-center">${detail.is_kronis} </td>
                                <td class="text-center">${detail.jumlah}</td>
                                <td class="text-right">
                                    ${(addThousandSeparator(detail.hargajual / (detail.jumlah))) }
                                </td>
                                <td class="text-right">${detail.uang_racik}</td>
                                <td class="text-right">${addThousandSeparator(detail.hargajual)}</td>
                                <td>${detail.etiket}</td>
                                <td>${detail.cetak}</td>
                                <td>
                                    <a href="/penjualan/deleteDetailBaru/${detail.id}/${pasien_id}/${idreg}/${penjualan_id}"
                                        class="btn btn-sm btn-danger"><i class="fa fa-trash-o"></i></a>
                                </td>
                            </tr>`
                        })
                        $('#dataObatPenjualan').html(tableStr);
                        // location.reload();
                    }else{
                        alert(resp.error);
                    }
                ...
                $.ajax({
                    ...
                    .done(function(resp) {
                        ...
                        if (resp.sukses == true) {
                            ...
                            if (resp.jenis == 'J') {
                                $('#selesai').attr('href', '/farmasi/penjualan')
                            } else if (resp.jenis == 'G') {
                                
                                $('#selesai').click(function (e) {
                                    e.preventDefault()
                                    $('#showCetak').modal('hide')
                                })
