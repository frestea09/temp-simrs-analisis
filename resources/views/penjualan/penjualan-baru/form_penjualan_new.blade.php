...
@section('content')
  <div class="box box-primary">
    ...
    <div class="form-group{{ $errors->has('is_kronis') ? ' has-error' : '' }}">
        ...
        {{-- {!! Form::text('is_kronis', null, ['class' => 'form-control']) !!} --}}
              ...
              <tbody>
                @foreach ($detail as $key => $d)
                  <tr>
                    ...
                    <td class="text-right">{{ number_format($d->hargajual_kronis+$d->hargajual) }}</td>
                    <td>{{ $d->etiket }}</td>
                    @if (satusehat())
...
idreg = "<?= $idreg ?>"
  ...
  function savePenjualan() {
    if (confirm('Yakin data penjualan sudah benar?')) {
      ...
      $.ajax({
        url: '/penjualan/simpan-total-penjualan-new',
        type: 'POST',
        dataType: 'json',
        data: data,
        beforeSend: function () {
