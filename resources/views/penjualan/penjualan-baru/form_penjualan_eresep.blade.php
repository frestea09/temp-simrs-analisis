...
@section('content')
  <div class="box box-primary">
    ...
    <div class="box-body">
      ...
      @if(isset($resep->id))
      <form action="{{url('penjualan/validasi-eresep')}}" method="POST">
        {{ csrf_field() }}
          ...
          <table class="table table-bordered" style="font-size: 12px !important">
            <thead>
              <tr>
                ...
                <th colspan="3" style="color:blueviolet">NAMA RACIKAN ATAU CARA PEMBUATAN : {{ $resep->nama_racikan }}</th>    
                @endif
              </tr>
              <tr>
                <th>No</th>
            ...
            <tbody>
              ...
              <input type="hidden" name="resep_detail[{{@$b->id}}][cara_minum]" value="{{$b->cara_minum}}" />
              ...
              <tr>
                ...
                <td colspan="2">
                  ...
                  @endif
                  {{ @$b->logistik_batch->master_obat->nama }}
                </td>
                <td>{{ @$b->logistik_batch->stok }}</td>
                <td>{{ @$b->qty }}</td>
                <td>
                  {{-- <select class="form-control" name="resep_detail[{{@$b->id}}][kronis]" >
                  <option value="Y" {{@$b->kronis == 'Y' ? 'selected' : '' }}>Ya</option>
                  <option value="N" {{@$b->kronis == 'N' ? 'selected' : '' }} >Tidak</option>
                ...
                <td>{{ number_format(@$b->logistik_batch->hargajual_umum) }}</td>
                <td>{{ number_format(@$b->logistik_batch->hargajual_umum*$b->qty) }}</td>
                {{-- <td><input class="form-control" max="{{$b->qty}}" min="0" type="number" style="width: 70px;" value="{{$b->is_empty ? '0': $b->qty}}" name="resep_detail[{{$b->id}}][qty]"></td> --}}
                <td><input class="form-control" min="0" type="number" style="width: 70px;" value="{{$b->is_empty ? '0': $b->qty}}" name="resep_detail[{{@$b->id}}][qty]"></td>
                {{-- <td>{{ $b->cara_bayar }}</td> --}}
                {{-- <td>
                  <select class="form-control" name="resep_detail[{{@$b->id}}][tiket]" >
                    @foreach ($tiket as $d)
                      <option value="{{ @$d->nama }}" {{@$d->nama == $b->tiket ? 'selected' : '' }} >{{ @$d->nama }}</option>
                    @endforeach
                  </select>  
                </td> --}}
                <td>
                    <input class="form-control" type="text" style="width: 100%;" value="{{$b->cara_minum}}" name="resep_detail[{{@$b->id}}][cara_minum]">
                  {{-- <select class="form-control" name="resep_detail[{{@$b->id}}][cara_minum]">
                    <option value=""></option>
                    @foreach ($cara_minum as $key=>$d)
                      <option value="{{ $d }}"  {{$d == $b->cara_minum ? 'selected' : '' }}>{{ $d }}</option>
                    @endforeach
                  </select> --}}
                </td>
                <td>
                  <input class="form-control" type="text" tyle="width: 100%;" value="{{$b->takaran}}" name="resep_detail[{{@$b->id}}][takaran]">
                  {{-- <select class="form-control" name="resep_detail[{{$b->id}}][takaran]"> --}}
                  {{-- <option value=""></option>
                  @foreach ($takaran as $key=>$d)
                    <option value="{{ $d }}"  {{$d == $b->takaran ? 'selected' : '' }}>{{ $d }}</option>
                  @endforeach --}}
                </select>
                </td>
                {{-- <td>
                  ...
                  <td>
                    ...
                    {{-- <select class="form-control" name="resep_detail[{{@$b->id}}][informasi]" >
                      <option value="Ketepatan identitas pasien">
                      Ketepatan identitas pasien
                      </option>
                      <option value="Ketepatan Obat">
                        Ketepatan Obat
                      </option>
                      <option value="Ketepatan Dosis">
                        Ketepatan Dosis
                      </option>
                      <option value="Ketepatan frekuensi">
                        Ketepatan frekuensi
                      </option>
                      <option value="Ketepatan aturan minum/makan obat">
                        Ketepatan aturan minum/makan obat
                      </option>
                      <option value="Ketepatan waktu pemberian">
                        Ketepatan waktu pemberian
                      </option>
                      <option value="Duplikasi pengobatan">
                        Duplikasi pengobatan
                      </option>
                      <option value="Potensi alergi atau sensitivitas">
                        Potensi alergi atau sensitivitas
                      </option>
                      <option value="Interaksi antara obat dan obat lain atau dengan makanan">
                        Interaksi antara obat dan obat lain atau dengan makanan
                      </option>
                      <option value="Variasi kriteria penggunaan obat dari rumah sakit (obat dagang,obat generik)">
                        Variasi kriteria penggunaan obat dari rumah sakit (obat dagang,obat generik)
                      </option>
                      <option value="Berat badan pasien">
                        Berat badan pasien
                      </option>
                      <option value="Kontra indikasi">
                        Kontra indikasi
                      </option>
                  ...
                  {{-- <td>
                    <input class="form-control" type="text" style="width: 70px;" value="" name="resep_detail[{{@$b->id}}][catatan]">
                  </td> --}}
                  <td>
                ...
                {{-- <td>
                  <select class="form-control" name="resep_detail[{{@$b->id}}][uang_racik]">
                    <option value=""></option>
                    @foreach ($tipe_uang_racik as $d)
                      <option value="{{ @$d->id }}" {{ ( @$d->id == 3) ? 'selected' : '' }}>{{ $d->nama }}({{ $d->nominal }})</option>
                    @endforeach
                  </select>
                </td> --}}
                {{--<td>
                  <a href="{{ url('penjualan/deleteDetailBaruValidasi/'.$b->id.'/'.$idreg.'/'.$resep->id) }}" class="btn btn-sm btn-danger"><i class="fa fa-trash-o"></i></a>
                </td>--}}
              </tr>
              @endforeach
              <td></td>
              <td>Total Harga:</td>
              {{-- <td colspan="4" style="padding-left:160px">{{ number_format($tot_sementara) }} </td> --}}
            ...
            <tfoot>
              ...
              <tr>
                <td style="width: 300px">
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['1']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['1']" type="checkbox" value="1" id="flexCheckDefault">
                      Ketepatan identitas pasien
                    </label>
                  </div>
                </td>
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['2']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['2']" type="checkbox" value="2" id="flexCheckChecked">
                      Ketepatan Obat
                    </label>
                  </div>
                ...
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['3']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['3']" type="checkbox" value="3" id="flexCheckChecked">
                      Ketepatan frekuensi
                    </label>
                  </div>
                ...
                <td colspan="4">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['4']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['4']" type="checkbox" value="4" id="flexCheckChecked">
                      Ketepatan aturan minum/makan obat
                    </label>
                  </div>
                ...
                <td colspan="4">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['5']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['5']" type="checkbox" value="5" id="flexCheckChecked">
                      Ketepatan waktu pemberian
                    </label>
                  </div>
              ...
              <tr>
                <td colspan="1">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['6']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['6']" type="checkbox" value="6" id="flexCheckChecked">
                      Duplikasi pengobatan
                    </label>
                  </div>
                ...
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['7']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['7']" type="checkbox" value="7" id="flexCheckChecked">
                      Potensi alergi atau sensitivitas
                    </label>
                  </div>
                ...
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['8']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['8']" type="checkbox" value="8" id="flexCheckChecked">
                      Interaksi antara obat dan obat lain atau dengan makanan
                    </label>
                  </div>
                ...
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['9']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['9']" type="checkbox" value="9" id="flexCheckChecked">
                      Variasi kriteria penggunaan obat dari rumah sakit (obat dagang,obat generik)
                    </label>
                  </div>
                ...
                <td colspan="2">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['10']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['10']" type="checkbox" value="10" id="flexCheckChecked">
                      Berat badan pasien
                    </label>
                  </div>
                ...
                <td colspan="3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="flexCheckChecked">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['11']" type="hidden" value="0" id="flexCheckDefault">
                      <input class="form-check-input check-item"  name="resep_detail[{{@$b->id}}][catatan]['11']" type="checkbox" value="11" id="flexCheckChecked">
                      Kontra Indikasi
                    </label>
                  </div>
              ...
              <tr><td class="text-right" colspan="11">
                ...
                {{-- <a onclick="return confirm('Yakin akan validasi obat?')" href="{{url('penjualan/validasi-eresep/'.$pasien->id.'/'.$reg->id.'/'.$resep->id)}}" class="btn btn-success">Validasi</a> --}}
                <button onclick="return confirm('Yakin akan validasi obat?')" type="submit" class="btn btn-success">Validasi</a>
                @endif
      ...
      <form method="POST" id="formPenjualan" class="form-horizontal">
        ...
        {{--  <input type="hidden" name="cara" value="{{ $reg->bayar }}">  --}}
          ...
          {!! Form::label('masterobat_id', 'Pilih Obat', ['class' => 'col-sm-2 control-label']) !!}
            ...
            {{-- <div class="form-group{{ $errors->has('kronis') ? ' has-error' : '' }}">
              {!! Form::label('kronis', 'Kronis', ['class' => 'col-sm-2 control-label']) !!}
              <div class="col-sm-4">
              ...
              <tbody>
                @foreach ($detail as $key => $d)
                  <tr>
                    ...
                    {{-- <td class="text-center"><input type="number" id="jumlahKronis" class="form-control" onchange="editKronis({{ $d->id }})" value="{{ $d->jml_kronis }}"></td> --}}
                    <td class="text-center">{{ $d->jumlah}}</td>
                    {{-- <td class="text-right">{{ number_format(($d->hargajual+$d->hargajual_kronis)/($d->jumlah+$d->jml_kronis)) }}</td> --}}
                    <td class="text-right">{{ @$d->jumlah > 0 ? @number_format((@$d->hargajual)/(@$d->jumlah)) : number_format(@$d->logistik_batch->hargajual_umum) }}</td>
                    <td class="text-right">{{ number_format($d->uang_racik) }}</td>
                    <td class="text-right">{{ number_format($d->hargajual) }}</td>
                    {{-- @else
                      <td class="text-right" colspan="5">Stok habis</td>
                    @endif --}}
      ...
      <div class="modal-dialog">
        ...
        <div class="modal-content">
          ...
          <div class="modal-body row" style="display: grid;">
              ...
              <input type="hidden" class="form-control" name="nik_hidden" id="nik_hidden">
            <div class="form-group">
...
$(".skin-blue").addClass( "sidebar-collapse" );
  ...
  $('#button-proses-tte').click(function(e) {
    ...
    if (confirm('Yakin data penjualan sudah benar?')) {
      ...
      $.ajax({
        url: '/penjualan/simpan-total-penjualan?tte=true',
        type: 'POST',
        dataType: 'json',
        data: data,
        beforeSend: function () {
  ...
  function savePenjualanWithTTE() {
    if (confirm('Apakah anda yakin akan menyimpan dan melakukan proses tanda tangan elektronik?')) {
        let penjualan_id = $("input[name='penjualan_id']").val();
        let registrasi_id = $("input[name='reg_id']").val();
        $.ajax({
  ...
  function editJumlah(id) {
      let index = `#jumlahObat${id}`;
      let data = $(index).val();
      $.ajax({
          headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          },
          url: '/penjualan/editJumlah/'+id,
          type: 'POST',
          dataType: 'json',
          data: {
              jumlah : data,
  ...
  function editStatusKronis(id) {
    let idElement = `#kronis${id}`;
    let data = $(idElement).val();
    $.ajax({
          headers: {
          'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          },
          url: '/penjualan/editStatusKronis/'+id,
          type: 'POST',
          dataType: 'json',
          data: {
              kronis : data,
