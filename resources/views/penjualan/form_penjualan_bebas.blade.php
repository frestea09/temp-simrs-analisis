...
{!! Form::close() !!}
      ...
      @if (session('idpenjualan'))
        {!! Form::open(['method' => 'POST', 'url' => 'penjualan/savedetailbebas', 'class' => 'form-horizontal']) !!}
            {!! Form::hidden('pasien_id', 0) !!}
            {!! Form::hidden('idreg', session('reg_id')) !!}
                ...
                <div class="col-sm-4">
                    <select class="chosen-select" name="masterobat_id" id="masterobat_id">
                      ...
                      @foreach ($barang as $key => $d)
                          <option value="{{ $d->id }}">{{ $d->nama_obat }} | Batch {{ $d->nomorbatch }} |
      ...
      @isset($detail)
            ...
            <tbody>
              @foreach ($detail as $key => $d)
                <tr>
                  ...
                  <td>{{ $d->cetak }}</td>
                  <td>
                    <a href="{{ url('penjualan/deleteDetailbebas/'.$d->id.'/0/'.session('reg_id').'/'.$penjualan->id) }}" class="btn btn-sm btn-danger"><i class="fa fa-trash-o"></i></a>
                  </td>
                </tr>
              @endforeach
            </tbody>
        ...
        {!! Form::open(['method' => 'POST', 'url' => 'penjualan/savetotalbebas']) !!}
          <div class="pull-right">
              <input type="hidden" name="id_penjualan" value="{{ $penjualan->id }}">
              {!! Form::submit("SIMPAN", ['class' => 'btn btn-success btn-flat']) !!}
              {{-- <a href="{{ url('penjualan/savetotalbebas/'.$penjualan->id) }}" class="btn btn-success" onclick="return confirm('Yakin sdh selesai?')"><i class="fa fa-save"></i> SIMPAN</a> --}}
          </div>
          <div class="pull-right">
