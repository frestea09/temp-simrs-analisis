            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                ...
                @if (count($penjualan) > 0)
                  ...
                  <tr>
                    <td>{{$key+1}}</td>
                    <td><a href="{{ url('copy-resep/cetak-detail-copy-resep-farmasi/'.$p->id) }}" class="btn btn-flat btn-sm" target="_blank">{{ $p->no_resep }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}}</small></td>
                  </tr>
                  @endforeach
                @else
                <tr>
