...
<body class="hold-transition skin-red sidebar-mini">
              ...
              <tbody>
                ...
                @if (count($penjualan) > 0)
                  ...
                  <tr>
                    <td>{{$key+1}}</td>
                    @if (isset($non_kronis))
                    <td><a href="{{ url('farmasi/cetak-detail/'.$p->id . '?faktur=true') }}" class="btn btn-flat btn-sm" target="_blank">{{ $p->no_resep }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}}</small></td>
                    @else
                    <td><a href="{{ url('farmasi/cetak-detail/'.$p->id) }}" class="btn btn-flat btn-sm" target="_blank">{{ $p->no_resep }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}}</small></td>
                    @endif
                    <td>
                        <div class="btn btn-flat btn-danger btn-sm" onclick="hapusFaktur('{{$p->no_resep}}')"><i class="fa fa-trash"></i></div>
                    </td>
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Faktur</i></td>
                </tr>
                @endif
  ...
  <script>
        function hapusFaktur(no_faktur){
            let isConfirm = confirm('Yakin akan menghapus faktur ' + no_faktur);
            if(isConfirm){
                window.location.href = '/penjualan/hapus-faktur-baru/' + no_faktur;
            }
        }
  </script>
</body>
