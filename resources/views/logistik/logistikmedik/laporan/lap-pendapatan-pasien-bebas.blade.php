...
@section('content')
  <div class="box box-primary">
    ...
    <div class="box-body">
      <form action="{{ url('logistikmedik/laporan/pendapatan-pasien-bebas') }}" class="form-horizontal" role="form" method="POST">
        {{ csrf_field() }}
        <div class="row">
          <div class="col-sm-12">
                    ...
                    <table class="table table-hover table-bordered table-condensed" id="data">
                        ...
                        <tbody>
                            @if (!empty($reg))
                                ...
                                @foreach ($reg as $d)
                                    @php
                                        $pasien = \App\Pasienlangsung::where('registrasi_id', $d->id)->first();
                                    @endphp
                                    <tr>
                                        <td>{{ $no++ }}</td>
                                        <td>{{ !empty($pasien) ? $pasien->nama : '' }}</td>
                                        <td>{{ number_format(\Modules\Registrasi\Entities\Folio::where('registrasi_id', $d->id)->where('namatarif', 'LIKE', 'FPB%')->sum('total')) }}</td>
                                    </tr>
                                @endforeach
                            @endif
                        </tbody>
