...
@section('header')
  <h1>
    <small>Kategori Obat</small> Narkotik
...
<script type="text/javascript">
  ...
  var masterobat;
    masterobat = $('#dataMasterObat').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax: '/data-master-obat/narkotik',
        columns: [
            {data: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'nama', orderable: false},
            {data: 'kode', orderable: false},
  ...
  function masterObat() {
    $('#dataObat').modal('show')
    $('.modal-title').text('Tambahkan Obat Narkotik')
    $('input[name="kategoriobat"]').val('narkotik');
  }
