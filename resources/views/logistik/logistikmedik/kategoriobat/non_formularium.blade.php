...
@section('header')
  <h1>
    <small>Kategori Obat</small> Non Formularium
...
<script type="text/javascript">
  ...
  table = $('.obatNarkotik').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax:'/data-obat-non_formularium',
        columns: [
  ...
  var masterobat;
    masterobat = $('#dataMasterObat').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax: '/data-master-obat/non_formularium',
        columns: [
            {data: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'nama', orderable: false},
            {data: 'kode', orderable: false},
  ...
  function masterObat() {
    $('#dataObat').modal('show')
    $('.modal-title').text('Tambahkan Obat Non Formularium')
    $('input[name="kategoriobat"]').val('non_formularium');
  }
