            ...
            <table style="width: 100%" class="table table-striped table-bordered table-hover table-condensed form-box"
              ...
              <tr>
                ...
                <td style="padding: 5px;">
                  <input type="checkbox" value="Primary" name="kategori" checked>&nbsp;Primary<br/>
                  <small style="color:blue"><i>Uncheck jika kategori secondary</i></small>
              ...
              <tr>
                <td style="width:20%;">Kasus</td>
                <td style="padding: 5px;">
                  <input type="checkbox" value="Baru" name="kasus" >&nbsp;Baru<br/>
                  <input type="checkbox" value="Lama" name="kasus" >&nbsp;Lama<br/>
              ...
              <tr>
                  <td style="padding: 5px;" colspan="2">
                    <textarea rows="15" name="diagnosis" style="display:inline-block" placeholder="[Masukkan Catatan Dokter]" class="form-control"></textarea></td>
                  </td>
              </tr>
         ...
         <tbody>
          @foreach ($riwayat as $key=>$item)
              <tr>
                ...
                <td>{{$item->kategori ?  $item->kategori : 'Secondary'}}</td>
                <td>{{$item->kasus}}</td>
                <td>{{$item->id_condition_ss}}</td>
                <td>{{date('d-m-Y, H:i:s',strtotime($item->created_at))}}</td>
                
                <td>
                  <form method="POST" action="{{ url('emr-soap-icd/icd10/'.$item->id) }}" class="form-horizontal">
                    {{ csrf_field() }}
