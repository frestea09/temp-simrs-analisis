@extends('master')
@section('header')
  <h1>Input Hasil Lab Rawat Jalan</h1>
@endsection
      ...
      <div class="widget-user-header bg-aqua-active">
        <div class="row">
          <div class="col-md-2">
            ...
            <h5 class="widget-user-desc">Tanggal Lahir</h5>
          ...
          <div class="col-md-7">
            ...
            <h5 class="widget-user-desc">: {{ $pasien->kelamin}}</h5>
            <h5 class="widget-user-desc">: {{ date('d-M-Y', strtotime($pasien->tgllahir))  }}</h5>
...
<div class="box box-primary">
  ...
  <div class="box-body">
      {{-- @if (! session('pj')) --}}
    {!! Form::open(['method' => 'POST', 'url' => 'pemeriksaanlab/store-pasien-langsung', 'class' => 'form-horizontal']) !!}
      <div class="col-md-6">
        {!! Form::hidden('pasien_id', @$pasien->id) !!}
        {!! Form::hidden('dokter_id', @$dokterlab->id) !!}
        ...
        <div class="form-group{{ $errors->has('penanggungjawab') ? ' has-error' : '' }}">
            ...
            {{-- session('pj') --}}
            <div class="col-sm-8">
                {!! Form::select('penanggungjawab', $dokter, null, ['class' => 'chosen-select'], ['required']) !!}
        ...
        <div class="form-group{{ $errors->has('jam') ? ' has-error' : '' }}">
          ...
          <div class="col-sm-8">
              {!! Form::text('jam', null, ['class' => 'form-control timepicker'], ['required']) !!}
      ...
      <div class="col-md-6">
        <div class="form-group{{ $errors->has('tgl_pemeriksaan') ? ' has-error' : '' }}">
          ...
          <div class="col-sm-8">
              {!! Form::text('tgl_pemeriksaan', date('d-m-Y'), ['class' => 'form-control datepicker'], ['required']) !!}
                    ...
                    <div class="box-body table-responsive no-padding">
                      <table class="table table-striped table-bordered table-hover table-condensed dataTable no-footer">
                          ...
                          <tr>
                            ...
                            <td>{{ $lab->nama }}</td>
                            <td width="10%">
                              <input name="hasil[{{ $lab->id }}_{{ $lab_kat->id }}_{{ $sec->id }}]" class="form-control" required />
                            </td>
                            <td>
                              <input name="hasiltext[{{ $lab->id }}_{{ $lab_kat->id }}_{{ $sec->id }}]" class="form-control" required />
                            </td>
      ...
      @if(count($hasillabs) > 0)
        ...
        @foreach ($rincian as $item)
          <tr>
            ...
            <td>
            <a href="{{ url('pemeriksaanlab/hapus/'.$item->id) }}" class="btn btn-danger btn-sm btn-flat pull-right">Hapus</a>
            </td>
          </tr>
        @endforeach
        @endforeach
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $('select[name="labsection_id"]').on('change', function() {
          ...
          if(sec_id) {
              $.ajax({
                  url: '/pemeriksaanlab/getkategori/'+sec_id,
                  type: "GET",
                  dataType: "json",
      ...
      $('select[name="labkategori_id"]').on('change', function() {
          ...
          if(cat_id) {
              $.ajax({
                  url: '/pemeriksaanlab/getlab/'+cat_id,
                  type: "GET",
                  dataType: "json",
                  success:function(data) {
