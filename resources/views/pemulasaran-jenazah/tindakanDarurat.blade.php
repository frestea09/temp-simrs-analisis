...
@section('header')
  <h1>Pemulasaran Jenazah & Ambulans - Rawat Darurat </h1>
@endsection
...
@section('content')
    <div class="box box-primary">
      ...
      <div class="box-body">
        {!! Form::open(['method' => 'POST', 'url' => 'pemulasaran-jenazah/tindakan-darurat', 'class'=>'form-hosizontal']) !!}
        <div class="row">
          <div class="col-md-6">
            ...
            <tbody>
              @foreach ($reg as $key => $d)
                  ...
                  {{-- @role(['jenazah', 'administrator']) --}}
                    <a href="{{ url('pemulasaran-jenazah/insert-tindakan/jenazah/darurat/'. $d->id.'/'.$d->pasien_id) }}" onclick="return confirm('APAKAH YAKIN AKAN DILAKUKAN TINDAKAN INI ?')" class="btn btn-sm btn-info btn-flat">
                      <i class="fa fa-bed"></i>
                    </a>
                  {{-- @endrole --}}
                  @role(['ambulans', 'administrator'])
                    <a href="{{ url('pemulasaran-jenazah/insert-tindakan/ambulans/darurat/'. $d->id.'/'.$d->pasien_id) }}" onclick="return confirm('APAKAH YAKIN AKAN DILAKUKAN TINDAKAN INI ?')" class="btn btn-sm btn-danger btn-flat">
                      <i class="fa fa-ambulance"></i>
                    </a>
                  @endrole
                  </td>
                  <td class="text-center">
                    @if (Modules\Registrasi\Entities\Folio::where('registrasi_id', $d->id)->where('jenis','TG')->whereIn('poli_tipe', ['Z', 'B'])->count() > 0)
                      <a href="{{ url('pemulasaran-jenazah/cetak-tindakan/TG/'.$d->id) }}" target="_blank" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print"></i></a>
                    @endif
                  </td>
                </tr>
              @endforeach
