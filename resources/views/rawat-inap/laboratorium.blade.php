@extends('master')
@section('header')
  <h1>Rawat Inap - Laboratorium <small></small></h1>
@endsection
...
@section('content')
  ...
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">Data Tindakan Laboratorium Rawat Inap</h3>
      <div class="box-tools pull-right">
        <button type="button" class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
      </div>
    </div>
      ...
      <div class="row">
        ...
        <div class="col-md-5">
          ...
          <div class="form-group">
              ...
              <div class="col-sm-9">
                  <a href="{{ url('rawat-inap/billing') }}" class="btn btn-warning btn-flat">Kembali</a>
        ...
        <table class='table table-striped table-bordered table-hover table-condensed'>
          ...
          <tbody>
            @foreach ($folio as $key => $d)
              <tr>
                <td>{{ $no++ }}</td>
                <td>{{ ($d->tarif_id <> 10000 ) ? !empty($d->tarif->nama) ? $d->tarif->nama : '' : 'Penjualan Obat' }}</td>
                <td>{{ baca_poli($d->poli_id) }}</td>
                @if (@$d->verif_kasa_user = 'tarif_new')
                <td>{{ ($d->tarif_id <> 10000 ) ? number_format($d->tarif_baru->total,0,',','.') : '' }}</td>
                <td>{{ ($d->tarif_id <> 10000 ) ? ($d->total / $d->tarif_baru->total) : '' }}</td>
                @else
                <td>{{ ($d->tarif_id <> 10000 ) ? number_format($d->tarif->total,0,',','.') : '' }}</td>
                <td>{{ ($d->tarif_id <> 10000 ) ? ($d->total / $d->tarif->total) : '' }}</td>
                @endif
                {{-- <td class="text-right">{{ ($d->tarif_id <> 10000 ) ? !empty($d->tarif->total) ? number_format($d->tarif->total,0,',','.') : '' : '' }}</td>
                <td class="text-center">{{ ($d->tarif_id <> 10000 ) ? !empty($d->tarif->total) ? ($d->total / $d->tarif->total) : '' : '' }}</td> --}}
  ...
  <div class="box box-primary">
    ...
    <div class="box-body">
      @include('tinymce')
      {!! Form::open(['method' => 'POST', 'url' => 'rawat-inap/simpan-laboratorium', 'class' => 'form-horizontal']) !!}
        {!! Form::hidden('registrasi_id', $reg->id) !!}
