...
<html lang="en">
    ...
    <body onload="print()">
        <h3 class="text-center">Laporan Pengunjung Rawat Inap Periode {{ date('d M Y', strtotime($tga)) }} Sampai {{ date('d M Y', strtotime($tgb)) }}</h3>
	@if($crb == 5)
        ...
        <table class='table table-bordered table-hover'>
            <thead>
                <tr>
                    ...
                    <th class="v-middle text-center" rowspan="2" style="min-width:60px">Masuk</th>
                    <th class="v-middle text-center" rowspan="2" style="min-width:60px">Keluar</th>
                    ...
                    <th class="v-middle text-center" rowspan="2" style="min-width:70px">Kamar</th>
                ...
                <tr>
                    ...
                    <th class="v-middle text-center">T. Inap</th>
            ...
            <tbody>
                ...
                @foreach ($rawatinap as $rn)
                    <tr>
                        ...
                        <td class="text-center">{{ date('d-m-Y', strtotime($rn->tgl_masuk)) }}</td>
                        <td class="text-center">{{ date('d-m-Y', strtotime($rn->tgl_keluar)) }}</td>
                        <td class="text-center">{{ $rn->pasien->no_rm }}</td>
                        <td>{{ $rn->pasien->nama }}</td>
                        <td class="text-center">{{ baca_carabayar($rn->carabayar_id) }}</td>
                        <td>
                            @foreach($rn->kamar as $k)
                                <p class="no-wrap">{{ $k->nama }}</p>
                            @endforeach
                        </td>
						@for($i = 1; $i <= 16; $i++)
                            @if($i != 2 && $i != 1)
