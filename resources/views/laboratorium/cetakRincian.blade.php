    ...
    <td>
      ...
      <td style="width: 50%">
        ...
        <table style="width: 100%">
          ...
          <tr> <td>Kamar</td> <td>:</td><td>{{ baca_kamar(@$reg->rawat_inap->kamar_id) }}</td></tr>
  ...
  <table class="table table-condensed" style="width: 100%">
    ...
    @foreach ($tindakan as $d)
      @php
          ...
          $order_lab = \App\Orderlab::where('id', $d->order_lab_id)->first();
      @endphp
        <tr>
          <td style="text-align: center">{{ $no++ }}</td>
          <td>{{ @$d->created_at ? @date('d-m-Y H:i',strtotime(@$d->created_at)) : @date('d-m-Y H:i', strtotime(@$order_lab->created_at)) }}</td>
          <td>{{ $d->namatarif }}</td>
          <td style="text-align: center">{{ baca_carabayar($d->cara_bayar_id) }}</td>
          {{-- @php
            $qty = ($d->total + $d->diskon) / $d->tarif->total;
            $total = $d->tarif->total;
          @endphp
          @if (is_decimal($qty))
            @php
          ...
          @endif
          {{-- <td style="text-align: center">{{ ($d->tarif->total <> 0) ? @$qty : NULL }}</td>
          <td style="text-align: right">{{ ($d->tarif->total <> 0) ? number_format(@$total) : number_format($det) }}</td> --}}
