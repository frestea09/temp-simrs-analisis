            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                  ...
                  <td> --}}
                    {{-- <div class="btn-group"> --}}
                      ...
                      {{-- <ul class="dropdown-menu dropdown-menu-right" role="menu" style=""> --}}
                        @foreach (cek_hasil_lis($reg->id) as $item)
                        <tr>
                          <td>
                            <a href="{{url('cetak-lis-pdf/'.$item->no_lab.'/'.$item->registrasi_id)}}" target="_blank" class="btn btn-primary btn-flat btn-xs"><i class="fa fa-print"></i> {{$item->no_lab}}</a>
                            @if (cek_jenis_reg($reg->status_reg) == 'Rawat Jalan')
                              <a href="{{ url('pemeriksaanlab/cetakAll/'.$reg->id) }}" target="_blank" class="btn btn-sm btn-danger btn-flat"><i class="fa fa fa-print"></i></a>
                            @endif
                          </td>
                        </tr>
                        @endforeach 
                ...
                {{-- @if (count($penjualan) > 0)
                  ...
                  <tr>
                    <td>{{$key+1}}</td>
                    <td><a href="{{ url('farmasi/laporan/etiket/'.$p->id) }}" class="btn btn-flat btn-sm" target="_blank">{{ $p->no_resep }}</a> - <small>{{date('d-m-Y',strtotime($p->created_at))}}</small></td>
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Etiket</i></td>
                </tr>
                @endif --}}
