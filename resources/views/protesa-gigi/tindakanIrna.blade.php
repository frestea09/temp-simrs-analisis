...
@section('content')
    <div class="box box-primary">
      ...
      <div class="box-body">
        ...
        {!! Form::open(['method' => 'POST', 'url' => 'protesa-gigi/tindakan-ranap', 'class'=>'form-hosizontal']) !!}
        <div class="row">
          <div class="col-md-6">
            ...
            <tbody>
              @foreach ($reg as $key => $d)
                  ...
                  {{-- @role(['protesagigi', 'administrator']) --}}
                    <a href="{{ url('protesa-gigi/insert-tindakan/ranap/'. $d->id.'/'.$d->pasien_id) }}" onclick="return confirm('APAKAH YAKIN AKAN DILAKUKAN TINDAKAN INI ?')" class="btn btn-sm btn-info btn-flat">
                      <i class="fa fa-refresh"></i>
                    </a>
                  {{-- @endrole --}}
                  </td>
                  <td class="text-center">
                    @if (Modules\Registrasi\Entities\Folio::where('registrasi_id', $d->id)->where('jenis','TI')->where('poli_tipe', 'T')->count() > 0)
                      <a href="{{ url('protesa-gigi/cetak-tindakan/TI/'.$d->id) }}" target="_blank" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print"></i></a>
                    @endif
                  </td>
                </tr>
              @endforeach
