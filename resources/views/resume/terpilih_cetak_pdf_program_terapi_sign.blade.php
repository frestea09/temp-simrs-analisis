...
<html lang="">
  <head>
    ...
    <title>{{ @$reg->pasien->no_rm }}_{{ @$reg->pasien->nama }}</title>
  ...
  <body onload="print()">
    ...
    <table style="width: 100%; border: 1px solid;">
      ...
      <tr>
        ...
        <td colspan="2">: {{ @$soap->assesment ?? '' }}</td>
      </tr>
    </table>
    <br>
    ...
    <table style="width: 100%; border: 1px solid;">
      @php
          $riwayats = \App\EmrInapPemeriksaan::where('registrasi_id', $reg->id)->where('type', 'program_terapi_rehab')->first();
      @endphp
      <tr>
        <td style="width: 25%">PERMINTAAN TERAPI</td>
        <td colspan="2">: {{ @json_decode(@$riwayats->fisik, true)['permintaanTerapi'] }}</td>
      </tr>
    </table>
    <br>
    ...
    <tr>
        ...
        <td class="border" style="padding-left: 5px; width: 20%; height: 50px; text-align: center;">
            {{ isset($data['tgl']) ? \Carbon\Carbon::parse($data['tgl'])->format('d-m-Y') : \Carbon\Carbon::parse($data['created_at'])->format('d-m-Y') }}
        </td>
        <td class="border" style="padding-left: 5px; width: 20%; height: 50px;" align="center">
            @if ($ttd_pasien->tanda_tangan)
              <img src="{{asset('images/upload/ttd/' . @$ttd_pasien->tanda_tangan)}}" alt="" width="150" height="70">
            @endif
        </td>
        <td class="border" style="padding-left: 5px; width: 20%; height: 50px;" align="center">
            @php
