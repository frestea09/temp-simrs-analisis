...
<html lang="">
  <head>
    ...
    <title>Suara Antrian Rawat Inap</title>
    <style type="text/css">
      .bg-image{
        /* background: url("/images/bg-rsu.png") no-repeat center center; */
        background-image: linear-gradient(-60deg, #39A7FF 0%, #E0F4FF 100%);
      }
      .my-container {
        position: fixed;
        background: #5C97FF;
  ...
  <body class="hold-transition login-page bg-image">
    <h3 class="besar">Suara Antrian Rawat Inap</h3>
    ...
    <h3 class="layarantrian">
      <a href="{{ url('/antrian-rawatinap/layarlcd') }}" target="_blank">Klik disini untuk membuka Layar urutan Antrian!</a>
...
@foreach ($antrian as $key => $d)
  ...
  @php
    DB::table('antrian_rawatinaps')->where('id', $d->id)->update(['panggil' => 1]);
  @endphp
@endforeach
  @php
  if (isset($antrian)) {
  ...
  <META HTTP-EQUIV="REFRESH" CONTENT="{{ $delay }}; URL={{ url('antrian-rawatinap/suara') }}">
