...
@section('header')
  <h1>Rawat Inap - Daftar Antrian Loket B </h1>
@endsection
                      ...
                      <tbody>
                        @foreach ($terpanggil as $key => $d)
                          <tr>
                            ...
                            <td>
                                @if ($d->status <= 2)
                                  <a href="{{ route('antrianrawatinap.panggilkembali2',$d->id) }}" class="btn btn-info btn-sm btn-flat"><i class="fa fa-microphone"></i></a>
                                @endif
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    setInterval(function () {
      $('#daftarantrian').load("{{ route('antrianrawatinap.daftarpanggil2') }}");
    },2000);
  });
