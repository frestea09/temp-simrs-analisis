        ...
        <div class="col-sm-12">
          <div class="form-group">
            ...
            <div class="col-sm-10 btn-group">
              <button type="button" onclick="nomorKartu()" class="btn btn-primary btn-flat">NOKA</button>
              <button type="button" onclick="nomorNik()" class="btn btn-danger btn-flat">NIK</button>
              ...
              {{-- <a href="{{ url('/sep/cetak-label/'.$reg->id) }}" class="btn btn-warning btn-flat" target="_blank"><i class="fa fa-print"></i> CETAK LABEL</a> --}}
...
@section('script')
  ...
  $(document).ready(function() {
      $('#diagnosa_awal').on('change', function() {
          var diagnosa = $(this).val();
          var pasien_id = "{{ @$data['reg']->pasien_id }}";

          if (diagnosa && pasien_id) {
              $.ajax({
                  url: '{{ route("cek.diagnosa.terakhir") }}', // route untuk cek diagnosa terakhir
                  type: 'GET',
                  data: {
                      pasien_id: pasien_id,
                      diagnosa: diagnosa
                  ...
                  success: function(response) {
                      if (response.sama === true) {
                          alert('⚠️ Pasien memiliki diagnosa yang sama seperti registrasi sebelumnya!');
                      }
                  },
                  error: function() {
                      console.error('Terjadi kesalahan saat memeriksa diagnosa.');
                  }
              });
          }
      });
  ...
  <script type="text/javascript">
    ...
    function nomorKartu() {
      ...
      $('.formInputTgl').empty()
      $('.formInputNoSep').addClass('hidden')
    ...
    function cariNIK() {
      ...
      .done(function(res) {
        ...
        if (data[0].metaData.code == 200) {
          ...
          $('input[name="niks"]').val(data[1].peserta.nik)
...
// ======================================================================================================================
    ...
    $(document).ready(function() {
      ...
      $(document).on('click', '.addICD', function (e) {
        ...
        // ambil kode ICD dari tombol yang diklik
        var kodeICD = $(this).attr('data-nomor');
        
        // isi ke input diagnosa_awal
        $('#diagnosa_awal').val(kodeICD);

        // tutup modal ICD10
        $('#ICD10').modal('hide');

        // ambil pasien_id dari backend
        var pasien_id = "{{ @$reg->pasien_id ?? '' }}";
        
        // jalankan AJAX untuk cek diagnosa terakhir
        if (kodeICD && pasien_id) {
            
            $.ajax({
                url: '{{ route("cek.diagnosa.terakhir") }}',
                type: 'GET',
                data: {
                    pasien_id: pasien_id,
                    diagnosa_awal: kodeICD
                },
                success: function(response) {
                    if (response.sama === true) {
                        swal("Perhatian", "⚠️ Pasien memiliki diagnosa yang sama seperti registrasi terakhir!", "warning");
                    }
                },
                error: function() {
                    console.error('Terjadi kesalahan saat memeriksa diagnosa.');
