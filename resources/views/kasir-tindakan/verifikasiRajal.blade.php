...
@section('header')
  <h1>
    Verifikasi Rawat Jalan | Total Tagihan: Rp. {{ number_format($tagihan) }}
    </h1>
@endsection
      ...
      <div class="box-body">
          ...
          <div class='table-responsive'>
            <table class='table table-striped table-bordered table-hover table-condensed'>
              <tbody>
                ...
                @endif
                <tr>
                  <th>Tanggal Masuk</th><td>{{ tanggal($reg->created_at) }}</td>
                  <th>Tanggal Keluar</th><td>{{ tanggal($reg->tgl_pulang) }}</td> 
                </tr>
        ...
        <div class='table-responsive'>
          <table class='table table-striped table-bordered table-hover table-condensed' >
            ...
            <tbody>
              {!! Form::open(['method' => 'POST', 'url' => '/kasir/save-verifikasi-kasa/', 'class' => 'form-horizontal']) !!}
              {!! Form::hidden('registrasi_id', $reg->id) !!}
              @foreach ($folio as $key => $d)
                <tr>
                  <td class="text-center">{{ $no++ }}</td>
                  <td onclick="detailVerif({{ $reg->id }}, '{{ $d->poli_tipe }}')" colspan="2">{{ baca_politipe($d->poli_tipe) }}</td>
                  <td onclick="detailVerif({{ $reg->id }}, '{{ $d->poli_tipe }}')" class="text-center">{{ $d->qty }}</td>
                  <td onclick="detailVerif({{ $reg->id }}, '{{ $d->poli_tipe }}')" class="text-right">{{ number_format($d->total) }}</td>
                  <td class="text-center">
                    {{-- <input type="checkbox" name="verif_kasa{{ $i_verif++ }}" value="{{ $d->tarif_id }}"> --}}
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content ">
          ...
          <div class="modal-body">
            ...
            {!! Form::hidden('pasien_id', $reg->pasien_id) !!}
            {!! Form::hidden('verifikasi_rajal_kasa', '1') !!}
            <div class="row">
              <div class="col-md-7">
...
{{-- END MODAL DETAIL TARIF VERIFIKASI --}}
  ...
  <script type="text/javascript">
    ...
    function detailVerif(registrasi_id, poli_tipe){
      $('#detailVerifikasiModal').modal('show');
      $('.modal-title').text('Detail Tindakan');
      $('#dataTindakanDetail').load('/kasir/detail-verifikasi-rajal/'+registrasi_id+'/'+poli_tipe);
    }
