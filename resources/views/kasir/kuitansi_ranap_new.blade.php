      ...
      <td class="text-right">
        <table style="width: 100%;">
          ...
          <tr style="font-size: 18px;" >
             ...
             @php
                $kwitansi = @str_replace("IRNA-", "", @$kuitansi->no_kwitansi);
             @endphp
             <td>: {{ @$kwitansi }}</td> 
          </tr>  
          <tr style="font-size: 18px;"> 
      ...
      <td class="text-right">
        <table style="width: 100%;">
          ...
          <tr style="font-size: 20px;"> 
             ...
             {{-- <td>: <b>{{number_format($tindakan_igd+$tindakan_irj_ina+$rad+$lab+$obat)}} Rupiah</b></td>  --}}
             <td>: <b>{{number_format($totals)}} Rupiah</b></td> 
          </tr>
          <tr style="font-size: 20px;"> 
            ...
            <td>: 
              @foreach ($folio as $item => $d)
                 {{ $d->namatarif }},
              @endforeach
              @foreach ($folio_null_tipe as $item => $tarif)
             {{ baca_tarif($tarif->tarif_id) }},
              @endforeach
              @if ($lab != null)
                   Laboratorium
                @endif
         ...
         <tr style="font-size: 20px;"> 
          ...
          {{-- <td><b>Rp.{{number_format($tindakan_igd+$tindakan_irj_ina+$rad+$lab+$obat)}}</b></td>  --}}
          <td>: <b>{{number_format($totals)}} Rupiah</b></td> 
       </tr>
        </table>
      </td>
