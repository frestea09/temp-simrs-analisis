...
<div class="row">
    <div class="col-md-12">
        {{-- <div class="collapse" id="collapseExample"> --}}
            {{-- form cari --}}
            <div class="card card-body mt-3 border-info" style="border: 1px solid;">
                <div class="form-group row">
                    <div class="col-sm-10">
                        <form id="formCariNik">
                            {{ csrf_field() }}
                            {!! Form::text('keyword', null, ['class' => 'form-control', 'onkeyup'=>'this.value =
                            this.value.toUpperCase()','placeholder'=>'Masukkan NIK Atau No .RM']) !!}
                            <small class="text-danger">{{ $errors->first('nama') }}</small>
                    </div>
                    <div class="col-sm-2">
                        <button type="" href="#" class="btn btn-primary btn-sm btn-cari-nik col-sm-12"> <i
                                class="fa fa-search"></i> CEK DATA</button>
                    </div>
                    </form>
                </div>
                ...
                <div class="form-group row">
                    <div class="col-sm-10">
                        <form id="formCariRm">
                            {{ csrf_field() }}
                            {!! Form::text('keyword', null, ['class' => 'form-control', 'onkeyup'=>'this.value =
                            this.value.toUpperCase()','placeholder'=>'No. Rekam Medik (RM)']) !!}
                            <small class="text-danger">{{ $errors->first('nama') }}</small>
                    </div>
                    <div class="col-sm-2">
                        <button type="submit" href="#" class="btn btn-primary btn-sm btn-cari-rm col-sm-12"> <i
                                class="fa fa-search"></i> CEK DATA</button>
                    </div>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    $('#formCariRm').submit(function(event) {
        event.preventDefault();
    });
    $('#kelurahan').select2({
    ...
    $('.btn-cari-nik').on('click', function(){
        ...
        if(cari !== ''){
            $.ajax({
            url: '/reservasi/cari-pasien',
            type: 'POST',
            dataType: 'json',
            data: $('#formCariNik').serialize(), 
            beforeSend: function () {
            //   $('.overlay').removeClass('hidden')
            },
            complete: function () {
