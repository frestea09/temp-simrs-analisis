...
use App\Antrian4 As Antrian;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;

class Antrian4Controller extends Controller {
	...
	public function savetouch(Request $request) {
		...
		Antrian::create($data);
		return view('antrian::cetak_antrian', $data)->with('loket', 4);
	}
	...
	public function datalayarlcd() {
		...
		$terpanggil = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			->orderBy('id', 'desc')
			->first();
		return view('modules.antrian4.datalayarlcd', compact('antrian', 'terpanggil'));
	}
	...
	public function daftarpanggil() {
		$antrian = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			->take(5)
			->get();
		return view('modules.antrian4.daftarpanggil', compact('antrian'));
	}
	...
	public function daftarantrian() {
		...
		$terpanggil = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			// ->take(5)
			->get();
		return view('modules.antrian4.daftarantrian', compact('terpanggil'));
	}

	public function panggil($id = '') {
		// DB::table('antrian4')->where('id', $id)->update(['status' => 1]);
		$atr = Antrian::find($id);
		$atr->status = 1;
		$atr->loket = 4;
		$atr->update();
		return redirect()->route('antrian4.daftarantrian');
	}
	...
	public function panggilkembali($id = '') {
		...
		$d->panggil = 0;
		$d->update();
		// DB::table('antrian4')->where('id', $id)->update(['status' => $d->status + 1, 'panggil'=>0]);
		return redirect()->route('antrian4.daftarantrian');
	}
	...
	public function registrasi($id, $jenis) {
		...
		$d->status = 3;
		$d->update();
		// DB::table('antrian4')->where('id', $id)->update(['status' => 3]);
		session(['no_loket' => 4]);
		if ($jenis == 'jkn') {
