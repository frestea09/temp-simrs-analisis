...
class KasirController extends Controller
{
	public function rawat_jalan()
	{
		...
		$polis = Poli::where('politype', 'J')->pluck('nama', 'id');
		$bayar = ModulesCarabayar::all();

		return view('kasir.index', compact('polis','bayar'))->with('no', 1);
	}
	...
	public function rawatjalanByTanggal(Request $request)
	{
		...
		$query = Registrasi::where('status_reg', 'like', 'J%')
			->with(['pasien', 'dokter_umum', 'poli', 'bayars']);
		...
		if (!empty($request['ket_lunas'])) {
			$query->where('lunas', $request['ket_lunas']);
		}

		// if (!empty($no_rm)) {
		// 	$no_rm_req = Pasien::where('no_rm', $request->no_rm)->first();
		// 	$query->where('pasien_id', $no_rm_req->id );
		// }

		// if (!empty($nama)) {
		// 	$nama_req = Pasien::where('nama', 'like', '%'.$request->nama.'%')->first();
		// 	$query->where('pasien_id', $nama_req->id );
		// }

		// if (!empty($alamat)) {
		// 	$alamat_req = Pasien::where('alamat', 'like', $request->alamat.'%')->first();
		// 	$query->where('pasien_id', $alamat_req->id );
		// }
		...
		if (!empty($request['tipe_jkn'])) {
			$query->where('tipe_jkn', $request['tipe_jkn']);
		}
		...
		foreach($today as $d){
			$folioTindakan = Folio::where('jenis', '!=', 'ORJ')->where('registrasi_id', '=', $d->id)->where('lunas', 'N')->sum('total');
			$d->tagihan = $folioTindakan;
		}

		$bayar = ModulesCarabayar::all();
		$poli = Poli::where('politype', 'J')->pluck('nama', 'id');
		$jenisJkn = Jenisjkn::pluck('nama', 'id');

		$poliSelect = @$request['poli'];
		$tipeJkn = @$request['tipe_jkn'];
		$caraBayar = @$request['carabayar'];
		return view('kasir.index', compact('today', 'bayar', 'caraBayar', 'poli', 'poliSelect', 'jenisJkn','tipeJkn'))->with('no', 1);
	}

	public function data_rawat_jalan(Request $req){
		...
		$d = Registrasi::leftJoin('pasiens', 'pasiens.id', '=', 'registrasis.pasien_id')
				...
				->whereHas('poli', function ($query){
					$query->where('kelompok', '!=', 'ESO');
				})
				->orderBy('registrasis.id', 'DESC');

		if($req->filled('keyword')){
			$d = $d->where(function ($query) use($req){
					$query->where('pasiens.no_rm', $req->keyword)
						->orWhere('pasiens.nama', 'LIKE', "%$req->keyword%");
				});
		} 
		
		if($req->filled('tglReg')){
    		$d = $d->whereDate('registrasis.created_at', date('Y-m-d',strtotime($req->tglReg)));
		}else{
			// $keyCache = 'kasir_tindakan';
			// $d = Cache::get($keyCache);
			// if(!$d){
				$d = $d->where('registrasis.created_at', 'LIKE', date('Y-m-d') . '%');
		...
		if($req->poli_id != null){
			$d = $d->where('registrasis.poli_id', $req->poli_id);
		}

		if($req->bayar != null){
			$d = $d->where('registrasis.bayar', $req->bayar);
		}
		...
		$d = $d->select([
			...
			'registrasis.lunas',
			'registrasis.keterangan',
			'registrasis.created_at'
		])->get();

		if ($req->filled('status_bayar')) {
			$d = $d->filter(function ($item) use ($req) {
				$tagihan = $item->folioBelumLunas->sum('total');
		
				if ($req->status_bayar == 'lunas') {
					return $tagihan == 0;
				} elseif ($req->status_bayar == 'belum') {
					return $tagihan > 0;
				}
		
				return true;
			})->values(); // penting untuk reset index collection!
		}
		...
		return DataTables::of($d)
			...
			->addColumn('nama', function ($d) {
				if(@$d->lunas == 'Y'){
					return '<span style="color: green;font-weight: bold;">' . @$d->pasien->nama . '</span>';
				} else {
			...
			->addColumn('rincian', function ($d) {
				return '<a href="'.url('ranap-informasi-unit-item-rincian-biaya/'. @$d->id).'" class="btn btn-sm btn-danger btn-flat"><i class="fa fa-print"></i></a>';
			})
			->addColumn('bayar', function ($d) {
				if(@$d->lunas == 'Y'){
					return '<a href="'.url('kasir-tindakan/rawatjalan/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-success btn-flat"><i class="fa fa-credit-card"></i></a>';
				}else{
					return '<a href="'.url('kasir-tindakan/rawatjalan/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-info btn-flat"><i class="fa fa-credit-card"></i></a>';
				}
			})
			->addColumn('billing', function ($d) {
					return '<a href="'.url('tindakan/entry/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-warning btn-flat"><i class="fa fa-edit"></i></a>';
			})
			->addColumn('tagihan', function ($d) {
				return number_format(@$d->folioBelumLunas->sum('total'));
			})
			->addColumn('pembayaran', function ($d) {
				return number_format(@$d->folioBelumLunas->sum('dibayar'));
			})
			->addColumn('no_kuitansi', function ($d) {
				return $d->folioBelumLunas->pluck('no_kuitansi')->filter()->unique()->implode('<br>');
			})
			->addColumn('piutang', function ($d) {
				@$confirm = "return confirm('Yakin akan di masukkan piutang?')";
				if (@$d->bayar <> 1){
                  if(@$d->lunas == 'P'){
                    return 'Piutang';
				  }else{
                    return '<a href="'.url('kasir/piutang/'.@$d->id).'" onclick="'.@$confirm.'" class="btn btn-sm btn-warning btn-flat"><i class="fa fa-dollar"></i></a>';
                  }
                }
			})
			->addColumn('kwitansi_penunjang', function ($d) {
				return '
					<div class="btn-group">
						...
						<ul class="dropdown-menu dropdown-menu-right" role="menu" style="color:white !important">
								<li>
									...
									<a href="'.url("kasir/cetakkuitansi-penunjang-resume-medis/".$d->id).'" class="btn btn-info btn-sm btn-flat"> <i class="fa fa-print"></i>Resume Medis</a>
								</li> 
						</ul>
					</div>
				';
			})
			->rawColumns(['rincian', 'bayar', 'billing', 'piutang', 'kwitansi_penunjang', 'nama'])
			->addIndexColumn()
			->make(true);
	}
	...
	//Kasir Eksekutif
	public function kasirEksekutif()
	{
		ini_set('max_execution_time', 0);
		ini_set('memory_limit', '10000M');

		$polis = Poli::whereIn('kelompok', ['ESO','VAK','MCUO'])->pluck('nama', 'id');
		// dd($polis);
		return view('kasir.eksekutif.index', compact('polis'))->with('no', 1);
	}

	public function data_eksekutif(Request $req){
		ini_set('max_execution_time', 0); //0=NOLIMIT
		ini_set('memory_limit', '-1');
		$d = Registrasi::leftJoin('pasiens', 'pasiens.id', '=', 'registrasis.pasien_id')
				->with(['pasien', 'dokter_umum', 'poli', 'bayars'])
				->where('registrasis.status_reg', 'like', 'J%')
				->whereHas('poli', function ($query) use ($req){
					$query->where('id', $req->poli_id);
				})
				->orderBy('registrasis.id', 'DESC');
		...
		if($req->filled('tglReg2')){
			// $d = $d->whereDate('registrasis.created_at', date('Y-m-d',strtotime($req->tglReg)));
			$d = $d->whereBetween('registrasis.created_at', [valid_date($req->tglReg) . ' 00:00:00', valid_date($req->tglReg2) . ' 23:59:59']);
		}else{
				$d = $d->where('registrasis.created_at', 'LIKE', date('Y-m-d' . '%'));
		}
		...
		return DataTables::of($d)
			...
			->addColumn('bayar', function ($d) {
				if(@$d->lunas == 'Y'){
					return '<a href="'.url('kasir/eksekutif/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-success btn-flat"><i class="fa fa-credit-card"></i></a>';
				}else{
					return '<a href="'.url('kasir/eksekutif/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-info btn-flat"><i class="fa fa-credit-card"></i></a>';
				}
			})
			->addColumn('tagihan', function ($d) {
				return number_format(@$d->folioBelumLunas->sum('total'));
			...
			->addColumn('cetak_rajal', function ($d) {
					return '<a href="'.url('frontoffice/cetak-irj').'" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-print"></i></a>';
			})
			->addColumn('kwitansi', function ($d) {
					return'<a href="'.url('kasir/kwitansi-eksekutif/'.@$d->id).'" target="_blank" class="btn btn-warning btn-sm"><i class="fa fa-print"></i></a>';
			})
			->addColumn('kwitansiTindakan', function ($d) {
					return'<a href="'.url('kasir/rincian-biaya-tindakan-registrasi/'.@$d->id).'" target="_blank" class="btn btn-info btn-sm"><i class="fa fa-print"></i></a>';
			})
			->addColumn('kwitansi_penunjang', function ($d) {
				return '
					<div class="btn-group">
			...
			->rawColumns(['rincian', 'bayar', 'billing', 'piutang', 'kwitansiTindakan','kwitansi_penunjang', 'nama', 'kwitansi', 'cetak_rajal'])
			->addIndexColumn()
			->make(true);
	}

	public function bayarEksekutif($reg_id, $pasien_id)
	{
		$data['tag'] = Tagihan::where('registrasi_id', '=', $reg_id)->get();
		// $data['fol'] = Folio::where('jenis','!=','ORJ')->where('folios.registrasi_id', '=', $reg_id)->where('folios.lunas', 'N')->get();
		...
		// $data['tagihan'] = Folio::where('jenis','!=','ORJ')->where('registrasi_id', $reg_id)->where('lunas', 'N')->sum('total');
		...
		// return $data; die;
		return view('kasir.eksekutif.bayar', $data)->with('no', 1);
	}

	public function save_bayar_eksekutif(Request $request)
	{
		// dd($request->all());
		...
		$no_kuitansi = 'EKS-' . date('Ymd') . '-' . sprintf("%05s", $cek_kuitansi);
		...
		if ($request['total'] != 0) {
			if (rupiah($request['totalBayar']) < rupiah($request['total_tagihan'] && $request['diskon_persen'] == 0 && $request['diskon_rupiah'] == 0)) {
				...
				return redirect('kasir/eksekutif/bayar/' . $r->id . '/' . $r->pasien_id);
			} else {
				DB::transaction(function () use ($request, $no_kuitansi, $r) {
					...
					$journal = Journal::create([
						...
						'type'				=> 'rawat_jalan_eksekutif',
						'keterangan'		=> 'Jurnal Rawat Jalan Eksekutif a.n ' . $reg->pasien->nama,
						'verifikasi'		=> 1
					]);
					...
					//Update Folio
					// $fol = Folio::with('tarif')->where('jenis','!=','ORJ')->where('registrasi_id', $request['registrasi_id'])->where('lunas', 'N')->get();
				...
				// return redirect('/kasir/rincian-biaya-tindakan/' . session('idk'));
	...
	public function rawat_inap()
	{
		...
		$today = Registrasi::where('status_reg', 'like', 'I%')
			...
			->get();
		$poli = Poli::all();
		return view('kasir.irna', compact('today', 'bayar', 'poli'))->with('no', 1);
	}
	...
	public function rawat_inap_byTanggal(Request $request)
	{
		...
		// request()->validate(['tanggal' => 'required']);
		$query = Registrasi::where('status_reg', 'like', 'I%');
		...
		$poli = Poli::all();
		$poliSelect = @$request['poli'];
		return view('kasir.irna', compact('today', 'bayar', 'poli', 'poliSelect'))->with('no', 1);
	}

	public function ubahPenanggungJawab(Request $request)
	{
		$reg = Registrasi::find($request->regId);
		$pasien = $reg->pasien;
		$pasien->nama_keluarga = $request->nama_keluarga;
		$pasien->nik_penanggung_jawab = $request->nik_penanggung_jawab;
		$pasien->alamat_penanggung_jawab = $request->alamat_penanggung_jawab;
		$pasien->hub_keluarga = $request->hubungan_keluarga;

		if ($pasien->save()) {
			return response()->json([
				'sukses' => true,
				'text' => 'Sukses merubah penanggung jawab'
			]);
		} else {
			return response()->json([
				'sukses' => false,
				'text' => 'Gagal merubah penanggung jawab'
			]);
		}
	}
	...
	public function igd()
	{
		...
		// 	->get();
		// $poli = Poli::all();
		return view('kasir.index_igd')->with('no', 1);

	}

	public function data_igd(Request $req){
		ini_set('max_execution_time', 0); //0=NOLIMIT
		ini_set('memory_limit', '-1');

		if($req->filled('keyword')){
			$d = Registrasi::leftJoin('pasiens', 'pasiens.id', '=', 'registrasis.pasien_id')
				->with(['pasien', 'dokter_umum', 'poli', 'bayars', 'folioBelumLunas'])
			...
			$d = Cache::get($keyCache);
			if(!$d){
				$d = Registrasi::where('status_reg', 'like', 'G%')
				->with(['pasien', 'dokter_umum', 'poli', 'bayars', 'folioBelumLunas'])
				->where('created_at', 'LIKE', date('Y-m-d' . '%'))
				->select([
					'registrasis.id',
		...
		return DataTables::of($d)
			...
			->addColumn('rincian', function ($d) {
					return '
						<button type="button" onclick="rincianBiaya('.@$d->id.', \''.@$d->pasien->nama. '\', '.@$d->pasien->no_rm.')" class="btn btn-info btn-sm btn-flat">
							<i class="fa fa-search"></i>
						</button>
					';
			})
			->addColumn('sip', function ($d) {
				return '<a href="'.url('tindakan/cetak-sip/'. @$d->id).'" target="_blank" class="btn btn-sm btn-warning btn-flat"><i class="fa fa-print"></i></a>';
			})
			->addColumn('pulpak', function ($d) {
				return '<a href="'.url('kasir/cetak-surat-pulang-paksa/'. @$d->id).'" target="_blank" class="btn btn-sm btn-warning btn-flat"><i class="fa fa-print"></i></a>';
			})
			->rawColumns(['bayar', 'billing', 'rincian', 'sip', 'pulpak'])
			->addIndexColumn()
			->make(true);
	}
	...
	public function igdByTanggal(Request $request)
	{
		...
		// request()->validate(['tanggal' => 'required']);
		$query = Registrasi::where('status_reg', 'like', 'G%');
		...
		return view('kasir.index_igd',compact('today', 'bayar', 'poli', 'poliSelect'))->with('no', 1);
	}
	...
	public function bayar_tindakan_rawat_jalan($reg_id, $pasien_id)
	{
		$data['tag'] = Tagihan::where('registrasi_id', '=', $reg_id)->get();

		$data['reg'] = Registrasi::with(['bayars'])->find($reg_id);
		if (substr($data['reg']->status_reg,0,1) == 'G') { // IGD
			$data['fol'] = Folio::with(['dokterPelaksana', 'dokter', 'poli'])
		...
		// $data['jasa_racik'] = Folio::where('registrasi_id', '=', $reg_id)->where('folios.lunas', 'N')->sum('jasa_racik');
		// return $data; die;
		return view('kasir.bayar_tindakan_rawat_jalan', $data)->with('no', 1);
	}
	...
	public function save_bayar_tindakan_rawat_jalan(Request $request)
	{
		// dd($request->all());
		// return $request->all(); die;
		$r = Registrasi::find($request['registrasi_id']);
		...
		if ($request['total'] != 0) {
			if (rupiah($request['totalBayar']) < rupiah($request['total_tagihan'] && $request['diskon_persen'] == 0 && $request['diskon_rupiah'] == 0)) {
				...
				return redirect('kasir/cetakkuitansi/' . session('idk'));
				// return redirect('/kasir/rincian-biaya-tindakan/' . session('idk'));
				// return redirect('/kasir/rincian-biaya/' . session('idk'));
			}
		}

		Flashy::info('Total Pembayaran adalah 0, Tidak ada pembayaran');
		return redirect()->back();
	}

	public function save_bayar_rawat_jalan_obat(Request $request)
	{
		// dd($request->all());
		// return $request->all(); die;
		$r = Registrasi::find($request['registrasi_id']);
	...
	public function cetak_kuitansi_langsung($id = '')
	{
		...
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('jenis', $jenis)->get();
		$jml      = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('jenis', $jenis)->sum('total');
	...
	// 					->get();
		$no_resep = [];
		foreach($obat as $ob){
			$no_resep[] = $ob->namatarif;
		}
	   $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
	                    ->whereIn('penjualandetails.no_resep',$no_resep)
						->where('penjualans.registrasi_id',$kuitansi->registrasi_id)
						->select('penjualandetails.*')
						->get();
		...
		$norah = Pegawai::where('id', '=', 390)->first();
		return view('kasir.kuitansi', compact('norah','jenisracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj','Totracik','uangracik'));
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}

	public function cetak_kuitansi_dipilih(Request $request, $registrasi_id)
	{
		$idFolio = array(); 
		if($request['pilih']){
			
			foreach ($request['pilih'] as $folio_id) {
	
				$idFolio[] = $folio_id;
	
				
				$update_cetak = Folio::where('id',$folio_id)->first();
				$update_cetak->tercetak = 'Y';
				$update_cetak->lunas = 'Y';
				$update_cetak->dibayar = $update_cetak->total;
				$update_cetak->save();
	
				@$insert_pendapatan = Pendapatan::where('folio_id',$folio_id)->first();
			// // dd($insert_pendapatan);
				if(!$insert_pendapatan){
					@$insert_pendapatan = new Pendapatan();
				}
				@$insert_pendapatan->folio_id = @$folio_id;
				@$insert_pendapatan->registrasi_id = @$registrasi_id;
				@$insert_pendapatan->total = @$update_cetak->total;
				@$insert_pendapatan->user_id = Auth::user()->id;
				@$insert_pendapatan->save();
		...
		if(!$request['pilih']){
			$folio    = Folio::where('registrasi_id', $registrasi_id)->where('tercetak','Y')->get();
			if(count($folio) == 0){
				Flashy::info('Tidak ada tindakan yang diceklis');
				return redirect()->back();
			}


			$jml      = Folio::where('registrasi_id', $registrasi_id)->where('tercetak','Y')->sum('total');
			$idFolio = $folio->pluck('id')->toArray();
			@$bayar_umum = bayarUmum($registrasi_id,$jml,$idFolio);

		}else{
			$folio    = Folio::whereIn('id', $idFolio)->get();
			$jml      = Folio::whereIn('id', $idFolio)->sum('total');
			@$bayar_umum = bayarUmum($registrasi_id,$jml,$idFolio);
		}
		...
		$reg     = Registrasi::with('bayars')->find($registrasi_id);
		
		if(@$bayar_umum->id){
			$kuitansi = Pembayaran::find($bayar_umum->id);
		}else{
			$kuitansi = Pembayaran::where('registrasi_id', $registrasi_id)->first();
		}
		...
		// LUNASKAN KE PEMBAYARAN

		// @$id_folio_implode = implode(', ', $idFolio);

		// $insert_pendapatan = Pendapatan::where('registrasi_id',$registrasi_id)->first();
		// // dd($insert_pendapatan);

		// $insert_pendapatan = new Pendapatan();
		// $insert_pendapatan->folio_id = $id_folio_implode;
		// $insert_pendapatan->registrasi_id = $registrasi_id;
		// $insert_pendapatan->total = $jml;
		// $insert_pendapatan->user_id = Auth::user()->id;
		// $insert_pendapatan->save();

		return view('kasir.kuitansi-dipilih', compact('norah','kuitansi', 'folio', 'jml','reg'));
	}
	public function cetak_kuitansi_dipilih_eksekutif(Request $request, $registrasi_id)
	{
		$idFolio = array();
		foreach ($request['pilih'] as $folio_id) {
		...
		return view('kasir.kuitansi-dipilih-eksekutif', compact('norah','kuitansi', 'folio', 'jml','reg'));
	}

	public function cetak_kuitansi_langsung_tretribusi($id = '')
	{
		$kuitansi = Pembayaran::find($id);
		$reg     = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('lunas', 'Y')->where('namatarif','not like','%Retribusi%')
			->where('namatarif','not like','%Sticker%')
			->where('namatarif','not like','%Dokumen Medik%')
			->where('jenis', 'TA')
			->get();
	...
	// 					->get();
	   ...
	   $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('jenis','TA')
			->where('namatarif','not like','%Retribusi%')
			->where('namatarif','not like','%Sticker%')
			->where('namatarif','not like','%Dokumen Medik%')
			->where('lunas', 'Y')
		...
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		$norah = Pegawai::where('id', '=', 390)->first();
		return view('kasir.kuitansi-tindakan', compact('norah','jenisracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj','Totracik','uangracik'));
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}

	public function cetak_kuitansi_perkwitansi($id = '')
	{
		$kuitansi = Pembayaran::find($id);
		// dd($kuitansi);
		$reg     = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('no_kuitansi', $kuitansi->no_kwitansi)->where('namatarif', 'NOT LIKE','%STICKER%')->get();
		// dd($kuitansi->no_kwitansi);
		$jml      = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('no_kuitansi', $kuitansi->no_kwitansi)->where('namatarif', 'NOT LIKE','%STICKER%')->sum('total');
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			// ->where('lunas', '!=','Y')
			->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		return view('kasir.kuitansi_blmlunas', compact('kuitansi', 'folio', 'jml','reg','jml_irj'));
	}
	public function cetak_kuitansi_langsung_tindakan($id = '')
	{
		$kuitansi = Pembayaran::find($id);
		// dd($kuitansi);
		$reg     = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('jenis', 'TA')->where('poli_tipe', 'J')->get();
		$jml      = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		$jasa_racik  = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->first();
		$obat     = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->get();
       //dd($obat);
	   ...
	   $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir.kuitansi_tindakan', compact('jenisracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj','Totracik','uangracik'));
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}
	public function cetak_kuitansi_langsung_obat($id = '')
	{
		$kuitansi = Pembayaran::find($id);
		// dd($kuitansi);
		$reg     = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		...
		$jml      = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		$jasa_racik  = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->first();
		$obat     = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'Y')->get();
    //    dd($obat);
		$no_resep = [];
		foreach($obat as $ob){
			$no_resep[] = $ob->namatarif;
	...
	//    }
	   ...
	   $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir.kuitansi_obat', compact('jenisracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj','Totracik','uangracik'));
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}
	public function cetak_kuitansi_langsung_penunjang_rad($reg_id)
	{
		...
		$data['folio'] = Folio::where('registrasi_id', $id)
			...
			->selectRaw('sum(total) AS total,poli_tipe ,registrasi_id,namatarif,created_at,jenis,tarif_id')
			->orderBy('namatarif','ASC')
			->groupBy('tarif_id');

			// ->get();
		// $data['folio_usg'] = Folio::where('registrasi_id', $id)
		// ->with('tarif')
		// ->where('jenis','!=','ORJ')
		// ->whereNotIn('poli_tipe',['R','L'])
		...
		// $total_biaya = Folio::where('registrasi_id', $id)->sum(\DB::raw('total'));
		$data['total_biaya'] = $data['folio']->get()->sum('total');
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang Radiologi');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}
	public function cetak_kuitansi_langsung_penunjang_lab($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		$data['total_biaya'] = $data['folio']->get()->sum('total');
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang Laboratorium');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_usg($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang USG');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_ekg($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang EKG');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_ctscan($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada Billingan CT Scan');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_citologi($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang citologi');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_fnab($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang FNAB');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_pa_biopsi($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang PA Bopsi');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_kuitansi_langsung_penunjang_resume_medis($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
	...
	public function cetak_kuitansi_langsung_penunjang_pa_operasi($reg_id)
	{
		$id = $reg_id;
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
		...
		// dd($data['total_biaya']);
		if($data['total_biaya'] == 0){
			Flashy::info('Tidak ada data penunjang PA Operasi');
			return redirect()->back();
		}
		// dd($data['folio']->get());
		return view('kasir.kuitansi_penunjang', $data);
	...
	public function cetak_RincianBiaya($id = '') {
		...
		$nokuitansi       = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->where('namatarif', '!=', 'Retur penjualan')->where('jenis', 'ORJ')->where('lunas', 'Y')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->first();
		if(!$nokuitansi){
		  $noresep = '';
		}else{
		...
		$data['folio_irj_lab'] = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)
			...
			->where('namatarif', '!=', 'Retur penjualan')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, cyto, count(tarif_id) as jumlah, sum(total) as total')
			->get();
	   ...
	   // $data['folio_irna']   = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->groupBy('tarif_id')->where('lunas', 'Y')->where('jenis', 'TI')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->get();
		...
		$data['no'] = 1;
		$data['norah'] = Pegawai::where('id', '=', 390)->first();
		
		
		// $pdf        = PDF::loadView('kasir.cetakUlangKuitansi-all', $data)->setpaper('folio', 'landscape');
		// return $pdf->stream();
		return view('kasir.cetakUlangKuitansi-all', $data);
	}

	public function cetak_RincianBiayaRajal($id = '') {
			
		$data['kuitansi'] = Pembayaran::where('id',$id)->first();
		$data_kwit = Pembayaran::where('registrasi_id',$data['kuitansi']->registrasi_id)->get();
		$kwit = [];
		foreach($data_kwit as $d){
			$kwit[] = $d->no_kwitansi;
		}
		// dd($kwit);
		// dd($data['kuitansi']);
		...
		$data['penjualan'] = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
							...
							->select('penjualandetails.*')
							->get();
		// dd(explode('-',$data['kuitansi']->no_kwitansi));
        // dd($data['kuitansi']->no_kwitansi);
		// dd($data['penjualan']);
		...
		//kondisi to rad rajal
		$data['folio_irj_rad'] = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)
			->whereIn('no_kuitansi',$kwit)->where('lunas', 'Y')
			->where('jenis','TA')
			->where('poli_tipe','R')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total')
		...
		// dd($data['folio_irj'],$data['penjualan']);
	   ...
	   // $data['folio_irna']   = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->groupBy('tarif_id')->where('lunas', 'Y')->where('jenis', 'TI')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->get();
		...
		$data['jml'] = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)
			->where('lunas', 'Y')->whereIn('no_kuitansi',$kwit)->sum('total');
			$data['no'] = 1;
		...
		$data['jml_igd_lab'] = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)
			...
			->where('lunas', 'Y')
			->where('poli_tipe','L')
			->whereIn('no_kuitansi',$kwit)->sum('total');
		...
		$data['no'] = 1;
		// $pdf        = PDF::loadView('kasir.cetakUlangKuitansi-all', $data)->setpaper('folio', 'portrait');
		// return $pdf->stream();
		return view('kasir.cetakUlangKuitansi-all', $data);
	}
	public function cetak_RincianBiayaTindakan($id = '') {
	   ...
	   // $data['folio_irna']   = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->groupBy('tarif_id')->where('lunas', 'Y')->where('jenis', 'TI')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->get();
		...
		$data['no'] = 1;
		$data['norah'] = Pegawai::where('id', '=', 390)->first();
		$pdf        = PDF::loadView('kasir.cetakUlangKuitansiTindakan-all', $data)->setpaper('folio', 'landscape');
		return $pdf->stream();
		// return view('kasir.cetakUlangKuitansiTindakan-all', $data);
	}

	public function cetak_RincianBiayaTindakanRegistrasi($regId = '') {
		
		$data['reg']      = Registrasi::with('bayars')->find($regId);
       
		//kondisi to lab rajal
		$data['folio_irj_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('poli_tipe','L')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
			->get();
       
		//kondisi to rad rajal
		$data['folio_irj_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('poli_tipe','R')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		//kondisi to lab igd
		$data['folio_igd_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TG')
			->where('poli_tipe','L')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		//kondisi to rad igd
		$data['folio_igd_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TG')
			->where('poli_tipe','R')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		//kondisi to lab irna
		$data['folio_irna_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TI')
			->where('poli_tipe','L')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
	    ...
	    //kondisi to lab irna
		$data['folio_irna_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TI')
			->where('poli_tipe','R')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		//Master Loop
		$data['folio_irj'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('poli_tipe', 'J')
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		$data['folio_irna'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TI')
			->where('poli_tipe', null)
			->groupBy('tarif_id')
			->selectRaw('tarif_id,verif_kasa_user, count(tarif_id) as jumlah, sum(total) as total, cyto')
		...
		$data['dokter'] = Folio::whereIn('dokter_pelaksana', [$data['listDokter']])
			->groupBy('dokter_pelaksana')->select('dokter_pelaksana')->get();

		$data['jml'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->sum('total');
			$data['no'] = 1;

		//JUMLAH IGD

		$data['jml_igd'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TG')
			->where('lunas', 'Y')
			// ->where('poli_tipe', 'J')
			->Where('poli_tipe','G')
		...
		$data['jml_igd_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TG')
			->where('lunas', 'Y')
			->where('poli_tipe','L')
			->sum('total');

		$data['jml_igd_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TG')
			->where('lunas', 'Y')
			->where('poli_tipe','R')
			->sum('total');
		...
		$data['jml_irj_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('lunas', 'Y')
			->where('poli_tipe','L')
			->sum('total');

		$data['jml_irj_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('lunas', 'Y')
			->where('poli_tipe','R')
			->sum('total');
		
		$data['jml_irj'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TA')
			->where('lunas', 'Y')
			->where('poli_tipe', 'J')
			->sum('total');
		...
		$data['jml_irna_lab'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TI')
			->where('lunas', 'Y')
			->where('poli_tipe','L')
			->sum('total');

		$data['jml_irna_rad'] = Folio::where('registrasi_id', '=', $data['reg']->id)
			->where('jenis','TI')
			->where('lunas', 'Y')
			->where('poli_tipe','R')
			->sum('total');

		$data['jml_irna'] = Folio::where('registrasi_id', $data['reg']->id)
			->where('jenis','TI')
			->where('lunas', 'Y')
			->where('poli_tipe', null)
			->sum('total');
		...
		$data['no'] = 1;
		$data['norah'] = Pegawai::where('id', '=', 390)->first();
		$pdf        = PDF::loadView('kasir.cetakRincianTindakanRegistrasi', $data)->setpaper('folio', 'portrait');
		return $pdf->stream();
		// return view('kasir.cetakUlangKuitansiTindakan-all', $data);
	}
	...
	{
		$pemb = Pembayaran::where('created_at', 'LIKE', date('Y-m-d') . '%')->where('flag', 'Y')->orderBy('id', 'desc')->where('no_kwitansi', 'LIKE', 'RD%')->groupBy('registrasi_id')->get();
	...
	{
		request()->validate(['tga' => 'required', 'tgb' => 'required']);
		$pemb = Pembayaran::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])->where('flag', 'Y')->where('no_kwitansi', 'LIKE', 'RD%')->groupBy('registrasi_id')->get();
		return view('kasir.cetakIGD', compact('pemb'))->with('no', 1);
	}
	...
	public function cetakIRNA()
	{
		$pemb =  Pembayaran::join('rawatinaps', 'rawatinaps.registrasi_id', '=', 'pembayarans.registrasi_id')
			...
			->orderBy('id', 'desc')->where('no_kwitansi', 'LIKE', 'IRNA%')
	...
	public function cetakIRNAByTanggal(Request $request)
	{
		request()->validate(['tga' => 'required', 'tgb' => 'required']);
		$pemb = Pembayaran::join('rawatinaps', 'rawatinaps.registrasi_id', '=', 'pembayarans.registrasi_id')
			->whereBetween('rawatinaps.tgl_masuk', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])->where('no_kwitansi', 'LIKE', 'IRNA%')
			->select('pembayarans.id as id', 'pembayarans.registrasi_id as registrasi_id', 'pembayarans.pasien_id as pasien_id', 'rawatinaps.tgl_keluar as tgl_keluar')
			->get();
		// return $pemb; die;
		return view('kasir.cetakIRNA', compact('pemb'))->with('no', 1);
	...
	{
		$pemb = Pembayaran::where('created_at', 'LIKE', date('Y-m-d') . '%')->where('flag', 'Y')->where('no_kwitansi', 'LIKE', 'RJ%')->groupBy('registrasi_id')->select(DB::raw('*, max(id) as id'))->orderBy('id','DESC')->get();
	...
	{
		request()->validate(['tga' => 'required', 'tgb' => 'required']);
		$pemb = Pembayaran::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])->where('flag', 'Y')->where('no_kwitansi', 'LIKE', 'RJ%')->select(DB::raw('*, max(id) as id'))->orderBy('id','DESC')->groupBy('registrasi_id')->get();
		return view('kasir.cetak', compact('pemb'))->with('no', 1);
	}
	...
	public function cetakkuitansiIrna($id = '') 
	{
		$data['kuitansi'] = Pembayaran::find($id);
		$id = $data['kuitansi']->registrasi_id;
		// dd($id);
		$data['reg'] = Registrasi::find($id);
		$data['tagihan'] = Folio::where('registrasi_id', $id)
			->select('registrasi_id', 'namatarif','created_at', 'total', 'jenis')
		...
		$total_biaya = Folio::where('registrasi_id', $id)->sum(\DB::raw('total'));
		$jasa_racik = Folio::where('registrasi_id', $id)->sum(\DB::raw('jasa_racik'));
		$data['total_biaya'] = $total_biaya+$jasa_racik;
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir.kuitansi_inap_new', $data);
		// $pdf = PDF::loadView('kasir.kuitansi', compact('kuitansi', 'folio', 'jml'));
	
	}


	public function cetak_kuitansi_igd($id = '') {
		
		$kuitansi = Pembayaran::where('id',$id)->first();
		$reg      = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('namatarif', '!=', 'Retur penjualan')->where('namatarif','not like','%Resume Medis%')->where('no_kuitansi', $kuitansi->no_kwitansi)->where('jenis', '!=', 'ORJ')->get();
		$jml      = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('namatarif', '!=', 'Retur penjualan')->where('namatarif','not like','%Resume Medis%')->where('lunas', 'Y')->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		$jasa_racik  = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('no_kuitansi', $kuitansi->no_kwitansi)->where('lunas', 'Y')->where('namatarif', '!=', 'Retur penjualan')->where('namatarif','not like','%Resume Medis%')->first();
		// $obat     = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('namatarif', '!=', 'Retur penjualan')->where('no_kuitansi', $kuitansi->no_kwitansi)->where('lunas', 'Y')->get();
        // //dd($jml);
		if(!$jasa_racik ){
			$noresep = '';
		  }else{
		  ...
		  $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
						...
						->select('penjualandetails.*')
						->get();
		// $obat = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
		// ->where('namatarif', '!=', 'Retur penjualan')
		// ->where('no_kuitansi', $kuitansi->no_kwitansi)
		// ->where('jenis','ORJ')
		// ->sum(\DB::raw('total + jasa_racik'));
		...
		$jasa_racik = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('jenis', 'ORJ')->where('namatarif','not like','%Resume Medis%')->sum('jasa_racik');

		$jml_obat = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
					->where('namatarif','!=','Retur penjualan')
					->where('jenis','ORJ')
					->where('lunas', 'Y')
					->where('namatarif','not like','%Resume Medis%')
					->where('namatarif', '!=', 'Retur penjualan')
					->sum('total');
		$uangracik = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
					->where('penjualans.registrasi_id', $kuitansi->registrasi_id)->sum('uang_racik');
		...
		$Totracik = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
			->join('folios', 'folios.namatarif', '=', 'penjualans.no_resep')
			->where('folios.lunas', 'Y')
			->where('folios.namatarif','not like','%Resume Medis%')
			->where('penjualans.registrasi_id', $kuitansi->registrasi_id)
			->where('folios.jenis','ORJ')
			->groupBy('uang_racik')
			->where('folios.namatarif', '!=', 'Retur penjualan')
		...
		$jml_igd_lab = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('jenis','TG')
			->where('namatarif', '!=', 'Retur penjualan')
			->where('lunas', 'Y')
			->where('namatarif','not like','%Resume Medis%')
		...
		$jml_igd_rad = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('jenis','TG')
			->where('lunas', 'Y')
			->where('namatarif', '!=', 'Retur penjualan')
			->where('poli_tipe','R')
		...
		$folio_igd_lab = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('no_kuitansi', '=', $kuitansi->no_kwitansi)->where('lunas', 'Y')
			->where('jenis','TG')
			->where('namatarif', '!=', 'Retur penjualan')
			->where('namatarif','not like','%Resume Medis%')
			->where('poli_tipe','L')
		...
		//kondisi to rad igd
		$folio_igd_rad = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			->where('no_kuitansi', '=', $kuitansi->no_kwitansi)->where('lunas', 'Y')
			->where('jenis','TG')
			->where('namatarif', '!=', 'Retur penjualan')
			->where('namatarif','not like','%Resume Medis%')
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			->where('namatarif','not like','%Resume Medis%')
			->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		$jml_igd = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			return view('kasir.kuitansi_igd_new', compact('jenisracik','Totracik','uangracik','kuitansi', 'folio', 'jml', 'jml_igd', 'jasa_racik','uang_racik','penjualan','reg','jml_obat','jml_irj', 'jml_igd_lab', 'jml_igd_rad', 'folio_igd_lab', 'folio_igd_rad'));
	}

	public function cetak_kuitansi_eksekutif($registrasi_id) {
		
		$kuitansi = Pembayaran::where('registrasi_id',$registrasi_id)->first();

		if (empty($kuitansi)) {
			Flashy::info('Belum ada pembayaran !');
			return redirect()->back();
		}
		$reg      = Registrasi::with('bayars')->find($kuitansi->registrasi_id);
		$folio    = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->where('namatarif', '!=', 'Retur penjualan')->where('namatarif','not like','%Resume Medis%')->where('no_kuitansi', $kuitansi->no_kwitansi)->where('jenis', '!=', 'ORJ')->get();
		...
		$jml_igd = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			return view('kasir.kuitansi_eksekutif', compact('jenisracik','Totracik','uangracik','kuitansi', 'folio', 'jml', 'jml_igd', 'jasa_racik','uang_racik','penjualan','reg','jml_obat','jml_irj', 'jml_igd_lab', 'jml_igd_rad', 'folio_igd_lab', 'folio_igd_rad'));
	}




	public function cetak_kuitansi_ranap($id = '') {
		$data['kuitansi'] = Pembayaran::find($id);
		$reg_id = $data['kuitansi']->registrasi_id;
		$data['reg'] = Registrasi::find($reg_id);
		$data['tagihan'] = Folio::where('registrasi_id', $reg_id)
			->select('registrasi_id', 'namatarif','created_at', 'total', 'jenis')
			->get();
			// dd($data['tagihan']);
		$data['obat'] = Folio::where('registrasi_id', '=', $reg_id)->where('jenis','ORJ')->sum(\DB::raw('total'));
		$data['folio'] = Folio::where('registrasi_id', $reg_id)
			->with('tarif')
			->where('jenis','!=','ORJ')
			->whereIn('poli_tipe',['G', 'J','O','M','A','I','Z','A','B','V','H','D'])
			->selectRaw('sum(total) AS total,poli_tipe ,registrasi_id,namatarif,created_at,jenis,tarif_id')
		...
		$data['folio_null_tipe'] = Folio::where('registrasi_id', $reg_id)
		->with('tarif')
		->where('jenis','!=','ORJ')
		->whereNull('poli_tipe')
		->selectRaw('sum(total) AS total,poli_tipe ,registrasi_id,namatarif,created_at,jenis,tarif_id')
		...
		$data['lab'] = Folio::where('registrasi_id', $reg_id)
			->where('poli_tipe','L')
			->sum(\DB::raw('total'));

		$data['rad'] = Folio::where('registrasi_id', $reg_id)
			->where('poli_tipe','R')
			->sum(\DB::raw('total'));
		$data['uang_racik'] = Folio::where('registrasi_id', $reg_id)->sum(\DB::raw('jasa_racik'));	
		$data['totals'] = Folio::where('registrasi_id', '=', $reg_id)->sum(\DB::raw('total'));
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir.kuitansi_ranap_new', $data);	
	}
	...
	public function transaksi_lain_lain()
	{
		...
		foreach ($reg as $key => $d) {
			$row[] = [
				...
				'no_rm' => (@$d->status_reg == 'A1') ? @Registrasi::find(@$d->id)->first()->pasien->no_rm : @Pasienlangsung::where('registrasi_id', @$d->id)->first()->no_rm,
				'alamat' => (@$d->status_reg == 'A1') ? @Registrasi::find(@$d->id)->first()->pasien->alamat : @Pasienlangsung::where('registrasi_id', @$d->id)->first()->alamat,
	...
	public function form_bayar_lain_lain($registrasi_id = '')
	{
		...
		$data['data_obat'] = Folio::where('registrasi_id', $registrasi_id)->where('lunas', 'N')->where('jenis', 'ORJ')->first();;
		if ($reg->status_reg == 'A1') {
			// $data['total'] = Folio::where('registrasi_id', $registrasi_id)->where('lunas', 'N')->first()->total;
			$data['total'] = Folio::where('registrasi_id', $registrasi_id)->where('lunas', 'N')->where('jenis', 'ORJ')->sum('total');
			...
			// $data['jasa_racik'] = Folio::where('registrasi_id', '=', $registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'N')->sum('jasa_racik');
	...
	{
		// $data['reg'] = Registrasi::where('created_at', 'LIKE', date('Y-m-d') . '%')->whereIn('lunas', ['Y','P'])->where('bayar', '<>', '1')->get();
		// Pembayaran::where('flag', 'N')->where('created_at', '<', date('Y-m-d 00:00:00'))->delete();
		$data['reg'] = Pembayaran::with(['registrasis', 'registrasis.pasien', 'registrasis.poli', 'user'])
			->where('created_at', 'LIKE', date('Y-m-d') . '%')
			->where('flag', 'Y')
			->get();
	...
	{
		request()->validate(['tga' => 'required', 'tgb' => 'required']);
		// $data['reg'] = Registrasi::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])->whereIn('lunas', ['Y','P'])->get();
		$data['reg'] = Pembayaran::with(['registrasis', 'registrasis.pasien', 'registrasis.poli', 'user'])
			->whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])
			->where('flag', 'Y')
			->get();
	...
	public function data_batal_bayar(Request $req){
		ini_set('max_execution_time', 0); //0=NOLIMIT
		ini_set('memory_limit', '-1');

		if($req->filled('keyword')){
			$d = Pembayaran::with(['registrasis', 'registrasis.pasien', 'registrasis.poli', 'user'])
				->leftJoin('registrasis', 'registrasis.id', '=', 'pembayarans.registrasi_id')
				->leftJoin('pasiens', 'pasiens.id', '=', 'registrasis.pasien_id')
				->where(function ($query) use ($req){
					$query->where('pasiens.no_rm', $req->keyword)
						->orWhere('pasiens.nama', 'LIKE', "%$req->keyword%");
				})
				->where('pembayarans.flag', 'Y')
				->whereNull('registrasis.deleted_at')
				->orderBy('pembayarans.id', 'DESC')
				->select([
					'pembayarans.id',
					'pembayarans.registrasi_id',
					'pembayarans.user_id',
					'pembayarans.dibayar',
		...
		return DataTables::of($d)
			...
			->addColumn('no_rm', function ($d) {
				return @$d->registrasis->pasien->no_rm;
			})
			->addColumn('nama', function ($d) {
				return @$d->registrasis->pasien->nama;
			})
			->addColumn('poli', function ($d) {
				return @$d->registrasis->poli->nama;
			})
			->addColumn('alamat', function ($d) {
				return @$d->registrasis->pasien->alamat;
			})
			->addColumn('tglReg', function ($d) {
				return date('d-m-Y H:i', strtotime(@$d->registrasis->created_at));
			})
			->addColumn('tglBayar', function ($d) {
			...
			->addColumn('kwitansi', function ($d) {
				return @$d->no_kwitansi ?? '-';
			})
			->addColumn('dibayar', function ($d) {
				return number_format(@$d->dibayar);
			})
			->addColumn('detail', function ($d) {
					return '<button class="btn btn-primary btn-sm btn-flat" id="showRincian" data-id='.$d->id.'><i class="fa fa-search-plus"></i></button>';
			})
			->rawColumns(['detail'])
			->addIndexColumn()
			->make(true);
	...
	public function rincian_pembayaran($id)
	{
		$data['pembayaran'] = Pembayaran::where('id', $id)->where('flag', 'Y')->first();
		$data['reg'] = Registrasi::where('id', $data['pembayaran']->registrasi_id)->first();
		$data['rincian'] = Folio::where('registrasi_id', $data['pembayaran']->registrasi_id)
			->where('no_kuitansi', $data['pembayaran']->no_kwitansi)->get();
		$data['jasa_racik'] = $data['rincian']->sum('jasa_racik');
		$data['total'] = $data['rincian']->sum('total');
	...
	public function save_pembatalan($registrasi_id, $no_kwitansi)
	{
        ...
        if($pembayaran == null){
            Flashy::error('Kwitansi tidak ditemukan!');
            return redirect()->back();
        }
		...
		DB::transaction(function () use ($registrasi_id, $no_kwitansi, $dibayar) {
			...
			// if ($cek_piutang <= 0) {
				$folio = Folio::where('registrasi_id', $registrasi_id)->where('no_kuitansi', $no_kwitansi)->get();
				foreach ($folio as $d) {
					$fol = Folio::find($d->id);
					$fol->lunas = 'N';
				...
				$journal = Journal::where('code', $no_kwitansi)->first();
				if($journal){

					$journalDetail = JournalDetail::where('id_journal', $journal->id)->get();
					foreach($journalDetail as $d){
						$detail = JournalDetail::find($d->id);
						$detail->delete();
					}
	...
	public function hapusTindakanSelect(Request $request){
		// dd($request->all());
		$idFolio = $request->idFolio;
		$reg = Registrasi::find($request->idReg);

		Folio::where('registrasi_id', $reg->id)
			->whereIn('id', explode(', ', $idFolio))
			->delete();
	...
	public function cariPasienProses(Request $request)
	{
		...
		$today = Registrasi::where('registrasis.pasien_id', @$data['pasien']->id)
		->orderby('registrasis.id','DESC')
		->get();
		// return $data; die;
		return view('kasir.cari-pasien', $today)->with('no',1);
	}

	public function cetakSuratPulangPaksa($registrasi_id){
		$data['registrasi'] = Registrasi::find($registrasi_id);

		return view('kasir.cetakSuratPulangPaksa', $data);
	}

	public function buatSip(Request $request){

		$registrasi = Registrasi::find($request['regId']);
		$registrasi->keterangan_sip = $request['keterangan'];
		$registrasi->update();
		// dd($registrasi);

		$sip = new SuratIzinPulang();
		$sip->registrasi_id = $registrasi->id;
		$sip->status = $request['status'];
		$sip->pembayaran = $request['pembayaran'];
		$sip->save();

		return response()->json([
			'sukses' => true,
			'text' => 'Sukses Membuat SIP'
		]);
	}

	public function batalSIP(Request $request) {

		$registrasi = Registrasi::find($request['regId']);
		$registrasi->keterangan_sip = null;
		$registrasi->update();

		$sip = SuratIzinPulang::where('registrasi_id', $registrasi->id)->first();
		$sip->delete();

		return response()->json([
			'sukses' => true,
			'text' => 'Sukses Membatalkan SIP'
		]);
	}
}
