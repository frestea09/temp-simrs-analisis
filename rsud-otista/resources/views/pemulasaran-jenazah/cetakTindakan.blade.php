...
.table-hover>tbody>tr:hover {
    ...
    <tr>
      ...
      <td style="width: 50%">
        ...
        <table style="width: 100%">
          ...
          <tr> <td>Unit yang Order</td> <td>:</td><td> Pemulasaran Jenazah & Ambulans</td></tr>
  ...
  <table class="table table-condensed" style="width: 100%">
      ...
      @endphp
        <tr>
          ...
          @if(date('Y-m-d H:i', strtotime($d->created_at)) < config('app.tarif_new') && date('Y-m-d H:i', strtotime($d->updated_at)) < config('app.tarif_new'))
            <td style="text-align: center">{{ ($d->tarif_lama->total <> 0) ? ($d->total + $d->diskon) / $d->tarif_lama->total : NULL }}</td>
            <td style="text-align: right">{{ ($d->tarif_id == 10000) ? number_format($d->total) : number_format($d->tarif_lama->total) }}</td>
          @else
            <td style="text-align: center">{{ ($d->tarif_baru->total <> 0) ? ($d->total + $d->diskon) / $d->tarif_baru->total : NULL }}</td>
            <td style="text-align: right">{{ ($d->tarif_id == 10000) ? number_format($d->total) : number_format($d->tarif_baru->total) }}</td>
          @endif
    ...
    <tr>
      <td colspan="6"><i>Terbilang: {{ terbilang($tindakan->sum('total')) }} rupiah</i></td>
    ...
    <tr>
      ...
      <th style="text-align: right">{{ number_format($total - $dibayar) }}</th>
    </tr>
  </table>
  ...
  <table style="width: 60%; float: right;">
      ...
      <td style="text-align: center;max-width:150px;">
        Tanda Tangan Pasien <br /><br /><br /><br /><br /><br />
        ( {{ $reg->pasien->nama }} )
      </td>
      <td style="text-align: center">
