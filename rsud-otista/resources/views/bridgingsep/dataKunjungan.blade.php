@extends('master')
@section('header')
  <h1>Data Kunjungan SEP <small></small></h1>
@endsection
        ...
        <div class="col-sm-6">
          <form method="POST" class="form-horizontal" id="formKunjungan">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <input type="text" name="tglSep" value="{{ date('d-m-Y') }}" class="form-control datepicker">
              </div>
            </div>
            <div class="form-group">
            ...
            <table class="table table-hover table-bordered">
              <thead class="bg-primary">
                <tr>
                  ...
                  <th class="text-center" style="vertical-align: middle;">Jns Pelayanan</th>
                  <th class="text-center kelasRawat" style="vertical-align: middle;">Kls Rawat</th>
                  ...
                  <th class="text-center poli" style="vertical-align: middle;">Poli</th>
                  <th class="text-center" style="vertical-align: middle;">Tgl SEP</th>
                  <th class="text-center" style="vertical-align: middle;">Tgl Plg SEP</th>
                </tr>
              </thead>
              <tbody>
...
@section('script')
  ...
  <script type="text/javascript">
    ...
    function tampilkanHistoriSEP() {
      $('.respon').html('');
      $.ajax({
        url: '/bridgingsep/data-kunjungan',
        type: 'POST',
        dataType: 'json',
      ...
      .done(function(res) {
        ...
        if (data[0].metaData.code == 200) {
          ...
          if ($('select[name="jenisPelayanan"]').val() == 1) {
            ...
            $.each(data[1].sep, function(index, val) {
              $('tbody').append('<tr> <td>'+(index + 1)+'</td>  <td>'+val.diagnosa+'</td> <td>'+val.jnsPelayanan+'</td>  <td class="text-center">'+val.kelasRawat+'</td> <td>'+val.nama+'</td> <td>'+val.noKartu+'</td> <td>'+val.noRujukan+'</td> <td>'+val.noSep+'</td>  <td>'+val.tglSep+'</td> <td>'+val.tglPlgSep+'</td> </tr>')
             });
          } else {
            $('.poli').removeClass('hidden')
            $('.kelasRawat').addClass('hidden')
            $.each(data[1].sep, function(index, val) {
               $('tbody').append('<tr> <td>'+(index + 1)+'</td>  <td>'+val.diagnosa+'</td> <td>'+val.jnsPelayanan+'</td>  <td>'+val.nama+'</td> <td>'+val.noKartu+'</td> <td>'+val.noRujukan+'</td> <td>'+val.noSep+'</td> <td>'+val.poli+'</td>  <td>'+val.tglSep+'</td> <td>'+val.tglPlgSep+'</td> </tr>')
            });
          }
        }
      });
