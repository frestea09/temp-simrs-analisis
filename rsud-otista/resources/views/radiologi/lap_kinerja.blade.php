...
@section('content')
  <div class="box box-primary">
    <div class="box-body">
       <div class="row">
        <form action="{{ url('radiologi/laporan-kinerja') }}" method="POST" class="form-horizontal">
          {{ csrf_field() }}
            <div class="col-sm-6">
                  ...
                  @if (isset($data) && !empty($data))
                    @foreach ($data as $d)
                      @php
                        ...
                        $reg = \Modules\Registrasi\Entities\Registrasi::select('poli_id', 'pasien_id')->where('id', $d->registrasi_id)->first();
                        $irna = \App\Rawatinap::select('kamar_id')->where('registrasi_id', $d->registrasi_id)->first();
                        @$pasien = \Modules\Pasien\Entities\Pasien::select('nama', 'no_rm', 'kelamin')->where('id', $reg->pasien_id)->first();
                     ...
                     @endphp
                      <tr>
                        ...
                        <td>
                          ...
                          @elseif($d->jenis == 'TI')
                            {{ $irna ? baca_kamar($irna->kamar_id) : '-' }}
                          @endif
                        </td>
                        <td>{{ baca_carabayar($d->cara_bayar_id) }} {{@$d->registrasi->tipe_jkn}}</td>
                        <td>{{ @date('d/m/Y H:i:s',strtotime(@$ekspertise->created_at)) }}</td>
