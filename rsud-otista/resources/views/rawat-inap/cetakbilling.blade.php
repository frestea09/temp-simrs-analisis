                ...
                @foreach ($folio_irj as $noirj => $d)
                  <tr>
                    ...
                    @if (@$d->verif_kasa_user = 'tarif_new')
                      <td class="text-right">{{ (@$d->tarif_baru->total <> 0 ) ? (@$d->total / @$d->tarif_baru->total) : '' }}</td>
                      <td class="text-center">{{ (@$d->tarif_baru->total <> 0 ) ? number_format($d->tarif_baru->total,0,',','.') : '' }}</td>
                    @else
                      <td class="text-right">{{ (@$d->tarif->total <> 0 ) ? (@$d->total / @$d->tarif->total) : '' }}</td>
                      <td class="text-center">{{ (@$d->tarif->total <> 0 ) ? number_format($d->tarif->total,0,',','.') : '' }}</td>
                    @endif
