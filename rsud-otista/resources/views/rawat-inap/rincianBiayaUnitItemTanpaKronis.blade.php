  ...
  {{-- <body> --}}
    ...
    {{-- <table style="width:95%; margin-bottom: -10px;">
      ...
      {{-- END FOLIO RAJAL --}}
       ...
       {{-- JIKA ADA BANK DARAH Rajal --}}
       @if (count($tindakan_bank_darah_irj) > 0)
       <tr>
         <th style="border-bottom:1px solid black;border-top:1px solid black; text-align:start;">BANK DARAH RAJAL</th>
       ...
       @php
         @$total_tindakan_bank_darah_irj = 0
       @endphp
       @foreach ($tindakan_bank_darah_irj as $key => $item)
         @php
           @$total_tindakan_bank_darah_irj += $item->total;
           if($item->verif_kasa_user =='tarif_new'){
             @$jml = @floor(@$item->total / @$item->tarif_baru->total);
           }else{
             @$jml = @floor(@$item->total / @$item->tarif->total);
       ...
       @endforeach
       @if ($total_tindakan_bank_darah_irj >0)
         <tr>
           <td colspan="3"><span style="text-align:start;font-weight:900">Total Bank Darah</span></td>
           <td><span style="color:grey">Rp.</span><span style="float:right"><b>{{number_format($total_tindakan_bank_darah_irj)}}</b></span></td>
         </tr>
       @endif
     @endif
            ...
            <tr>
              ...
              @foreach ($detail_obat_gudang_rajal_null as $key => $item)
               ...
               <td><span style="color:grey">Rp.</span><span style="float:right">{{number_format($items->hargajual)}}</span></td>
                    ...
                    @php
                        $total_obat_rajal += @$items->hargajual;
                    @endphp
                  @endforeach
                @endif
              @endforeach
      ...
      <tr>
        ...
        <td class="dotTop"><span style="color:grey">Rp.</span><span style="float:right"><b>{{number_format(@$total_obat_operasi+@$total_obat_rajal+@$tindakan_rajal+@$lab_irj+@$rad_irj+@$total_operasi+@$total_tindakan_bank_darah_irj)}}</b></span></td>
      </tr>
