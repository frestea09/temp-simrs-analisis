...
@section('header')
    <h1 style="font-size: 20pt;color:black;font-weight: bold;">Front Office - Daftar Antrian Loket {{ $no_loket }} JKN
    </h1>
@endsection
...
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            <h3 class="box-title" id="statistik-registrasi">
                                    ...
                                    <tbody>
                                        @foreach ($terpanggil as $key => $d)
                                            <tr>
                                                ...
                                                <td>{{ $d->created_at }}</td>
                                                <td>
                                                    {{-- @if (cek_registrasi_new(@$d->id, $bagian) < 1) --}}
                                                        @if ($d->status <= 2)
                                                            <div class="btn-group" style="min-width:0px !important">
                                                                <a href="{{ url('antrian-news/' . $bagian . '/' . $no_loket . '/panggilkembali/' . $d->id) }}"
                                                        ...
                                                        <button type="button" class="btn btn-success btn-flat btn-sm"
                                                            onclick="searchPasien({{ @$d->id }})"><i
                                                                class="fa fa-registered"></i> Proses</button>
                                                    {{-- @else --}}
                                                        {{-- <span style="color:green">Terproses</span> --}}
                                                    {{-- @endif --}}
                                                </td>
                                            </tr>
                                        @endforeach
    ...
    <div class="modal fade" id="pasien">
                                ...
                                <div class="box box-primary" style="border: 1.5px solid #0275d8 !important;">
                                    ...
                                    <div class="box-body">
                                        ...
                                        <a href="{{ url('/registrasi/create') }}"
                                            class="btn btn-primary btn-flat btn-sm">JKN</a>
                                        <a href="{{ url('/registrasi/create_umum') }}"
                                            class="btn btn-success btn-flat btn-sm">NON JKN</a>
                                        <p style="margin-top:15px"><small class="text-primary">*Pasien Baru adalah Pasien
                                                yang belum pernah berobat dan baru mengunjungi RS pertama kali.</small></p>
                                    </div>
                                </div>
                            </div>
                            <div class="col-xs-6">
                                <div class="box box-primary" style="border: 1.5px solid #D9534F !important;">
                                    ...
                                    <div class="box-body">
                                        ...
                                        <a href="{{ url('/antrian/reg_blmterdata/1/jkn') }}"
                                            class="btn btn-default btn-flat btn-sm">JKN</a>
                                        <a href="{{ url('/antrian/reg_blmterdata/1/umum') }}"
                                            class="btn btn-default btn-flat btn-sm">NON JKN</a>
                                        <p style="margin-top:15px"><small class="text-danger">*Pasien Belum Terdata adalah
                                                Pasien yang secara berkas sudah pernah tercatat di Rekam Medis namun belum
                                                terinput di Database SIMRS.</small></p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="table-responsive" style="margin-top: -30px;">
                                    <div class="controls">
                                        <div class="col-md-2">
                                            <input type="text" name="no_rm" id="no_rm_s" class="form-control"
                                                placeholder="No.RM">
                                        </div>
                                        <div class="col-md-2">
                                        ...
                                        <div class="col-md-2">
                                            <input type="text" name="tgllahir" id="tgllahirs" class="form-control"
                                                placeholder="TglLahir : 06101993">
                                        </div>
                                        <div class="col-md-3">
                                            <input type="text" name="alamats" id="alamats" class="form-control"
                                                placeholder="Alamat">
                                        </div>
                                        <div class="col-md-3">
                                            <button type="text" id="btnFiterSubmitSearch" class="btn btn-info"><i
                                                    class="fa fa-search"></i> Cari</button>
                                            <button style="margin-left:80px;" type="text" id="btnFiterSubmitRefresh"
                                                class="btn btn-danger"><i class="fa fa-refresh"></i> Refresh</button>
                                        </div>
                                    </div>
                                    &nbsp;
                                    {{-- <input type="text" name="email" class="form-control searchEmail" placeholder="Search for Email Only..."/> --}}
                                    <table class="table-hover table-condensed table-bordered table" id="tablePasien">
                                        <thead>
                                            <tr>
        ...
        <script type="text/javascript">
            ...
            function loadDaftarAntrian() {
                no_loket = '{{ $no_loket }}'
                bagian = '{{ $bagian }}'
                url = '/antrian-news/' + bagian + '/' + no_loket + '/daftarpanggil'
                $('#daftarantrian').load(url);
            }
            ...
            function searchPasien(antrian_id) {
                ...
                table = $('#tablePasien').DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: '/pasien/search-pasien/' + antrian_id + '/' + no_loket,
                        data: function(d) {
                            d.no_rm = $('#no_rm_s').val();
        ...
        <script>
            $(document).ready(function () {
                loadStatistikRegistrasi();
            });

            function loadStatistikRegistrasi() {
                $.ajax({
                    url: '/antrian-news-statistik-registrasi',
                    method: 'GET',
                    success: function (data) {
                        $('#statistik-registrasi').html(
                            `Data Antrian Hari Ini | Perjanjian : ${data.regPerjanjian} | Online : ${data.regOnline} | Onsite : ${data.regOnsite}`
                        );
                    },
                    error: function () {
                        $('#statistik-registrasi').html("Gagal memuat data registrasi.");
                    }
                });
            }
        </script>
