...
<html lang="">
  <head>
    ...
    <title>Rincian Penjualan Obat Kronis</title>
  ...
  <body onload="print()">
              ...
              @foreach ($detail_obat_gudang_rajal as $key => $item)
                @if (count($item->obat) > 0)
                    ...
                    @foreach ($item->obat as $items)
                        ...
                        @php
                            $total_obat_rajal2 += $items->hargajual;
                        @endphp
                    @endforeach
                @endif
              @endforeach
            @endif
            @if ($total_obat_rajal > 0 || $total_obat_rajal2 > 0)
             {{-- OBAT RAJAL --}}
              <tr>
                <td colspan="3"><span style="text-align:start;font-weight:900">Total Obat RAJAL</span></td>
                <td><span style="color:grey">Rp.</span><span style="float:right"><b>{{number_format($total_obat_rajal+$total_obat_rajal2)}}</b></span></td>
              </tr>    
            @endif
            <tr><td colspan="4"><div style="height:5px;"></div></td></tr>
            {{--END OBAT RAJAL--}}
      ...
      <tr>
        ...
        <td class="dotTop"><span style="color:grey">Rp.</span><span style="float:right"><b>{{number_format(@$total_obat_operasi+@$total_obat_rajal2+@$total_obat_rajal+@$tindakan_rajal+@$lab_irj+@$rad_irj+@$total_operasi)}}</b></span></td>
      </tr>
      </table>
      <br/>
