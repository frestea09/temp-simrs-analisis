...
<body>
                    ...
                    <div class="blockloket2">
                        ...
                        <div class="d-flex" style="height: 320px; overflow: hidden">
                            <div class="flex-1 container-box" style="border-right: black solid 2px;  overflow: hidden;">
                                ...
                                <div style="padding: 2rem; margin-top: 5rem;" id="top-point">
                                    <table class="table-list-pasien">
                                        <tbody id="antrian_belum_dipanggil_kemuning">
                                        </tbody>
                                    </table>
                                </div>
                            ...
                            <div class="flex-1">
                                ...
                                <div class="text-center">
                                    {{-- @if( !baca_nomorantrian_bpjs(@$antrianKM->register_antrian->nomorantrian) )
                                        <div id="16_antrian" class="nomor_antrian">
                                            {{ @$antrianKM->kelompok . @$antrianKM->nomor }}
                                        </div>
                                    @else
                                    @endif --}}
                                    <div id="16_antrian" class="nomor_antrian">
                                        {{ @$antrianKM->register_antrian->nomorantrian_jkn }}
                                    </div>
                                    <div id="16_nama" class="nama_pasien">
                                        {{ @$antrianKM->register_antrian->pasien->nama }}
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script type="text/javascript">
        ...
        function ajaxAntrian() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    let antrianDelay = antrianDipanggil.length * 9000;
                    setTimeout(() => {
                        startInterval();
                        console.log('mulai ajax lagi dalam ' + (antrianDelay + ajaxDelay)+ 's');
                    }, antrianDelay);
                    ...
                    antrianDipanggil.forEach(antrian => {
                        if(antrian != null){
                            if (bacaNomorAntrianBPJS(antrian.register_antrian.nomorantrian)) {
                                ...
                                $('#' + antrian.poli_id + '_antrian').html(antrian.register_antrian.nomorantrian_jkn);
                                $('#' + antrian.poli_id + '_nama').html(antrian.register_antrian?.pasien?.nama ?? '');
                                console.log('#' + antrian.poli_id + '_antrian', $('#' + antrian.poli_id + '_antrian'));
                                // 2X play
                                audios.push(
                                    // new Audio('/audio/in.mp3'),
                                    // new Audio('/audio/nomorurut.mp3'),
        ...
        function ajaxAntrianBP() {
            $.ajax({
                type: 'GET',
                url: '/antrian_poliklinik/tv2/belum-dipanggil',
                success: function (data) {
                    let poli_kemuning = data;
                    poli_kemuning.sort((a, b) => {
                        let numA = parseInt((a?.nomorantrian || '').match(/\d+$/)?.[0]) || 0;
                        let numB = parseInt((b?.nomorantrian || '').match(/\d+$/)?.[0]) || 0;
                        return numA - numB; // ascending
                    });
                    let html;
                    poli_kemuning.forEach(kemuning => {
                        if (kemuning != null) {
                            // if (bacaNomorAntrianBPJS(kemuning.nomorantrian)) {
                            //     html += `<tr>
                            //                 <td class="nomor">${bacaNomorAntrianBPJS(kemuning.nomorantrian)}</td>
                            //                 <td class="text-center">${kemuning?.nama ?? ''}</td>
                            //             </tr>`
                            // } else {
                                html += `<tr>
                                            <td class="nomor">${kemuning.nomorantrian}</td>
                                            <td class="text-center">${kemuning?.nama ?? ''}</td>
                                        </tr>`
                            // }
                        }
                    })
                    $('#antrian_belum_dipanggil_kemuning').html(html);
                    if (html == undefined) {
                        $('#antrian_belum_dipanggil_kemuning').html('');
                    }
                    html = null;
                }
            });
