...
@section('content')
  <div class="box box-primary">
    ...
    <div class="box-body">
      {!! Form::open(['method' => 'POST', 'url' => 'copy-resep/darurat', 'class'=>'form-horizontal']) !!}
        <div class="row">
          <div class="col-md-6">
         ...
         <table class='table table-striped table-bordered table-hover table-condensed' id='data'>
           ...
           <tbody>
               @foreach ($data as $key => $d)
                 @php
                   $penjualan = App\CopyResep::where('registrasi_id', $d->registrasi_id)->orderBy('created_at', 'asc')->get();
                 @endphp
                   <tr>
                     <td>{{ $no++ }}</td>
                     ...
                     <td>
                        <a href="{{ url('copy-resep/form-penjualan/'.$d->pasien_id).'/'.$d->registrasi_id }}" class="btn btn-success btn-flat btn-sm"><i class="fa fa-check"></i></a>
                     </td>
                     <td>
                       @if ($penjualan->first())
                       <div class="btn-group">
                      ...
                      <td>
                       @foreach ($penjualan as $d)
                          ...
                          @endphp
                          @if ($detail > 0)
                          <a href="{{ url('farmasi/cetak-copy-resep-fakturkronis/'.$d->id) }}" class="btn btn-danger btn-flat btn-sm" target="_blank"> <i class="fa fa-file-pdf-o"></i> </a>
                          @endif
                       @endforeach
                     </td> 
                     <td>
