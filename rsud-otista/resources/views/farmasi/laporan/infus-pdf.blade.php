...
@isset($penjualan->id)
  @php
    $det = App\Penjualandetail::where('penjualan_id', $penjualan->id)
        ->where('cetak', 'Y')
        ->whereHas('masterobat', function ($q) {
            $q->where('nama', 'like', '%infus%')
              ->orWhere('nama', 'like', '%nacl%');
        })
        ->groupBy('masterobat_id')
        ->select('*', DB::raw('SUM(jumlah) as jumlahTotal'), DB::raw('SUM(hargajual) as hargaTotal'))
        ->get();
    $no = 1;
  @endphp
  ...
  <div class="ticket">
      ...
      <table style="text-align: left;">
        <tbody>
          ...
          <tr>
            <th style="font-size:10px;font-family:Arial">Nama </th> <td style="font-size:10px;font-family:Arial"><b> : {{ @$reg->pasien->nama }} / {{ strtoupper( @$reg->pasien->kelamin == 'L') ? 'L' : 'P' }} / {{ hitung_umur(@$reg->pasien->tgllahir,'Y') }}</b></td>
            <th style="font-size:10px;"></th> <td style="font-size:10px;font-family:Arial"> </td>
          </tr>
          <tr>
        ...
        <table style="text-align: left">
          <tbody>
            <tr>
              ...
              <td style="font-size:10px;font-family:Arial"><b>{{ strtoupper($d->etiket) }}</b></td> 
              @else
              <td style="font-size:11px;font-family:Arial"><b>{{ @$d->etiket == null || @$d->etiket == '-' ? @$d->cara_minum : @$d->etiket }}</b></td> 
              {{-- <th style="font-size:10px;"></th> <td style="font-size:10px;font-family:Arial"></td>  --}}
              @endif
            </tr>
            <tr>
              ...
              @endphp
              <td style="font-size:10px;font-family:Arial"><b>{{ $namaobat }} / {{ strtoupper($d->informasi1) }} </b></td>
              @if( \App\LogistikBatch::where('id', $d->logistik_batch_id)->first() )
                <th style="font-size:10px;"></th> <td style="font-size:10px;font-family:Arial"></b><b>{{ strtoupper($d->jumlahTotal) }}.0 </b></b></td>
              @else
                <th style="font-size:10px;"></th> <td style="font-size:10px;font-family:Arial">  </td>
              @endif
            </tr>
            <tr>
              @if (\App\masterCaraMinum::where('id',$d->cara_minum_id)->first())
              <td style="font-size:14px;font-family:Arial"><b>{{ strtoupper(\App\masterCaraMinum::where('id',$d->cara_minum_id)->first()->nama) }}</b>
  ...
  <table class="table  table-condensed">
    <tr>
      <td colspan="2" style="text-align: center; font-size: 8pt">
        ...
        <p style=" margin: 0;"><span style="font-weight: bold; font-size: 8pt;">{{ @$reg->dokter_umum->nama }} ({{ @$reg->poli->nama }})</span></p>
