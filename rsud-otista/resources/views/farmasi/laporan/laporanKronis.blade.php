...
@section('content')
    <div class="box box-primary">
        ...
        <h4>Laporan Pemakaian Obat Kronis</h4>
        </div>
        <div class="box-body">
            <form action="{{ url('farmasi/laporan-kronis') }}" class="form-horizontal" role="form" method="POST">
                {{ csrf_field() }}
                <div class="row">
                <div class="col-sm-6">
    ...
    <div class="box box-success">
        ...
        <div class="box-body">
            <div class="table-responsive">
                <br>
                <h3 class="box-title">Detail List Pemakaian Obat Kronis</h3>
                    ...
                    <tbody>
                        @if (!empty($pemakaian))
                            ...
                            @php
                                  ...
                                  $totalJual = 0;
                                  $totalJual = @$d->jumlah * $d->hargajual;
                            @endphp
                            ...
                            <tr>
                                ...
                                <td>{{ optional($reg)->pasien_id ? baca_pasien($reg->pasien_id) : '-' }}</td>
                                <td>{{ optional(optional($reg)->pasien)->no_rm ?? '-' }}</td>
                                <td>{{ optional($reg)->no_sep ?? '-' }}</td>
                                <td>{{ optional(optional($reg)->pasien)->alamat ?? '-' }}</td>
                                ...
                                <td>{{ number_format(@$d->hargajual) }}</td>
                                <td>{{ number_format(@$totalJual) }}</td>
                                @php
                                    $ruang = $reg ? App\Rawatinap::where('registrasi_id', $reg->id)->first() : null;
                                @endphp
                                @if (isset($_POST['jenis']) && $_POST['jenis'] == 'TI')
                                    <td>{{ $ruang ? baca_kelompok($ruang->kelompokkelas_id) : '-' }}</td>
                                @else
                                    <td>{{ $reg ? baca_poli($reg->poli_id) : '-' }}</td>
                                @endif
                              
                                <td>{{ baca_carabayar(@$d->cara_bayar_id )}}</td>
                                <td>{{ optional($reg)->tgl_sep ?? '-' }}</td>
