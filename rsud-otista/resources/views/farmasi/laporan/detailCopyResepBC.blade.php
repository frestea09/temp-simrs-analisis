...
.table-hover>tbody>tr:hover {
  ...
  <table border=0 style="width: 100%;"> 
    <tr>
      {{-- <td style="width:10%;">
        <img src="http://simrs.merantikab.go.id/images/1668124444logomeranti-removebg-preview.png"style="width: 40px;">
      </td> --}}
      <td style="text-align: center">
        <b style="font-size:9px;">BLUD UPT RSUD OTO ISKANDAR DI NATA</b><br/>
        <b style="font-size:9px;">{{ configrs()->alamat }} Telp. {{ configrs()->tlp }}</b><br><br>
        <b style="font-size:9px;">APOTIK</b>
      </td>
    </tr>
  </table>
  ...
  <table style="width: 100%">
    <tr style="line-height:0.75;">
      <td style="width: 50%; vertical-align: top;">
        {{-- <b>{{ config('app.merek') }}</b> <br /> --}}
          ...
          {{-- Website:  --}}
          <table style="width: 100%;">
            <tr> <td style="width: 30%">Nama </td> <td>:</td> <td>{{ $reg->pasien->nama }}</td> </tr>
            <tr> <td>No SEP</td> <td>:</td> <td>{{ $reg->no_sep }}</td> </tr>
            <tr> <td>No.RM</td> <td>:</td> <td>{{ $reg->pasien->no_rm }}</td> </tr>
            ...
            <tr> <td>Tanggal R/</td> <td>:</td> <td>{{ date('d-m-Y' ,strtotime($penjualan->created_at)) }}</td> </tr>
          </table>
      </td>
      <td style="width: 50%;">
        ...
        <table style="width: 100%; ">
          ...
          <tr> <td>Cara Bayar</td> <td>:</td> <td>{{ baca_carabayar($reg->bayar) }}</td> </tr>
          <tr> <td>Dari Dokter</td> <td>:</td> <td>{{ baca_dokter($reg->dokter_id) }}</td> </tr>
    ...
    {{-- Faktur Penjualan Apotik Rawat Jalan --}}
    <tr style="line-height:0.5;">
      @if (substr($reg->status_reg, 0,1) == 'J')
        <th colspan="2" style="text-align: center"><br> Copy Resep Apotik Rawat Jalan</th>            
      @elseif (substr($reg->status_reg, 0,1) == 'G')
        <th colspan="2" style="text-align: center"><br> Copy Resep Apotik Rawat Darurat</th>  
      @elseif (substr($reg->status_reg, 0,1) == 'I') 
        <th colspan="2" style="text-align: center"><br> Copy Resep Apotik Rawat Inap</th>
      @endif
      <br/> <br/>
    </tr>
    <tr style="line-height:0.75;text-align: center">
      <td colspan="5"><i> No. R/ : {{ $penjualan->no_resep }} </i></td>
    </tr>
  ...
  @foreach ($detail as $no => $d)
    <tr style="line-height:0.75;">
      ...
      <td>{{ !empty($d->masterobat_id) ? baca_satuan_jual_report($d->masterobat_id) :'' }}</td>
  ...
  @endforeach
    {{-- <tr style="line-height:0.75;">
      <td colspan="2"><i>Terbilang: {{ terbilang($total+$uang_racik) }} rupiah</i></td>
  ...
  <table style="width: 80%">
    <tr>
      <td>
        ** Semoga Cepat Sembuh** <br/><br/>
        Catatan: <br />
        Lembar 1: Pasien <br />
        Lembar 2: Apotik <br />
        Lembar 3: Keuangan <br/><br/>
      </td><br/><br/>
