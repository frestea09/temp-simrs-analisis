...
@section('content')
    <div class="box box-primary">
      ...
      <div class="box-body">
        {!! Form::model($masterpagu, ['route' => ['masterpagu.update', $masterpagu->id], 'method' => 'PUT', 'class' => 'form-horizontal']) !!}

          <div class="form-group{{ $errors->has('biaya') ? ' has-error' : '' }}">
            {!! Form::label('biaya', 'Biaya' ,['class' => 'col-sm-3 control-label']) !!}
            <div class="col-sm-9">
                {!! Form::number('biaya', $masterpagu->biaya, ['class' => 'form-control']) !!}
        ...
        <div class="form-group{{ $errors->has('diagnosa_awal') ? ' has-error' : '' }}">
            ...
            <div class="col-sm-9">
                {!! Form::text('diagnosa_awal', $masterpagu->diagnosa_awal, ['class' => 'form-control']) !!}
                <small class="text-danger">{{ $errors->first('diagnosa_awal') }}</small>
            </div>
        </div>

        <div class="form-group{{ $errors->has('kelas') ? ' has-error' : '' }}">
            {!! Form::label('kelas', 'Diagnosa Awal', ['class' => 'col-sm-3 control-label']) !!}
            <div class="col-sm-9">
                {{-- {!! Form::text('kelas', $masterpagu->kelas, ['class' => 'form-control']) !!} --}}
                <select class="form-control select2" style="width: 100%" name="kelas">
                  @foreach($kelas as $d)
                    <option value="{{ $d->id }}" {{$masterpagu->kelas_id == $d->id ? 'selected' : ''}}>{{ $d->nama }}</option>
                  @endforeach
                </select>
