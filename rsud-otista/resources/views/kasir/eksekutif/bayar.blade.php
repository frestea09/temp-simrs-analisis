          ...
          <div class="widget-user-header bg-aqua-active"  style="height: auto;">
            <div class="row">
              ...
              <div class="col-md-7">
                ...
                <h5 class="widget-user-desc">: {{ @$reg->bayars->carabayar }} {{ (!empty($reg->tipe_jkn) && $reg->bayar == 1) ? ' - '.$reg->tipe_jkn : '' }}</h5>
                <h5 class="widget-user-desc">: {{ @$reg->dokter_umum->nama }}</h5>
              </div>
            ...
            <table class='table table-striped table-bordered table-hover table-condensed'>
              ...
              <tbody>
                @foreach ($fol as $key => $d)
                  @php
                    // $obat = \App\Penjualandetail::where('no_resep', $d->namatarif)->sum('uang_racik');
                    $obat = 0;
                  @endphp
                  <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $d->namatarif }} </td>
                    <td>{{ !empty($d->dokter_pelaksana) ? baca_dokter($d->dokter_pelaksana) : baca_dokter($d->dokter_id) }}</td>
                    <td>{{ !empty($d->poli_id) ? $d->poli->nama : '' }}</td>
                    <td>
                      @if ($d->jenis == 'ORJ')
                        {{ number_format($d->total+$jasa_racik) }}
                      @else
                ...
                {!! Form::open(['method' => 'POST', 'route' => 'kasir.save_bayar_eksekutif', 'class' => 'form-horizontal']) !!}
