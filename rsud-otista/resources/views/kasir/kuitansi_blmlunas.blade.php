  ...
  <body onload="window.print()">
    <table>
      ...
      <div class="table-responsive">
        <table class="table table-bordered" >
          <thead>
            <tr>
              <th colspan="6"  style="font-size:17px" class="text-left"> Tindakan</th>
            </tr>
          {{-- <tr>
          ...
          <tbody>
            ...
            @if (@$d->verif_kasa_user == 'tarif_new')
              <tr>
                <td  style="font-size:17px">{{ $d->tarif_baru->nama }}</td>
                <td class="text-right"  style="font-size:17px">{{ (@$d->tarif_baru->total <> 0) ? number_format(@$d->tarif_baru->total) : NULL }}</td>
                <td class="text-center"  style="font-size:17px">{{ (@$d->tarif_baru->total <> 0) ? (@$d->total / @$d->tarif_baru->total) : NULL }}</td>
                <td class="text-left"  style="font-size:17px">{{ number_format($d->total) }}</td>
              </tr>
            @else
              <tr>
                <td  style="font-size:17px">{{ $d->tarif->nama }}</td>
                <td class="text-right"  style="font-size:17px">{{ (@$d->tarif->total <> 0) ? number_format(@$d->tarif->total) : NULL }}</td>
                <td class="text-center"  style="font-size:17px">{{ (@$d->tarif->total <> 0) ? (@$d->total / @$d->tarif->total) : NULL }}</td>
                <td class="text-left"  style="font-size:17px">{{ number_format(@$d->total) }}</td>
              </tr>
            @endif
            @endforeach
          ...
          <tfoot>
            <tr>
              <td colspan="2" class="text-right"  style="font-size:17px"><b>Total : </b></td>
              <td colspan="2" class="text-left"  style="font-size:21px"><b>{{number_format($total)}}</b></td>
            </tr>
          </tfoot>
        </table>
      </div>
    @endif
      ...
      @if ($bayar->bayar <> 1)
        <tr>
          @if ($total !== 0)
            <th colspan="2"><b style="font-size:17px">"{{ terbilang(@$total) }} Rupiah"</b></th>  
          @endif
        </tr>
      @endif
