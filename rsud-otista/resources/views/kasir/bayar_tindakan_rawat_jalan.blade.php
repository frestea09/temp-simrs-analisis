          ...
          <div class='table-responsive'>
            <table class='table table-striped table-bordered table-hover table-condensed'>
              <thead>
                ...
                <tr>
                  <th colspan="6">
                    <a href="{{ url('ranap-informasi-unit-item-rincian-biaya/' . @$reg->id) }}"class="btn btn-info btn-sm"
                      style="margin-left:20px;" target="_blank">Rincian Biaya</a>
                  </th>
                </tr>
                @endif
                <tr>
              ...
              <tbody>
                @foreach ($fol as $key => $d)
                  <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $d->namatarif }} </td>
                    <td>{{ !empty($d->dokter_pelaksana) ? $d->dokterPelaksana->nama : $d->dokter->nama }}</td>
                ...
                {!! Form::open(['method' => 'POST', 'route' => 'kasir-tindakan.save_bayar_rawat_jalan', 'class' => 'form-horizontal']) !!}
