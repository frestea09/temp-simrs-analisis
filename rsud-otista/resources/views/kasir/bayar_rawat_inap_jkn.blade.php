      ...
      <div class="row">
        ...
        <div class="box box-widget widget-user">
          ...
          <div class="widget-user-header bg-aqua-active" style="height: 200px;">
            <div class="row">
              <div class="col-md-2">
                <h3 class="widget-user-username">Namax</h3>
          ...
          <div class='table-responsive'>
            <table class='table table-striped table-bordered table-hover table-condensed'>
              ...
              <tbody>
                @foreach ($fol as $key => $d)
                  ...
                  <tr>
                    ...
                    <td>
                      @if ($d->poli_tipe == 'R')
                        @if (!empty($d->dokter_radiologi))
                          {{baca_dokter($d->dokter_radiologi)}}    
                        @endif
                      @elseif ($d->poli_tipe == 'O')
                        @if (!empty($d->dokter_bedah))
                          {{baca_dokter($d->dokter_bedah)}}    
                        @endif
                      @else
                        @if (!empty($d->dokter_pelaksana ))
              ...
              <tfoot>
                {{-- @if ($reg->bayar == 1)
                <tr>
                  <th colspan="4" class="text-right">Total INACBG</th>
                  <th style="width: 15%"> {!! Form::text('inacbg', number_format($dijamin), ['class' => 'form-control input-sm uang']) !!}</th>
                </tr>
                @endif --}}
                <tr>
                ...
                @if ($reg->bayar == 1)
                <tr>
                  <th colspan="4" class="text-right">Uang Muka / Deposit</th>
                  <th style="width: 15%"> {!! Form::text('deposit', number_format($deposit), ['class' => 'form-control']) !!}</th>
                </tr>
                @endif
                <tr>
                ...
                {{-- <tr>
                  <th colspan="4" class="text-right">Dijamin</th>
                  <th style="width: 15%"> {!! Form::text('iur', number_format($urun_bayar), ['class' => 'form-control input-sm uang', 'onkeyup'=>'totalHarusBayar()']) !!}</th>
                </tr> --}}
                @if ($reg->bayar == 1)
                  <tr>
                    <th colspan="4" class="text-right">Harus Bayar</th>
                    <th><input type="text" name="hrs_bayar" class="form-control uang input-sm"></th>
                  </tr>
                @endif
                <tr>
                ...
                <div class="btn-group pull-right">
                  @if ($reg->bayar == '2')
                    {!! Form::submit("Bayar", ['id'=>'btnSave','class' => 'btn btn-success', 'onclick'=>'javascript:return confirm("Yakin data sudah benar? Cek Sekali Lagi!")']) !!}
                  @else
                    {!! Form::submit("Tutup Transaksi", ['id'=>'btnSave','class' => 'btn btn-success', 'onclick'=>'javascript:return confirm("Yakin data sudah benar? Cek Sekali Lagi!")']) !!}
                  @endif
                </div>
            {!! Form::close() !!}
              <div class="pull-left">
                <a href="{{ URL::previous() }}" class="btn btn-info"><i class="fa fa-step-backward"></i> Halaman Sebelumnya</a>
                <button type="button" class="btn btn-danger" onclick="tambahTindakan('{{ $reg->id }}')">TAMBAH TINDAKAN</button>
              </div>
                ...
                <tbody>
                  <form method="POST" id="bayarPiutang">
                    ...
                    <tr>
                      ...
                      <td class="tanggal"></td>
                      <td>
                        <input type="text" name="total" class="form-control uang" style=" widht: ">
                      </td>
                      <td>
