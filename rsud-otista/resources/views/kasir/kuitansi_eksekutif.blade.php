...
<html>
  <head>
    <meta charset="utf-8">
    <title>Cetak Kwitansi Eksekutif</title>
    <style>
      body{
        margin: 5px 10px;
        font-size: 8pt;
    ...
    <tr style="line-height:0.75; font-size: 20px;">
      <td class="text-right">
        <table style="width: 100%;">
         ...
         <tr>
          <td colspan="2">
            ...
            <table style="width:100%" class="table table-bordered table-striped">
              ...
              @endforeach
              @foreach ($folio_igd_lab as $item => $d)
                <tr>
                  @if(date('Y-m-d H:i', strtotime($d->created_at)) < config('app.tarif_new'))
                      <td>{{ baca_tarif($d->tarif_id) }},</td>
                      <td class="text-right">{{ ($d->tarif_lama->total <> 0) ? number_format($d->tarif_lama->total) : NULL }}</td>
                  ...
                  @endif
                  @php
                        $total_igd_lab += $d->total;
                  @endphp
                </tr>
              @endforeach 
              @foreach ($folio_igd_rad as $item => $d)
                <tr>
                  @if(date('Y-m-d H:i', strtotime($d->created_at)) < config('app.tarif_new'))
                      <td>{{ baca_tarif($d->tarif_id) }},</td>
                      <td class="text-right">{{ ($d->tarif_lama->total <> 0) ? number_format($d->tarif_lama->total) : NULL }}</td>
              ...
              <tr>
                <td>Uang Racik</td>
                <td colspan="2"> </td>
                <td>{{number_format($uang_racik)}}</td>
              </tr>
              @endif
            </table>
          </td>
         ...
         <tr style="font-size: 20px;"> 
          <td style="width:15%;"></td>
          <td><b>Rp.{{ number_format($total_fol+$total_igd_rad+$total_igd_lab+$obat) }}</b></td> 
          {{-- <td><b>Rp.{{ number_format($jml_igd+$obat+$jml_igd_rad+$jml_igd_lab+$jml) }}</b></td>  --}}
        </tr>
         <tr style="font-size: 20px;"> 
             <td style="width:15%">Terbilang </td>
             {{-- <td>: <b>{{ terbilang($jml_igd+$obat+$jml_igd_rad+$jml_igd_lab+$jml)}} Rupiah</b></td>  --}}
             <td>: <b>{{ terbilang($total_fol+$total_igd_rad+$total_igd_lab+$obat)}} Rupiah</b></td> 
          </tr>
        </table>
      </td>
    </tr>
