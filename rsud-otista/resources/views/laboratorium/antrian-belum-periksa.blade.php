<div class='table-responsive'>
    <table class='table-striped table-bordered table-hover table-condensed table' >
        ...
        <tbody>
            ...
            @endphp
                <tr>
                    <td class="text-center">
                        <button onclick="markAsDone({{$d->id}})" type="button" class="btn btn-flat btn-success btn-sm"><i class="fa fa-check"></i></button>
                    </td>
                    <td>
                        @if(@$antrianLab)
            ...
            @empty
                <tr>
                    <td colspan="13" class="text-center">Tidak Ada Pasien Yang Belum Diperiksa</td>
                </tr>
            @endforelse
    ...
    @if ($datas->count() > 0)
        <script>
            new Audio('/audio/notif3.mp3').play();
        </script>
    @endif
    <script>
            function panggilAntrian(nomor, idAntrian, regId){
                $.ajax({
                    url: '/laboratorium/panggil-antrian/'+nomor+'/'+idAntrian+'/'+regId,
                    type: 'GET',
                })
                .done(function(res) {
                    if(res.code == 200){
