...
@endphp
  <div class="box box-primary">
    <div class="box-header with-border">
      <h3 class="box-title">
           <a href="{{ route('mastermapping_confrl310.index') }}" class="btn btn-default btn-sm btn-flat"> <i class="fa fa-fast-backward"></i> Mapping Config RL.310</a>
      </h3>
    </div>
    <div class="box-body">
        <div class="row">
                        ...
                        <div class="col-md-8">
                            <select class="form-control" name="mastermappingid" readonly="true">
                                @foreach ($data_mapping as $key => $d)                                   
                                        <option value="{{ $d->id_conf_rl310 }}" selected>{{ $d->kegiatan }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
    ...
    $('select[name="jenis"]').on('change', function(e) {
        ...
        if( $('select[name="jenis"]').val() == 'TI') {
          ...
          $('select[name="kategoritarif_id"]').on('change', function(e) {
            ...
            $('#mapping').load('/data-tarif-mastermapping_confrl310/'+tahuntarif_id+'/'+ $('select[name="jenis"]').val() +'/'+ $('select[name="kategoritarif_id"]').val())
          })
        } else {
          $('#katTarif').addClass('hidden')
          $('#mapping').load('/data-tarif-mastermapping_confrl310/'+tahuntarif_id+'/'+$(this).val())
        }
