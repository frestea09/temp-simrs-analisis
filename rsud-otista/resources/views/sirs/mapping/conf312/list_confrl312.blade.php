@extends('master')
@section('header')
<h1>Master Mapping CONF.RL312</h1>
@endsection
...
<div class="box box-primary">
    <div class="box-header with-border">
        ...
        <!-- <a href="{{route('table_conf_rl312')}}" class="btn btn-default btn-flat">
            <i class="fa fa-plus"></i> Rekap Laporan Mapping CONF.RL312 
        </a> -->
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class='table-responsive'>
                                <table id='table_conf_rl312' class='table table-striped table-bordered table-hover table-condensed'>
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Kegiatan</th>
...
<script type="text/javascript">
    $(document).ready(function(e) {
        var table = $('#viewMapping').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route("data-mastermapping_confrl312") }}',
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
                    orderable: false
    ...
    function editForm(id) {
        ...
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                $('input[name="id"]').val(data.id_conf_rl312)
    ...
    function saveForm() {
        ...
        if(id == '') {
            url = '{{ route("mastermapping_confrl312.store") }}';
            type = 'POST';
        } else {
    ...
    function detailconfrl312(id) {
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function (data) {
                $('.modal-title').text('Conf.RL312  '+data.kegiatan)
            }
        });


        $('#table_conf_rl312').DataTable({
          'language': {
          ...
          processing  : true,
          serverSide  : true,
          ajax: '/list-detail-conf-rl312/'+id,
          columns: [
    ...
    function hapusMapping(conf_rl312_id, tarif_id) {
      if (confirm('Yakin akan dihapus') == true) {
        $.ajax({
          url: '/hapus-conf-rl312/'+tarif_id,
          type: 'GET',
          dataType: 'json',
        })
        .done(function(data) {
            detailconfrl312(conf_rl312_id)
        });
