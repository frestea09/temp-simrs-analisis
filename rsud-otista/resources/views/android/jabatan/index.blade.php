            ...
            <div class="modal-footer" style="margin:0px; border-top:0px; text-align:center;">
                ...
                <form method="POST" action="" id="form-update" style="display:none;">
                    ...
                    <div style="display: grid;margin-bottom: 30px;">
                        <div class="form-group{{ $errors->has('jab_nama') ? ' has-error' : '' }}">
                            {!! Form::label('Nama Jabatan', 'Nama Jabatan', ['class' => 'col-sm-3 control-label']) !!}
                            <div class="col-sm-9">
                                {!! Form::text('jab_nama', null , ['class' => 'form-control']) !!}
                                <small class="text-danger">{{ $errors->first('jab_nama') }}</small>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function(){
        $(document).on('click','.edit-data', function(){
            ...
            let url = window.location.origin+'/android/jabatan/'+id;
            let source = eval($(this).attr('data-source'));
            jQuery('#modal-data').modal('show', {
                backdrop: 'static'
            });
            $('input[name="id"]').val(id);
            $('input[name="jab_nama"]').val(source[0].jab_nama);
        ...
        $(document).on('click','.add-data', function(){
            let id = $(this).attr('data-id');
            let url = window.location.origin+'/android/jabatan';
            let source = eval($(this).attr('data-source'));
            jQuery('#modal-data').modal('show', {
                backdrop: 'static'
            });
