@extends('master')
@section('header')
  <h1>Logistik Non Medik - Master Golongan</h1>
@endsection
...
@section('script')
  <script type="text/javascript">
    function tambah(){
      $('#Modal').modal('show')
      $('.modal-title').text('Tambah Golongan')
      $('input[name="id"]').val('')
    ...
    function edit(id){
      ...
      $.get('/logistiknonmedik/master-golongan/'+id+'/edit', function(resp){
          $('input[name="nama"]').val(resp.golongan.nama)
          $('input[name="kode"]').val(resp.golongan.kode)
      })
    }
    ...
    function save(){
        ...
        if(id == ''){
            var url = '{{ route('master-golongan.store') }}'
        } else {
            var url = '/logistiknonmedik/master-golongan/'+id
        }
    ...
    processing: true,
    serverSide: true,
    ajax: '{{ route('master-golongan.index') }}',
    columns: [
