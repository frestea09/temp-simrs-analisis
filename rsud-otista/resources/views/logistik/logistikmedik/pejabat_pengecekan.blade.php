@extends('master')
@section('header')
  <h1>Logistik Medik - Pejabat Pemeriksa<small></small></h1>
@endsection
...
@section('script')
  <script>
    ...
    function addPejabat() {
      ...
      $('.modal-title').text('Tambah Pejabat Pemeriksa')
      $('#formPejabat')[0].reset();
    }
    function editPejabat(id){
      status = 1
      $('#addPejabat').modal('show')
      $('.modal-title').text('Edit Pejabat Pemeriksa')
      $.get('getPejabatpengecekan/'+id, function(resp){
    ...
    function simpanPejabat(){
      ...
      if(status == 0){
        url = '/logistikmedik/pejabatpengecekanCreate'
      }else{
        url = '/logistikmedik/pejabatpengecekanUpdate'
      }
      $.ajax({
        url: url,
        type: 'POST',
