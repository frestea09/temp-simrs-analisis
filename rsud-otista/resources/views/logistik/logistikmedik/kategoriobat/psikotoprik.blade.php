...
@section('header')
  <h1>
    <small>Kategori Obat</small> Psikotoprik
...
<script type="text/javascript">
  ...
  var masterobat;
    masterobat = $('#dataMasterObat').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax: '/data-master-obat/psikotoprik',
        columns: [
            {data: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'nama', orderable: false},
            {data: 'kode', orderable: false},
  ...
  function masterObat() {
    $('#dataObat').modal('show')
    $('.modal-title').text('Tambahkan Obat Psikotoprik')
    $('input[name="kategoriobat"]').val('psikotoprik');
  }
