...
@section('header')
  <h1>
    <small>Kategori Obat</small> High Alert
...
<script type="text/javascript">
  ...
  var masterobat;
    masterobat = $('#dataMasterObat').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax: '/data-master-obat/high_alert',
        columns: [
            {data: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'nama', orderable: false},
            {data: 'kode', orderable: false},
  ...
  function masterObat() {
    $('#dataObat').modal('show')
    $('.modal-title').text('Tambahkan Obat High Alert')
    $('input[name="kategoriobat"]').val('high_alert');
  }
