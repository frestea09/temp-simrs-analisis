...
@section('header')
  <h1>
    <small>Kategori Obat</small> Formularium
...
<script type="text/javascript">
  ...
  var masterobat;
    masterobat = $('#dataMasterObat').DataTable({
        ...
        serverSide: true,
        ordering: false,
        ajax: '/data-master-obat/formularium',
        columns: [
            {data: 'DT_RowIndex', orderable: false, searchable: false},
            {data: 'nama', orderable: false},
            {data: 'kode', orderable: false},
  ...
  function masterObat() {
    $('#dataObat').modal('show')
    $('.modal-title').text('Tambahkan Obat Formularium')
    $('input[name="kategoriobat"]').val('formularium');
  }
