@extends('master')
@section('header')
  <h1>Logistik Medik - Pejabat Bendahara<small></small></h1>
@endsection
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form id="formPejabat" method="post" class="form-horizontal">
              ...
              <div class="form-group skGroup">
                <label for="sk" class="col-sm-3 control-label">SK</label>
                <div class="col-sm-9">
                  <input type="text" name="sk" class="form-control" placeholder="Masukkan sk !" required>
                  <small class="text-danger skError"></small>
                </div>
              </div>
...
@section('script')
  <script>
    ...
    function addPejabat() {
      ...
      $('.modal-title').text('Tambah Pejabat Bendahara')
      $('#formPejabat')[0].reset();
    }
    function editPejabat(id){
      status = 1
      $('#addPejabat').modal('show')
      $('.modal-title').text('Edit Pejabat Bendahara')
      $.get('getPejabatbendahara/'+id, function(resp){
        $("input[name='id']").val(resp.id)
        $("input[name='nip']").val(resp.nip)
        $("input[name='sk']").val(resp.sk)
    ...
    function simpanPejabat(){
      ...
      if(status == 0){
        url = '/logistikmedik/pejabatbendaharaCreate'
      }else{
        url = '/logistikmedik/pejabatbendaharaUpdate'
      }
      $.ajax({
        url: url,
        type: 'POST',
