...
@section('header')
  <h1>Protesa Gigi - Rawat Darurat </h1>
@endsection
...
@section('content')
    <div class="box box-primary">
      ...
      <div class="box-body">
        {!! Form::open(['method' => 'POST', 'url' => 'protesa-gigi/tindakan-darurat', 'class'=>'form-hosizontal']) !!}
        <div class="row">
          <div class="col-md-6">
            ...
            <tbody>
              @foreach ($reg as $key => $d)
                  ...
                  {{-- @role(['protesagigi', 'administrator']) --}}
                    <a href="{{ url('protesa-gigi/insert-tindakan/darurat/'. $d->id.'/'.$d->pasien_id) }}" onclick="return confirm('APAKAH YAKIN AKAN DILAKUKAN TINDAKAN INI ?')" class="btn btn-sm btn-info btn-flat">
                      <i class="fa fa-refresh"></i>
                    </a>
                  {{-- @endrole --}}
                  </td>
                  <td class="text-center">
                    @if (Modules\Registrasi\Entities\Folio::where('registrasi_id', $d->id)->where('jenis','TG')->where('poli_tipe', 'P')->count() > 0)
                      <a href="{{ url('protesa-gigi/cetak-tindakan/TG/'.$d->id) }}" target="_blank" class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print"></i></a>
                    @endif
                  </td>
                </tr>
              @endforeach
