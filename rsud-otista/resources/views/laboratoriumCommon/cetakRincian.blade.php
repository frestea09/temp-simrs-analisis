...
<html>
    ...
    <td>
      ...
      <td style="width: 50%">
        ...
        <table style="width: 100%">
          ...
          <tr> <td>Unit yang Order</td> <td>:</td><td>{{ $unit }} / Laboratorium PA</td></tr>
  ...
  <table class="table table-condensed" style="width: 100%">
    <tr>
      ...
      <th style="text-align: center; width: 10%">No Sediaan</th>
      <th style="text-align: center; width: 10%">Ket. Klinis</th>
    ...
    @foreach ($tindakan as $d)
      ...
      @endphp
        <tr>
          ...
          <td>{{ $d->namatarif }}</td>
          <td style="text-align: center">{{ @$d->no_sediaan }}</td>
          <td style="text-align: center">{{ @$d->diagnosa }}</td>
          <td style="text-align: center">{{ baca_carabayar($reg->bayar) }}</td>
          @if (@$d->verif_kasa_user == 'tarif_new')
          <td style="text-align: center">{{ ($d->tarif_baru->total <> 0) ? floor(($d->total + $d->diskon)) / $d->tarif_baru->total : NULL }}</td>
          <td style="text-align: right">{{ ($d->tarif_baru->total <> 0) ? number_format($d->tarif_baru->total) : number_format($det) }}</td>
          <td style="text-align: right">{{ ($d->total <> 0) ? number_format($d->total - $d->dijamin) : number_format($det) }}</td>
          @else
          <td style="text-align: center;">{{ ($d->tarif->total <> 0) ? floor(($d->total + $d->diskon) / $d->tarif->total) : NULL }}</td>
  ...
  <table style="width: 60%; float: right;">
      ...
      <td style="text-align: center">
        ...
        ( {{ !empty($folio->dokter_pelaksana) ? baca_dokter($folio->dokter_pelaksana) : '' }} )
      </td>
    </tr>
  </table>
