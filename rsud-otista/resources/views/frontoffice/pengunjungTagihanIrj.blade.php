...
@section('header')
  <h1>Laporan Tagihan Rawat Jalan</h1>
@endsection
...
@endphp
	<div class="box box-primary">
		...
		<div class="box-body">
		{!! Form::open(['method' => 'POST', 'url' => 'frontoffice/laporan/pengunjung-tagihan-irj', 'class'=>'form-horizontal']) !!}
			{{ csrf_field() }}
			<div class="row">
				<div class="col-md-7">
			...
			<table class='table table-bordered table-hover' style="font-size:11px;">
				<thead>
					  <tr>
						...
						<th class="v-middle text-center" style="color:red">Dokter Lab 3 (dr. Hapsari)</th>
						...
						@if (count($rajal) > 0)
							...
							<tr>
								...
								<td>
									...
									@php
										$drjenny = \Modules\Registrasi\Entities\Folio::where('registrasi_id',$d->registrasi_id)->where('dokter_lab',17)->sum('total');
								...
								<td>
									...
									@php
										$drandy = \Modules\Registrasi\Entities\Folio::where('registrasi_id',$d->registrasi_id)->where('dokter_lab',851)->sum('total');
										// ->where(function ($query){
								...
								<td>
									{{-- <i>Belum terkalkulasi</i> --}}
									{{-- Dokter Lab 3 (dr. Hapsari) --}}
									@php
										$drhapsari = \Modules\Registrasi\Entities\Folio::where('registrasi_id',$d->registrasi_id)->where('dokter_lab',868)->sum('total');
										// ->where(function ($query){
										// 	$query->where('dokter_lab', 868);
										// 		// ->orWhere('dpjp', 868);
										// })
										// ->sum('total');
									@endphp
								...
								<td>
									{{$retr_umum+$tojrs+$tm+$retr_spesialis+$drjenny+$drandy+$drhapsari+$retr_kons+$resume+$yuke+$usgtauf+$indrarini+$usgindrarini+$tauf+$nine+$ekganak+$ekgjantung+$ekgdalam+$uso+$paket_alkes+$retr_obs+$tmo+$tpo+$rehab+$ctg+$obat_gudang_operasi+$obatrajal}}
								</td>
							</tr>
							@endforeach
						@endif
