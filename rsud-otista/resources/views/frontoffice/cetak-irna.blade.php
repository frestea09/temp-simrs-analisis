@extends('master')
@section('header')
  <h1>Cetak Label Rekam Medis INAP<small></small></h1>
@endsection
...
@section('content')
  <div class="box box-primary">
    ...
    <div class="box-body">
      {!! Form::open(['method' => 'POST', 'url' => 'frontoffice/cetak-irna', 'class'=>'form-hosizontal']) !!}
      <div class="row">
        <div class="col-md-6">
        ...
        <table class='table table-striped table-bordered table-hover table-condensed' id='data'>
          <thead>
            <tr class="text-center">
              ...
              {{--<th class="text-center">Antrian Nomor Poli</th>--}}
            ...
            @foreach ($today as $key => $d)
              @if (!empty($d->pasien_id))
                <tr class="text-center">
                  ...
                  <td> 
                    <button type="button" class="btn btn-primary btn-sm btn-flat" onclick="showGeneralConsentIrna({{$d->id}})"><i class="fa fa-pencil text-center"></i> </button>
                    ...
                    {{-- @if($d->general_consent != null) --}}
                        <a target="_blank" href="/frontoffice/cetak-general-consent-irna/{{$d->id}}" class="btn btn-warning btn-sm btn-flat" ><i class="fa fa-print text-center"></i> </a> 
                    {{-- @endif --}}
                  </td>
                  <td>
                  ...
                  <td>
                    <a href="{{ url('tindakan/cetak-sbpk/'.$d->id) }}" target="_blank"  class="btn btn-warning btn-sm btn-flat"><i class="fa fa-print text-center"></i></a>
                  </td>
                  <td>
                    @if (!empty($d->no_sep))
                  ...
                  @php
                      @$spri = @\App\SuratInap::where('registrasi_id', $d->id)->first();
                  @endphp
                  <td>
                    @if (@$spri)
                      <a href="{{ url('spri/cetak/'.$d->id) }}" target="_blank" class="btn btn-sm btn-info btn-flat"><i class="fa fa-print"></i></a> 
                    @endif
                  </td>
                </tr>
              @endif
...
@section('script')
    ...
    <script>
        function showGeneralConsentIrna(registrasi_id) {
            $('#modalGeneralConsent').modal('show')
            $('#modalGeneralConsent form').attr('action', '/frontoffice/general-consent-irna/' + registrasi_id)
            $('#bodyGeneralConsent').load('/frontoffice/general-consent-irna/' + registrasi_id);
        }
    </script>
@endsection
