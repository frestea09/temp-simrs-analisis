          ...
          <tr>
            ...
            <td>
              ...
              <div class="box" style="font-size:12px !important">
                  ...
                  <div> <b>{{ $pasien->no_rm }}</b> ({{ $pasien->kelamin }}) {{ baca_carabayar(@$registrasi->bayar) }}
                    {{-- {{ baca_carabayar(@$registrasi->bayar) }} {{@$registrasi->tipe_jkn? @$registrasi->tipe_jkn : ''}} --}}
                  ...
                  <div>
                    @if (@$ruangan != null)
                          ({{ baca_kamar(@$ruangan->kamar_id) }})  ({{ baca_poli(@$registrasi->poli_id) }})
                    @endif
                    </div>
                  <div>{{ hitung_umur($pasien->tgllahir) }} {{ $pasien->tgllahir }}</div>
                  {{-- @if (@$histori)
                  <div>{{ @date('d-m-Y, H:i',strtotime(@$histori->tgl_masuk))}}</div>
                  @else --}}
                  <div>{{ @date('d-m-Y, H:i',strtotime(@$ruangan->tgl_masuk))}}</div>
                  {{-- @endif --}}
