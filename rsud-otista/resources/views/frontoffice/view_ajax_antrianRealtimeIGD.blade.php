  ...
  <table class='table table-striped table-bordered table-hover table-condensed' id='' style="font-size:12px;">
    ...
    <tbody>
      @foreach ($registrasi as $key => $r)
        @php
          $status_reg = cek_status_reg($r->registrasi->status_reg);
        @endphp
        <tr>
          ...
          <td class="text-center" style="font-size:11px;">{{$r->tgl_pulang}}</td>
          <td>
          @php
            $rawatinap = $r->registrasi->rawat_inap;
          @endphp
              {{-- Rawat Inap --}}
            @if ($status_reg == 'I')
            ...
            @elseif ($status_reg == 'J')
              {{-- Rawat Jalan --}}
              Poli {{ $r->registrasi->poli->nama }}
            @elseif ($status_reg == 'G')
              IGD
            @endif
          </td>
          <td>{{ $r->registrasi->poli->nama }}</td>
          <td>
            @if (!empty($rawatinap))
              {{ baca_kamar($rawatinap->kamar_id) }}
            @else
              -
            @endif
          </td>
          ...
          <td class="text-center">
            @if ($status_reg == "J")
              <a href="{{ url('/ranap-informasi-unit-item-rincian-biaya-tanpa-kronis/'.$r->registrasi->id) }}" target="_blank"  class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @else
              <a href="{{ url('/ranap-informasi-unit-item-rincian-biaya-tanpa-rajal/'.$r->registrasi->id) }}" target="_blank"  class="btn btn-danger btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @endif
          </td>
          <td class="text-center">
            @if (!empty($r->registrasi->no_sep))
            <a href="{{ url('cetak-sep/'.$r->registrasi->no_sep) }}" target="_blank"  class="btn btn-info btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @else
            -
            @endif
          </td>
          <td class="text-center">
            @if (!empty(json_decode(@$r->registrasi->tte_resume_pasien)->base64_signed_file))
              <a href="{{ url('cetak-tte-eresume-pasien/pdf/' . @$r->registrasi->id) }}"
                target="_blank" class="btn btn-success btn-sm btn-flat"> <i
                  class="fa fa-print"></i> </a>
            @else
            <a href="{{ url('cetak-eresume-pasien/pdf/'.$r->registrasi->id) }}" target="_blank"  class="btn btn-warning btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @endif
          </td>
          <td class="text-center">
            <button type="button" class="btn btn-sm btn-primary btn-flat" onclick="popupWindow('/penjualan/tab-resep/'+{{$r->registrasi->id}})" style="margin-bottom: 5px;"><i class="fa fa-list"></i></button>
          </td>
          <td class="text-center">
            @if (!empty(json_decode(@$r->registrasi->tte_hasillab_lis)->base64_signed_file))
              <a href="{{ url('pemeriksaanlab/cetakAll-lis-tte/' . @$r->registrasi->id) }}"
                target="_blank" class="btn btn-success btn-sm btn-flat"> <i
                  class="fa fa-print"></i> </a>
            @else
            <a href="{{ url('pemeriksaanlab/cetakAll-lis/'.$r->registrasi->id) }}" target="_blank"  class="btn btn-warning btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @endif
          </td>
          <td class="text-center">
            @if(@$r->registrasi->hasilLab_patalogi)
              <a href="{{ url('pemeriksaanlabCommon/cetakAll/'.$r->registrasi->id) }}" target="_blank"  class="btn btn-warning btn-sm btn-flat"><i class="fa fa-print text-center"></i> </a>
            @endif
          </td>
          <td class="text-center">
          ...
          <td class="text-center">
            <button type="button" class="btn btn-sm btn-success btn-flat" onclick="popupWindow('/frontoffice/tab-radiologi/'+{{$r->registrasi->id}})" style="margin-bottom: 5px;"><i class="fa fa-list"></i></button>
          </td>
          <td class="text-center">
          ...
          <td class="text-center">
            @if ($r->registrasi->rujukan_rumah_sakit)
              <a href="{{ url('emr-soap-print-surat-rujukan-rumahsakit/'. @$r->registrasi_id . '/'. @$r->registrasi->rujukan_rumah_sakit->id) }}" target="_blank" class="btn btn-sm btn-danger btn-flat" style="margin-bottom: 5px;"><i class="fa fa-download"></i></a>
            @endif
          </td>
          {{-- <td class="text-center">
          ...
          <td class="text-center">
            ...
            @endif
          <td class="text-center">
            @if ($r->registrasi->paps)
              <a href="{{ url('emr-soap-print-surat-paps/igd/'. @$r->registrasi_id . '/'. @$r->registrasi->paps->id) }}" target="_blank" class="btn btn-sm btn-danger btn-flat" style="margin-bottom: 5px;"><i class="fa fa-download"></i></a>
            @endif
          </td>
          <td class="text-center">
            <a href="{{ url('frontoffice/jkn-input-diagnosa-igd/'.@$r->registrasi->id) }}" target="_blank" class="btn btn-info btn-sm btn-flat"><i class="fa fa-tint"></i></a>
          </td>
          <td class="text-center">
              <a href="{{ url('cetak-e-claim/'.@$r->registrasi->no_sep) }}" target="_blank" class="btn btn-sm btn-danger btn-flat" style="margin-bottom: 5px;"><i class="fa fa-download"></i></a>
          </td>
          <td>
    ...
    @if ($registrasi instanceof \Illuminate\Contracts\Pagination\Paginator ||
      $registrasi instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator)
      <div class="pull-right">
          {!! $registrasi->appends(request()->except('page'))->render() !!}
      </div>
...
<script type="text/javascript">
  ...
  (function () {
    const pagingFlag = {!! isset($dt_paging) && $dt_paging ? 'true' : 'false' !!};
