...
<head>
  ...
  <title>LAPORAN TINDAKAN IRJ</title>
            ...
            <table class="table table-condensed table-bordered">
              <tbody>
                ...
                <tr>
                  ...
                  <td class="tgl">{{@$reg->created_at->format('d-m-Y H:i:s') }}</td>
                </tr>
                <tr>
                  <th>Nama Pasien</th>
              ...
              <tbody>
                ...
                @if (count($folio) > 0)
                  ...
                  <tr>
                    <td>{{$key+1}}</td>
                    <td>
                      + {{ @$f->namatarif }} / {{ @$f->total }} - {!!@$f->lunas == 'Y' ? '<span style="color:green">Dibayar</span>' : '<span style="color:red">Belum Dibayar</span>'!!}<br>  
                    </td>
                  </tr>
                  @endforeach
                @else
                <tr>
                  <td colspan="2"><i>Maaf Belum Ada Laporan Tindakan Rawat Jalan</i></td>
                </tr>
                @endif
