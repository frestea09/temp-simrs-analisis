...
@section('header')
  <h1>Bridging INACBG E-Klaim Rawat Jalan <small></small></h1>
@endsection
...
@section('content')
  <div class="box box-primary">
    ...
    <div class="box-body">
      ...
      @else
        {!! Form::open(['method' => 'POST', 'url' => 'newclaim', 'class' => 'form-horizontal', 'id'=>'formINACBG']) !!}
      @endif
          ...
          <div class="row">
            <div class="col-md-6">
              ...
              <div class="form-group{{ $errors->has('nama_dokter') ? ' has-error' : '' }}">
                  ...
                  <div class="col-sm-9">
                      {!! Form::select('nama_dokter', Modules\Pegawai\Entities\Pegawai::where('kategori_pegawai', 1)->pluck('nama','nama'), baca_dokter($reg->dokter_id), ['class' => 'form-control select2', 'style' => 'width: 100%']) !!}
              ...
              <div class="form-group" id="groupDiagnosa">
                  {!! Form::label('diagnosa', 'Diagnosa ICD10', ['class' => 'col-sm-3 control-label']) !!}
                  <div class="col-sm-7">
                      {!! Form::text('diagnosa', $diagnosa, ['class' => 'form-control']) !!}
                      <small class="text-danger" id="diagnosa-error"></small>
                  </div>
                  <div class="col-sm-2">
                      <button type="button" id="openICD10" class="btn btn-default btn-flat">ICD10</button>
                  </div>
              </div>
              <div class="form-group" id="groupDiagnosa">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group" id="groupKelas">
                  ...
                  <div class="col-sm-9">
                      {!! Form::select('kelas_rawat', [1=>'Eksekutif', 3=>'Reguler'], 3, ['class' => 'form-control select2', 'style' => 'width: 100%']) !!}
              ...
              <div class="form-group" id="groupProcedure">
                  {!! Form::label('procedure', 'Procedure ICD9', ['class' => 'col-sm-3 control-label']) !!}
                  <div class="col-sm-7">
          ...
          <div class="btn-group pull-left">
              ...
              <button type="button" class="btn btn-success btn-flat" onclick="viewRad({{ $reg->id }})">RAD</button>
              
          </div>

          <div class="btn-group pull-right">
              <a href="{{ url('frontoffice/e-claim/dataRawatJalan') }}" class="btn btn-default btn-flat">Batal</a>
              <button type="button" class="btn btn-warning btn-flat" onclick="viewTindakan({{ $reg->id }})">TINDAKAN</button>
...
<div class="modal fade" id="suksesReturn" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
          <div class="pull-left">
            <a href="{{ url('eklaim-detail-bridging/'.$reg->id) }}" target="_blank" class="btn btn-danger btn-flat"><i class="fa fa-print"></i> CETAK</a>
            <button class="btn btn-success btn flat" onclick="kirimDC({{ $reg->id }})"><i class="fa fa-paper-plane-o"></i> KIRIM DC</button>
          </div>
          <div class="btn-group btn-group-sm">
            <button type="button" class="btn btn-default btn-flat" id="closeModal" data-dismiss="modal">Tutup</button>
          </div>
      </div>
    </div>
  </div>
...
@section('script')
  <script type="text/javascript">
    ...
    function viewSpri(registrasi_id) {
      ...
      .done(function(json) {
        ...
        $.each(json.data, function(index, val) {
          ...
          $('#viewDetailSpri').append('<tr> <td>'+(index+1)+'</td> <td class="text-center">'+nospri+'</td> <td class="text-center">'+val.tgl_rencana_kontrol+'</td> <td>'+val.diagnosa+'</td><td>'+val.dokter_rawat+'</td><td>'+val.dokter_pengirim+'</td><td>'+val.kamar+'</td><td>'+val.cara_bayar+'</td> </tr>')
        }); 
      });
    ...
    function viewLab(registrasi_id) {
      ...
      .done(function(json) {
        ...
        $('#viewDetailLab').empty()
        if(json.data == 0){
          return $('#viewDetailLab').append('<tr><td colspan="7" class="text-center">Tidak ada Hasil Lab</td></tr>')
        }
        $.each(json.data, function(index, val) { 
          $('#viewDetailLab').append('<tr> <td>'+(index+1)+'</td> <td class="">'+val.pemeriksaan+'</td><td class="">'+val.user+'</td> <td>'+val.tgl+'</td><td><a target="_blank" href="/pemeriksaanlabCommon/cetak/'+val.url+'">Lihat</a></td></tr>')
        }); 
      });
    ...
    function viewRad(registrasi_id) {
      ...
      .done(function(json) {
        ...
        $('#viewDetailRad').empty()
        if(json.data == 0){
          return $('#viewDetailRad').append('<tr><td colspan="7" class="text-center">Tidak ada Hasil Radiologi</td></tr>')
        }
        $.each(json.data, function(index, val) { 
          $('#viewDetailRad').append('<tr> <td>'+(index+1)+'</td> <td class="">'+val.pemeriksaan+'</td> <td class="">'+val.user+'</td> <td>'+val.tgl+'</td><td><a target="_blank" href="/radiologi/cetak-ekpertise-eklaim/'+val.id+'/'+val.registrasi_id+'">Lihat</a></td></tr>')
        }); 
      });
  ...
  // Otomatis redirect ke halaman antrian realtime (Casemix)
  $('#closeModal').click(function () {
    window.location.href = "/frontoffice/antrian-realtime";
  })
    ...
    $('#startBRIDGING').on('click', function(){
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if(data.sukses == 1) {
                    $('#suksesReturn').modal({backdrop: 'static', keyboard: false});
