...
<div class="box box-primary">
  ...
  <div class="box-body">
    ...
    <form method="POST" action="{{ url('emr-soap/perencanaan/catatanKhusus/'.$unit.'/'.$reg->id) }}" class="form-horizontal">
      <div class="row">
        <div class="col-md-12">
            ...
            <table style="width: 100%" class="table table-striped table-bordered table-hover table-condensed form-box" style="font-size:12px;"> 
              ...
              <tr>
                  <td style="padding: 5px;" colspan="2">
                    <textarea rows="15" name="keterangan" style="display:inline-block" placeholder="[Keterangan]" required class="form-control"></textarea></td>
                  </td>
              </tr>
                ...
                @foreach ($riwayat as $item)
                  <tr>
                    <td> 
                      ...
                      <b>Tanggal	:</b> {{date('d-m-Y',strtotime($item->tgl_kontrol))}}, {{$item->jam_kontrol}} <br/>
                      {{-- <b>Keterangan	:</b> {!! trim(str_replace('\n', ' ', (str_replace('\r', ' ', $item->keterangan))))!!}<br/> --}}
                      <b>Keterangan	:</b> {{json_decode($item->keterangan)}}<br/>
                      {{date('d-m-Y, H:i:s',strtotime($item->created_at))}}
                      <span class="pull-right">
                        <a onclick="return confirm('Yakin akan menghapus data?')" href="{{url('emr-soap/perencanaan/catatanKhusus/jalan/'.$reg->id.'/'.$item->id.'/delete')}}" data-toggle="tooltip" title="Hapus"><i class="fa fa-trash text-danger"></i></a>&nbsp;&nbsp;
                      </span>
                    </td>
                  </tr>
                @endforeach
