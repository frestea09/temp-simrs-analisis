...
@endsection
@php
    $regDummy = \App\RegistrasiDummy::where('registrasi_id',@$reg->id)->first();
    $nokartu = @$reg->pasien->no_jkn ? @$reg->pasien->no_jkn : @$regDummy->nomorkartu;
        ...
        <div class="col-sm-12">
          <div class="form-group">
            ...
            <div class="col-sm-10 btn-group">
              ...
              <button type="button" onclick="nomorRujukan()" class="btn btn-success btn-flat">RJKN. PPK 1</button>
              ...
              <button type="button" onclick="postRawat()" class="btn btn-primary btn-flat">POST RAWAT </button>
              <button type="button" onclick="SURKON()" class="btn btn-warning btn-flat">SURKON</button>
              <button style="background-color: blueviolet" type="button" onclick='window.open("{{url("/create-spri/".$reg->id)}}","mywindow","menubar=1,resizable=1,width=1000,height=600");' class="btn btn-info btn-flat">SPRI</button>
      ...
      {!! Form::open(['method' => 'POST', 'url' => 'admission/save-sep/irj-igd', 'class' => 'form-horizontal', 'id'=>'formSEP']) !!}
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <div class="form-group">
                ...
                <div class="col-sm-8">
                  <select name="dpjpLayan" class="form-control select2" style="width: 100%">
                    {{-- <option value="" selected>{{ baca_dokter($reg->dokter_id) }}</option>
                    @foreach ($dokter as $d)
                    ...
                    @foreach ($dokter as $d)
                      {{-- @if ($d->kode_bpjs == $dokter_bpjs) --}}
                        {{-- <option value="{{ $d->kode_bpjs }}"="true">{{ $d->nama }}</option>
                      @else --}}
              ...
              {{--<div class="form-group">
                <label for="dpjpLayanan" class="col-sm-4 control-label">DPJP</label>
                <div class="col-sm-8">
                  <select name="dpjpLayan" class="form-control select2" style="width: 100%">
                    <option value="{{ $reg->dokter_id }}">{{ baca_dokter($reg->dokter_id) }}</option>
                  </select>
                </div>
              </div>--}}
              ...
              <div class="form-group{{ $errors->has('no_sep') ? ' has-error' : '' }}">
                  <div class="col-sm-4 control-label">
                    {{-- @if (Auth::user()->id == 1 || Auth::user()->id == 611 || Auth::user()->id == 573 || Auth::user()->id == 571 || Auth::user()->id ==627)  --}}
                    <button type="button" id="createSEPValidasi" class="btn btn-primary btn-flat"><i class="fa fa-recycle"></i> BUAT SEP</button>
                    {{-- @else
              ...
              {{-- @if (Auth::user()->id == 1 || Auth::user()->id == 611 || Auth::user()->id == 573 || Auth::user()->id == 571 || Auth::user()->id ==627)  --}}
                <h4 style="padding-top:30px;">Jika Error : "Rujukan nomor sudah terbit SEP"<br/><b>Solusinya</b> :<br/></h4>
                <img src="{{ asset('/images/bpjsantrean.jpg') }}" style="width: 100%" alt="">
              {{-- @endif --}}
            </div>
          </div><br/>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" class="form-horizontal" id="formPencarian">
            ...
            <div class="form-group judulInput">
              <label for="judul" class="col-sm-3 control-label judul"></label>
              <div class="col-sm-6 formInput">
              </div>
            </div>
            <div class="form-group surkonSep">
              Lihat Histori Pelayanan <b>SEP</b> Sebelumnya <a target="_blank" href="{{url('bridgingsep/histori-kunjungan?nokartu='.@$reg->pasien->no_jkn)}}">Disini</a><br/>
              <label for="NoSep" class="col-sm-3 control-label NoSep"></label>
              <div class="col-sm-6 formInputNoSep">
                <input type="text" name="noSepKontrol" class="form-control" id="noSepKontrol">
              </div>
            </div>
            <div class="form-group surkonTgl">
              <label for="Tgl" class="col-sm-3 control-label Tgl"></label>
              <div class="col-sm-6">
                <input type="date" class="form-control" value="{{date('Y-m-d')}}" name="tglKontrol">
              </div>
            </div>
            <div class="form-group surkonPoli">
              <label for="Poli" class="col-sm-3 control-label Poli"></label>
              <div class="col-sm-6 formInputPoli">
                <select name="poliKontrol" class="form-control select2" style="width: 100%">
                  @foreach ($poli as $d)
                       <option value="{{ $d->bpjs }}" {{$d->bpjs == $poli_bpjs ? 'selected' : ''}}>{{ $d->nama }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            <div class="form-group surkonDpjp">
              <label for="Dpjp" class="col-sm-3 control-label Dpjp"></label>
              <div class="col-sm-6 formInputDpjp">
                <select name="dokterKontrol" class="form-control select2" style="width: 100%">
                @foreach ($dokter as $d)
                    <option value="{{ $d->kode_bpjs }}" {{$d->kode_bpjs == $dokter_bpjs ? 'selected' : ''}}>{{ $d->nama }}</option>
                @endforeach
                </select>
            ...
            <div class="form-group surkonKontrol">
              <label for="noKontrol" class="col-sm-3 control-label btn btn-primary noKontrol" onclick="getDataKontrol()">
                Buat No Kontrol
              </label>
              <div class="col-sm-6 formInputNoKontrol">
                <input type="text" class="form-control" id="noSuratKontrol" name="noSuratKontrol" readonly><br/>
                <span class="text-danger">*</span> Abaikan tombol <b>"CARI"</b>
              </div>
            </div>
          </form>
          @if (count($surkon) > 0)
          ...
          {{-- <b>Histori SURKON (Data SIMRS) - 10 data terakhir</b><br/> --}}
          {{-- <table class="table table-bordered" style="font-size:12px;" id="data">
            <tr>
              <th>No</th>
              <th>No.Surat</th>
              <th>No.SEP</th>
          ...
          {{-- SHOW SEP 2 WEEKS AGO --}}
          <div class="table-responsive">
            <table class="table table-bordered table-condensed tableMultipleSep hidden" style="font-size:11px !important;">
              <thead>
                <tr>
                  {{-- <th>SEP</th> --}}
                  {{-- <th>No. Kartu</th> --}}
                  <th style="width:70px !important;">Kelas</th>
                  <th>SEP</th>
                  <th>No.Rujukan</th>
                  <th>Poli</th>
                  <th>Tgl.SEP</th>
...
@section('script')
  ...
  <script type="text/javascript">
    ...
    function nomorKartu() {
      $('.judulInput').removeClass('hidden');
      $('.surkonSep').addClass('hidden')
      $('.surkonDpjp').addClass('hidden')
      $('.surkonTgl').addClass('hidden')
      $('.surkonPoli').addClass('hidden')
      $('.surkonKontrol').addClass('hidden')
      $('#modalPencarian').modal('show')
      $('.modal-dialog').addClass('modal-lg')
      $('.tableMultipleRujukan').addClass('hidden')
      $('.tableMultipleSep').addClass('hidden')
      ...
      $('.formInput').empty()
      $('.formInput').append('<input type="text" name="no_kartu" value="'+noKartu+'" class="form-control id="noKartu">')
      $('.save').attr('onclick', 'cariNoKartu()');
    }
     function SURKON() {
      $('.surkonKontrol').removeClass('hidden')
      $('.surkonSep').removeClass('hidden')
      $('.surkonTgl').removeClass('hidden')
      ...
      $('.lanjut').addClass('hidden')
      $('.rujukan').addClass('hidden');
      $('.judulInput').addClass('hidden');
      $('.statusError').text('')
      $('.modal-title').text('Surat Kontrol')
      $('.NoSep').text('No Sep')
      $('.Tgl').text('Tanggal')
      $('.Poli').text('Poli')
      $('.Dpjp').text('DPJP')
      // $('.judulInput').addClass('hidden')
      // $('.surkonSep').removeClass('hidden')
      // $('.formInputNoSep').append('<input type="text" name="noSepKontrol" class="form-control" id="noSepKontrol">')
      // $('.surkonTgl').removeClass('hidden')
      // $('.formInputTgl').append('<input type="date" class="form-control" name="tglKontrol">')
      ...
      $.ajax({
        ...
        .done(function (res) {
          $('.surkonPoli').removeClass('hidden')
          // $('.formInputPoli').html(res);
        })
        .fail(function () {
        })
      $.ajax({
        ...
        .done(function (res) {
          $('.surkonDpjp').removeClass('hidden')
          // $('.formInputDPJP').html(res);
      ...
      // $('.surkonKontrol').removeClass('hidden')
      // $('.formInputNoKontrol').append('<input type="text" class="form-control" id="noSuratKontrol" name="noSuratKontrol" readonly>')

      $('.save').attr('onclick', 'url');
    }
    ...
    function postRawat2() {
      ...
      $('.rujukan').addClass('hidden');
      $('.statusError').text('')
      $('.modal-title').text('Pencarian Post Rawat Berdasar Nomor Kartu')
      $('.judul').text('Nomor Kartu')
      $('.formInput').empty()
      $('.formInput').append('<input type="text" name="no_kartu" value="'+noKartu+'" class="form-control id="noKartu">')
      $('.save').attr('onclick', 'cariHistori()');
    }
    ...
    function rujukanRS() {
      ...
      $('.judul').text('Nomor Kartu')
      $('.formInput').empty()
      $('.formInput').append('<input type="text" name="no_kartu" value="'+noKartu+'" class="form-control">')
    ...
    function rujukanRSbyRujukan() {
      ...
      $('.formInput').empty()
      $('.formInput').append('<input type="text" value="'+noRujukan+'" name="no_rujukan" class="form-control">')
    ...
    function nomorRujukan() {
      ...
      $('.judul').text('Nomor Rujukan')
      $('.formInput').empty()
      $('.formInput').append('<input type="text" name="no_rujukan" value="'+noRujukan+'" class="form-control" id="noRujukan">')
    ...
    function postRawat() {
      ...
      $('.judul').text('Nomor Kartu')
      $('.formInput').empty()
      $('.formInput').append('<input type="text" value="'+noKartu+'" name="no_kartu" class="form-control">')
...
// ================================== AJAX =============================================
    ...
    // Cari Histori Pelayanan peserta
    function cariHistori() {
      ...
      .done(function(res) {
        ...
        if (data[0].metaData.code == 200) {
          $('.tableMultipleSep').removeClass('hidden')
          $('#dataMultipleSep').empty()
          $.each(data[1].histori, function(index, val) {
             $('#dataMultipleSep').append('<tr> <td>'+val.kelasRawat+'</td> <td><button class="btn btn-default btn-flat btn-xs" onclick="getDataSep(\''+val.noSep+'\',\''+val.namaPeserta+'\')">'+val.noSep+'</button></td><td>'+val.noRujukan+'</td> <td>'+val.poli+'</td> <td>'+val.tglSep+'</td> <td>'+val.tglPlgSep+'</td> <td>'+val.diagnosa+'</td> </tr>')
          });
        }else{
    ...
    function getDataRujukan(noKunjungan) {
      ...
      if (confirm('Yakin Nomor Rujukan '+ noKunjungan +' akan di buat SEP?')) {
        ...
        .done(function(res) {
          ...
          $('input[name="niks"]').val(data[1].rujukan.peserta.niks)
    ...
    //GET SEP FROM MULTIPLE
    function getDataSep(sep,nama) {
      // console.log(noKartu,nama)
      // return
      // var token = $('input[name="_token"]').val()
      // if (confirm('Yakin Nomor Rujukan '+ noKunjungan +' akan di buat SEP?')) {
        // $.ajax({
        ...
        //   // console.log(data[1].histori[index])
        //   return 
        //   data = JSON.parse(res)
          $('.tableMultipleRujukan').removeClass('hidden')
          $('.statusError').text('')
    ...
    function getDataKontrol() {
      ...
      $('.statusError').text('') 
      $.ajax({
        url: '/bridgingsep/buat-surat-kontrol',
        type: 'POST',
        dataType: 'json',
        data: {
          registrasi_id : $('input[name="registrasi_id"]').val(),
          no_sep : $('input[name="noSepKontrol"]').val(),
          rencana_kontrol : $('input[name="tglKontrol"]').val(),
          poli_id : $('select[name="poliKontrol"]').val(),
          kode_dokter : $('select[name="dokterKontrol"]').val(),
          _token : $('input[name="_token"]').val(),
          _method : 'POST'
          },
        beforeSend: function () {
      ...
      .done(function(res) {
        ...
        if (data[0].metaData.code == 200) {
          $('input[name="noSuratKontrol"]').val(data[1].noSuratKontrol)
          $('input[name="noSurat"]').val(data[1].noSuratKontrol)  
        } else {
          alert(data[0].metaData.message)
        }
      });
    ...
    function cariNoRujukan() {
      ...
      .done(function(res) {
        ...
        if (data[0].metaData.code == 200) {
          ...
          $('input[name="nik"]').val(data[1].rujukan.peserta.nik)
          $('input[name="niks"]').val(data[1].rujukan.peserta.nik)
          $('input[name="nama_perujuk"]').val(data[1].rujukan.provPerujuk.nama)
          $('input[name="diagnosa_awal"]').val(data[1].rujukan.diagnosa.kode)
          $('input[name="hak_kelas_inap"]').val(data[1].rujukan.peserta.hakKelas.kode)
...
// ======================================================================================================================
    ...
    $(document).ready(function() {
      ...
      $('#createSEPValidasi').on('click', function () {
        $("input[name='no_sep']").val( ' ' );
        $.ajax({
          url : '{{ url('/buat-sep') }}',
          type: 'POST',
