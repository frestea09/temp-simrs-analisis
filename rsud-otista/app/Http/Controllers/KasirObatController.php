...
class KasirObatController extends Controller
{
	public function rawat_jalan()
	{
		...
		// 	// ->whereNotIn('bayar', [1])
		// 	// ->with(['dokter_umum', 'bayars'])
		// 	->where('created_at', 'LIKE', date('Y-m-d' . '%'))
		// 	->get();

		return view('kasir-obat.index', compact('bayar'))->with('no', 1);
	}
	public function data_rawat_jalan(Request $req)
	{
		...
		// if($req->filled('keyword')){
			...
			$d = Registrasi::leftJoin('pasiens', 'pasiens.id', 'registrasis.pasien_id')
				...
				$d = $d->where('registrasis.status_reg', 'like', 'J%')
		...
		// }else{
		// 	$keyCache = 'kasir_obat';
		// 		$d = Registrasi::where('status_reg', 'like', 'J%')
		// 		->with(['pasien', 'dokter_umum', 'poli', 'bayars'])
		// 		->whereHas('poli', function ($query){
		// 			$query->where('kelompok', '!=', 'ESO');
		// 		})
		...
		return DataTables::of($d)
			...
			->addColumn('bayar', function ($d) {
				if(@$d->lunas == 'Y'){
                  return '<a href="'.url('kasir-obat/rawatjalan/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-success btn-flat"><i class="fa fa-credit-card"></i></a>';
                }else{
                  return '<a href="'.url('kasir-obat/rawatjalan/bayar/'. @$d->id.'/'.@$d->pasien_id).'" class="btn btn-sm btn-info btn-flat"><i class="fa fa-credit-card"></i></a>';
				}
			})
	...
	public function rawatjalanByTanggal(Request $request)
	{
		...
		$today = Registrasi::leftJoin('pasiens', 'pasiens.id', '=', 'registrasis.pasien_id')
		...
		return view('kasir-obat.index', compact('today'))->with('no', 1);
	}
	...
	public function ajax_rawat_jalan()
	{
		...
		return view('kasir-obat.rawat_jalan', compact('today', 'bayar'))->with('no', 1);
	}
	...
	public function rawat_inap()
	{
		$today = Registrasi::join('rawatinaps', 'registrasis.id', '=', 'rawatinaps.registrasi_id')
			...
			->get();
		return view('kasir-obat.irna', compact('today'))->with('no', 1);
	}
	...
	public function igd()
	{
		...
		$today = Registrasi::where('status_reg', 'like', 'G%')
			...
			//->where('verif_kasa', 'N')
			->get();
		return view('kasir-obat.index_igd', compact('today', 'bayar'))->with('no', 1);
	...
	public function byRM(Request $request)
	{
		...
		$byRM = DB::table('registrasis')
			...
			->get();
		// return $byRM;
		return view('kasir-obat.rawat_jalan', compact('byRM'))->with('no', 1);
	}
	...
	public function byTanggal(Request $request)
	{
		...
		return view('kasir-obat.rawat_jalan', compact('today'))->with('no', 1);
	}

	public function bayar_rawat_jalan($reg_id, $pasien_id)
	{
		$data['tag'] = Tagihan::where('registrasi_id', '=', $reg_id)->where('user_id', '<>', 610)->get();
		$data['fol'] = Folio::where('jenis','ORJ')
		...
		$data['tagihan'] = Folio::where('jenis','ORJ')->where('registrasi_id', $reg_id)->where('lunas', 'N')->where('user_id', '<>', 610)->sum('total');
		$data['reg'] = Registrasi::find($reg_id);
		$data['metode'] = MetodeBayar::select('id', 'name')->get();
		// $data['eklaim'] = Inacbg::where('registrasi_id', $reg_id)->first();
		// dd($data['eklaim']);
		// $data['uang_racik'] = Penjualan::join('penjualandetails', 'penjualans.id', '=', 'penjualandetails.penjualan_id')
		...
		// foreach($data['fol'] as $d){
			// $data['obat'] = Penjualandetail::where('no_resep', $d->namatarif)->sum('uang_racik');
			// foreach($d->obat as $detail){
			// 	$data['uang_racik'] += $detail->uang_racik;
			// }
		// }
		$data['jasa_racik'] = 0;
		// $data['jasa_racik'] = Folio::where('registrasi_id', '=', $reg_id)->where('folios.lunas', 'N')->where('folios.user_id', '<>', 610)->sum('jasa_racik');
		// return $data; die;
		return view('kasir-obat.bayar_rawat_jalan', $data)->with('no', 1);
	}
	...
	public function save_bayar_rawat_jalan(Request $request)
	{
		...
		if ($request['total'] != 0) {
			if (rupiah($request['totalBayar']) < rupiah($request['total_tagihan'] && $request['diskon_persen'] == 0 && $request['diskon_rupiah'] == 0)) {
				...
				return redirect('/kasir/rincian-biaya-obat/' . session('idk'));
			}
		}
	}
	...
	public function cetak_RincianBiayaObat($id = '') {
		
		$data['kuitansi'] = Pembayaran::where('id',$id)->first();
		//dd($data['kuitansi']);
		$data['reg']      = Registrasi::with('bayars')->find($data['kuitansi']->registrasi_id);
		$nokuitansi       = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->where('jenis', 'ORJ')->where('lunas', 'Y')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->where('user_id', '<>', 610)->first();
		if(!$nokuitansi){
		  $noresep = '';
		}else{
		...
		// 					->get();
		$data['penjualan'] = Folio::join('penjualans','penjualans.no_resep','=','folios.namatarif')
			->join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
			->where('folios.registrasi_id', '=', $data['kuitansi']->registrasi_id)
			->where('folios.no_kuitansi', '=', $data['kuitansi']->no_kwitansi)->where('folios.lunas', 'Y')
	   ...
	   // $data['folio_irna']   = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->groupBy('tarif_id')->where('lunas', 'Y')->where('jenis', 'TI')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->get();
		...
		$data['no'] = 1;
		$pdf        = PDF::loadView('kasir-obat.cetakUlangKuitansiObat-all', $data)->setpaper('folio', 'landscape');
		return $pdf->stream();
	}
	...
	public function bayar_rawat_inap($reg_id, $pasien_id)
	{
		...
		if ($data['reg']->bayar == 1) {
			return view('kasir-obat.bayar_rawat_inap_jkn', $data)->with('no', 1);
		} else {
			return view('kasir-obat.bayar_rawat_inap_non_jkn', $data)->with('no', 1);
		}
	}
	...
	public function cetak_kuitansi_langsung_irna($id = '')
	{
		...
		return view('kasir-obat.kuitansi_rawat_inap', compact('kuitansi', 'folio', 'jml', 'reg', 'kamar'));
		// $pdf = PDF::loadView('kasir-obat.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}
	...
	public function cetak_kuitansi_langsung($id = '')
	{
	   ...
	   $penjualan = Penjualan::join('penjualandetails','penjualans.id','=','penjualandetails.penjualan_id')
		...
		return view('kasir-obat.kuitansi', compact('jenisracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj','Totracik','uangracik'));
		// $pdf = PDF::loadView('kasir-obat.kuitansi', compact('kuitansi', 'folio', 'jml'));
		// return $pdf->stream();
	}
	...
	public function cetak_RincianBiaya($id = '') {
	   ...
	   // $data['folio_irna']   = Folio::where('registrasi_id', '=', $data['kuitansi']->registrasi_id)->groupBy('tarif_id')->where('lunas', 'Y')->where('jenis', 'TI')->where('no_kuitansi', $data['kuitansi']->no_kwitansi)->get();
		...
		$data['no'] = 1;
		$pdf        = PDF::loadView('kasir-obat.cetakUlangKuitansi-all', $data)->setpaper('folio', 'landscape');
		return $pdf->stream();
	}
	...
	// 	$pdf = PDF::loadView('kasir-obat.cetakUlangKuitansi', compact('reg', 'kuitansi', 'folio', 'jml', 'no', 'dokter', 'uang_racik', 'jasa_racik'));
	// 	return $pdf->stream();
	// }
	...
	public function cetak_RincianBiayaNonJasa($id = '')
	{
		...
		$pdf = PDF::loadView('kasir-obat.cetakUlangKuitansiNonJasa', compact('reg', 'kuitansi', 'folio', 'jml', 'no', 'dokter', 'uang_racik', 'jasa_racik'));
		return $pdf->stream();
	}
	...
	public function cetakIGD()
	{
		...
		return view('kasir-obat.cetakIGD', compact('pemb'))->with('no', 1);
	}
	...
	public function cetakIRNA()
	{
		$pemb =  Pembayaran::join('rawatinaps', 'rawatinaps.registrasi_id', '=', 'pembayarans.registrasi_id')
			...
			->get();
		return view('kasir-obat.cetakIRNA', compact('pemb'))->with('no', 1);
	}
	...
	public function cetakRj()
	{
		...
		return view('kasir-obat.cetak', compact('pemb'))->with('no', 1);
	}
	...
	public function cetakkuitansiigd($id = ''){
		...
		$jml_irj = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)
			...
			->where('no_kuitansi', $kuitansi->no_kwitansi)->sum('total');
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir-obat.kuitansi_igd', compact('Totracik','jenisracik','uangracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj'));
	...
	public function cetakkuitansi($id = '')
	{
		...
		// 	->sum('uang_racik');
		// $jasa_racik = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->sum('jasa_racik');
		// return view('kasir-obat.kuitansi_rawat_darurat', compact('kuitansi', 'folio', 'jml', 'reg', 'kamar', 'jasa', 'konsul', 'obat', 'dokter', 'uang_racik', 'jasa_racik'));
		...
		// return compact('kuitansi', 'folio', 'jml','jasa_racik','uang_racik'); die;
		return view('kasir-obat.kuitansi_inap', compact('jenisracik','Totracik','uangracik','kuitansi', 'folio', 'jml', 'jasa_racik','uang_racik','obat','penjualan','reg','jml_obat','jml_irj'));
		// $pdf = PDF::loadView('kasir-obat.kuitansi', compact('kuitansi', 'folio', 'jml'));
	...
	public function cetaknonjasakuitansi($id = '')
	{
		...
		return view('kasir-obat.kuitansi_non_jasa_racik_rawat_jalan', compact('kuitansi', 'folio', 'jml', 'reg', 'kamar', 'jasa', 'konsul', 'obat', 'dokter', 'jasa_racik', 'uang_racik'));
	}

	//======== RAWAT INAP =================================
	public function verifikasi()
	{
		return view('kasir-obat.verifikasiDataTable');
	}
	...
	public function getDataVerifInap(Request $request)
	{
		...
		return view('kasir-obat.verifikasiDataTable', compact('verif'))->with('no', 1);
	}
	...
	public function detailVerifikasi($registrasi_id = '')
	{
		...
		$data['i_hapus'] = 1;
		// dd($data);
		return view('kasir-obat.detailVerifikasi', $data)->with('no', 1);
	}
	...
	public function detailTindakanVerifikasi($registrasi_id = '', $poli_tipe = '')
	{
		...
		$data['tipe']	= 'ranap';
		return view('kasir-obat.detailTindakanVerifikasi', $data)->with('no', 1);
	}
	...
	{
		$type = 'Rawat Inap';
		return view('kasir-obat.uangmuka', compact('type'));
	}
	...
	public function uangmuka_rawatinap_byPasien(Request $request)
	{
		...
		return view('kasir-obat.uangmuka', $data)->with('no', 1);
	}
	...
	public function viewDeposit($registrasi_id)
	{
		...
		return view('kasir-obat.viewDeposit', compact('dp', 'total'))->with('no', 1);
	}
	...
	public function cetakDeposit($id)
	{
		...
		return view('kasir-obat.kuitansiDeposit', compact('dp', 'reg', 'irna'));
	}
	...
	public function cetakReturn($id)
	{
		...
		$reg = Registrasi::find($dp->registrasi_id);
		$irna = Rawatinap::where('registrasi_id', $reg->id)->first();
		return view('kasir-obat.returnDeposit', compact('dp', 'reg', 'irna'));
	}

	public function tutup_transaksi()
	{
		return view('kasir-obat.tutup_transaksi');
	}
	...
	public function transaksi_lain_lain()
	{
		...
		$data = $row;
		return view('kasir-obat.lain_lain', compact('data'));
	}
	...
	public function form_bayar_lain_lain($registrasi_id = '')
	{
		...
		if ($reg->status_reg == 'A1') {
			...
			// return $data; die;
			return view('kasir-obat.form_bayar_lain_lain', $data)->with('no', 1);
		} else {
			$data['total'] = Folio::where('registrasi_id', $registrasi_id)->where('lunas', 'N')->sum('total');
			$data['rincian'] = Folio::where('registrasi_id', $registrasi_id)->where('lunas', 'N')->get();
			return view('kasir-obat.form_bayar_PasienLangsung', $data)->with('no', 1);
		}
	}
	...
	public function cetak_kuitansi_bebas($id = '')
	{
		...
		$folio = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->get();
		$jml = Folio::where('registrasi_id', '=', $kuitansi->registrasi_id)->where('lunas', 'Y')->sum('total');
		return view('kasir-obat.kuitansi_bebas', compact('kuitansi', 'folio', 'jml', 'pasien', 'uang_racik', 'jasa_racik'));
	}

	//SUPERVISOR
	public function edit_transaksi()
	{
		return view('kasir-obat.edit_transaksi');
	}
	...
	public function batal_bayar()
	{
		...
		return view('kasir-obat.batal_bayar', $data)->with('no', 1);
	}
	...
	public function rincian_pembayaran($registrasi_id)
	{
		...
		return view('kasir-obat.view_rincian_pembayaran', $data)->with('no', 1);
	}
	...
	{
		return view('kasir-obat.batal_piutang');
	}

	//LAPORAN
	public function lap_detail_tindakan()
	{
		return view('kasir-obat.lap_detail_tindakan');
	}
	...
	public function lap_detail_tindakanByFilter(Request $request)
	{
		...
		$data['reg'] = Registrasi::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])
			...
			->get();
		return view('kasir-obat.lap_detail_tindakan', $data)->with('no', 1);
	}

	public function lap_penerimaan_tunai()
	{
		return view('kasir-obat.lap_penerimaan_tunai');
	}
	...
	public function lap_penerimaan_tunai_byTanggal(Request $request)
	{
		...
		$data['piutang'] = Pembayaran::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])
			->where('jenis', 'piutang')
			->sum('dibayar');
		return view('kasir-obat.lap_penerimaan_tunai', $data);
	}
	...
	public function tutup_kasir()
	{
		$user = User::join('role_user', 'users.id', '=', 'role_user.user_id')
			...
			->get();
		return view('kasir-obat.tutup_kasir', compact('user'));
	}
	...
	public function tutup_kasir_byRequest(Request $request)
	{
		...
		if ($request['lanjut']) {
			return view('kasir-obat.tutup_kasir', compact('user', 'pembayaran', 'tunai', 'piutang'))->with('no', 1);
			...
			$pdf = PDF::loadView('kasir-obat.pdf_tutup_kasir', compact('user', 'pembayaran', 'tunai', 'piutang', 'no', 'petugas', 'periode'));
	...
	public static function dataPiutang()
	{
		$piutang = Piutang::join('registrasis', 'registrasis.id', '=', 'piutangs.registrasi_id')
			...
			->get();
		return view('kasir-obat.dataPiutang', compact('piutang'))->with('no', 1);
	}
	...
	public function verifikasiRajal($id = '')
	{
		...
		return view('kasir-obat.verifikasiRajal', $data)->with('no', 1);
	}
	...
	public function detailVerifRajal($reg = '', $poli_tipe = '')
	{
		...
		$data['poli_tipe'] = $poli_tipe;
		$data['tipe']	= 'rajal';
		return view('kasir-obat.detailTindakanRajal', $data)->with('no', 1);
	}
	...
	public function verifikasiKasa()
	{
		$data['registrasi'] = Registrasi::whereIn('status_reg', ['J1', 'J2', 'G1', 'G2'])
			...
			// ->where('verif_kasa', 'N')
			->get();
		return view('kasir-obat.verifikasiKasa.verifikasiKasa', $data)->with('no', 1);
	}
	...
	public function detailVerifikasiKasa($registrasi_id)
	{
		...
		return view('kasir-obat.verifikasiKasa.detailVerifikasiKasa', $data)->with(['no' => 1, 'baris' => 1]);
	}
	...
	public static function cetakVerifikasi($registrasi_id)
	{
		...
		$no = 1;
		$pdf = PDF::loadView('kasir-obat.verifikasiKasa.cetakVerifikasi', compact('folio', 'reg', 'jml', 'no', 'irna', 'user_kasa'));
		$pdf->setPaper('legal');
		return $pdf->stream();
	}
	...
	function tambahTindakan($registrasi_id)
	{
		...
		return view('kasir-obat.verifikasiKasa.tambahTindakan', $data);
	}
	...
	{
		return view('kasir-obat.verifikasiKasa.laporanVerifikasi', compact('today'))->with('no', 1);
	}
	...
	public function lap_Verifikasi_berkas_bytanggal(Request $request)
	{
		...
		$reg = Registrasi::whereBetween('created_at', [valid_date($request['tga']) . ' 00:00:00', valid_date($request['tgb']) . ' 23:59:59'])
			...
			->get();
		if ($request['tampil']) {
			return view('kasir-obat.verifikasiKasa.laporanVerifikasi', compact('reg', 'tga'))->with('no', 1);
		} else {
	...
	public function cetak_kuitansi_naik_kelas($id = '')
	{
		...
		$pdf = PDF::loadView('kasir-obat.kuitansi_naik_kelas', compact('reg', 'kuitansi', 'inacbg', 'irna'));
		return $pdf->stream();
	}
	...
	{
		$tarif = Tarif::all();
		return view('kasir-obat.diklat', compact('tarif'));
	}
	...
	{
		$diklat = \App\PendapatanDiklat::find($id);
		return view('kasir-obat.cetakDiklat', compact('diklat'));
	}
	...
	{
		$data = [];
		return view('kasir-obat.jamkesda', compact('data'));
	}
	...
	public function jamkesdaGetData(Request $request)
	{
		...
		$data = Registrasi::join('pasiens', 'registrasis.pasien_id', '=', 'pasiens.id')
			...
			->get();
		return view('kasir-obat.jamkesda', compact('data'))->with('no', 1);
	}
	...
	public function dunnoWhat()
	{
		$data['reg'] = Folio::join('registrasis', 'folios.registrasi_id', '=', 'registrasis.id')
			...
			->get();
		return view('kasir-obat.dunno_what', $data)->with('no', 1);
	}
	...
	public function transaksiKeluar()
	{
		...
		return view('kasir-obat.transaksiKeluar', compact('satuan', 'klasifikasi', 'jenis', 'akun_coa'));
	}
	...
	{
		$diklat = \App\TransaksiKeluar::find($id);
		return view('kasir-obat.cetakTransaksiKeluar', compact('diklat'));
	}
