...
use App\Antrian5 as Antrian;
use DB;
use Illuminate\Http\Request;
use Illuminate\Routing\Controller;

class Antrian5Controller extends Controller {
	...
	public function savetouch(Request $request) {
		...
		Antrian::create($data);
		return view('antrian::cetak_antrian', $data)->with('loket', 5);
	}
	...
	public function datalayarlcd() {
		...
		$terpanggil = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			->orderBy('id', 'desc')
			->first();
		return view('modules.antrian5.datalayarlcd', compact('antrian', 'terpanggil'));
	}
	...
	public function daftarpanggil() {
		$antrian = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			->take(5)
			->get();
		return view('modules.antrian5.daftarpanggil', compact('antrian'));
	}
	...
	public function daftarantrian() {
		...
		$terpanggil = Antrian::where('tanggal', '=', date('Y-m-d'))
			...
			// ->take(5)
			->get();
		return view('modules.antrian5.daftarantrian', compact('terpanggil'));
	}

	public function panggil($id = '') {
		// DB::table('antrian5')->where('id', $id)->update(['status' => 1]);
		$atr = Antrian::find($id);
		$atr->status = 1;
		$atr->loket = 5;
		$atr->update();
		return redirect()->route('antrian5.daftarantrian');
	}
	...
	public function registrasi($id, $jenis) {
		...
		$d->status = 3;
		$d->update();
		// DB::table('antrian5')->where('id', $id)->update(['status' => 3]);
		session(['no_loket' => 5]);
		if ($jenis == 'jkn') {
