...
class ApiControllerBACKUP extends Controller
{
    ...
    if ($request->header('x-token') == $signature && $request->header('x-username') == config('app.username_ws')) {
      ...
      foreach($operasi as $value){
        $map[] = [
          ...
          'tanggaloperasi' => $value->rencana_operasi,
          'jenistindakan' => strip_tags($value->suspect),
          'kodepoli' => $value->kodepoli,
          'namapoli' => $value->namapoli,
          'terlaksana' => $value->terlaksana,
          'nopeserta' => $value->no_jkn,
          'lastupdate' => (string) round(microtime(true) * 1000)
        ];
      }
      // return $map;
      // dd($map);
  ...
  public function rekapAntrian(Request $request)
  {
    ...
    if ($request->header('x-token') == $signature && $request->header('x-username') == config('app.username_ws')) {
      ...
      // $poli   = Poli::where('bpjs', $request->kodepoli)->first();
        ...
        $kuotaJKN = Poli::where('bpjs',$request->kodepoli)->first()->kuota;
  ...
  public function antrian(Request $request)
  {
    ...
    if(!$cekDokter->first()){
      $resp = [
        'metadata' =>[
          'message' =>"Dokter tidak ditemukan ditanggal ini",
          'code'=>201
        ]
      ]; 
    ...
    // CEK NO RM 
    $no_rm = Nomorrm::where('no_rm',$request->norm)->first();
    if(!$no_rm){
      $resp = [
        'metadata' =>[
  ...
  { 
    ...
    // Cek nomor kartu
    if(strlen($request->nomorkartu) < 13&& !is_numeric($request->nomorkartu))   {
      $resp = '{
        "metadata": {
    ...
    // Cek nomor NIK
    if(strlen($request->nik) < 16 && !is_numeric($request->nik))    {
      $resp = '{
        "metadata": {
  ...
  public function tambahantrian( Request $request ) {
    ...
    try{ 
      ...
      $req2   = '{
        "kodebooking": "'.$request['kodebooking'].'",
        "taskid": 1,
        "waktu": "'.$request['estimasidilayani'].'"
     }';
