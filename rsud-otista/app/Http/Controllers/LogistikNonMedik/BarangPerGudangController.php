...
use DataTables;
use Validator;
use App\LogistikNonMedik\LogistikNonMedikBarangPerGudang;
...
class BarangPerGudangController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $barang = LogistikNonMedikBarangPerGudang::latest();
            return DataTables::of($barang)
                ...
                ->addColumn('gudang', function ($barang) {
                    return $barang->gudang->nama;
                })
                ...
                ->rawColumns(['aksi', 'golongan', 'bidang', 'kelompok', 'subkelompok', 'supplier', 'gudang'])
                ->make(true);
        }
        // return $barang; die;
    ...
    public function store(Request $request)
    {
        $cek = Validator::make($request->all(), [
            'nama' => ['required', 'string', 'max:255', 'unique:logistik_non_medik_barang_per_gudangs,nama'],
            'kode' => ['required'],
            'harga_beli' => ['required'],
            'harga_jual' => ['required'],
        ...
        if ($cek->fails()) {
            ...
            $barang = new LogistikNonMedikBarangPerGudang();
    ...
    {
        $barang = LogistikNonMedikBarangPerGudang::find($id);
    ...
    {
        $cek = Validator::make($request->all(), [
            'nama' => ['required', 'string', 'max:255', 'unique:logistik_non_medik_barang_per_gudangs,nama,' . $id],
            'kode' => ['required'],
            'harga_beli' => ['required'],
            'harga_jual' => ['required'],
            'satuan_beli' => ['required'],
