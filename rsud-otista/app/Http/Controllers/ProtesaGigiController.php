...
class ProtesaGigiController extends Controller{
    ...
    public function pasienLangsung(){
        $data = Pasienlangsung::leftJoin('registrasis', 'registrasis.id', '=', 'pasien_langsung.registrasi_id')
                ...
                ->where('pasien_langsung.politype', 'T')
                ->get();
		return view('protesa-gigi.pasienLangsung', compact('data'))->with('no', 1);
    }

    public function entryLangsung($reg_id = ''){
        $data['reg']        = Registrasi::find($reg_id);
        $data['tagihan']    = Folio::where('registrasi_id', $reg_id)->where('poli_tipe', 'T')
        ...
        $data['cek']        = Folio::where(['jenis' => 'TA', 'registrasi_id' => $reg_id, 'poli_tipe' => 'T'])->count();
        $data['folio']      = Folio::where(['jenis' => 'TA', 'registrasi_id' => $reg_id, 'poli_tipe' => 'T'])->get();
        return view('protesa-gigi.entryTindakanLangsung', $data)->with('no', 1);
    }
    ...
    public function laporanPengunjung(){
        ...
        $data['pengunjung'] = Folio::with(['registrasi.pasien', 'registrasi.poli'])
                    ->whereDate('folios.created_at', date('Y-m-d'))
                    ->where('folios.poli_tipe', 'T')
                    ->get();
        return view('protesa-gigi.laporanPengunjung', $data)->with('no', 1);
    }
    ...
    public function filterLaporan(Request $req){
        ...
        $data['pengunjung'] = Folio::with(['registrasi.pasien', 'registrasi.poli'])
            ...
            ->select('folios.*', 'registrasis.pasien_id', 'registrasis.bayar', 'registrasis.status', 'registrasis.poli_id')
            ->orderBy('folios.registrasi_id')
            ->orderBy('folios.created_at')
            ->get();

        if($req->lanjut){
            return view('protesa-gigi.laporanPengunjung', $data)->with('no', 1);
        } else {
            return view('protesa-gigi.cetakLaporan', $data)->with('no', 1);
        }
    }
    ...
    public function insertTindakan($unit = '', $reg = '', $pasien = ''){
        ...
        if($unit == 'ranap'){
            ...
            $data['j']      = histori_kunjungan_prt::where('registrasi_id', $reg)->first()->pasien_asal;
		...
		$data['tagihan']    = Folio::leftJoin('foliopelaksanas', 'folios.id', '=', 'foliopelaksanas.folio_id')
                ...
                ->where('folios.poli_tipe', 'T')->sum('total');
        ...
        return view('protesa-gigi.insertTindakan', $data)->with('no', 1);
    }
    public function saveTindakan(Request $req){
        ...
        if($req->poli_tipe == 'T'){
            $data = histori_kunjungan_prt::where('registrasi_id', $req->registrasi_id)->first();
        }
    ...
    public function cetakTindakan($unit = '', $reg_id = ''){
        $id_fol             = Folio::where(['jenis' => $unit, 'registrasi_id' => $reg_id])->where('poli_tipe', 'T')->first();
		$data['reg']        = Registrasi::find($reg_id);
        $data['tindakan']   = Folio::where(['jenis' => $unit, 'registrasi_id' => $reg_id])->where('poli_tipe', 'T')->get();
        // $data['pelaksana']  = Foliopelaksana::where('folio_id', $id_fol->id)->first();
        $data['dibayar']	= Folio::where(['jenis' => $unit, 'registrasi_id' => $reg_id, 'lunas' => 'Y'])->where('poli_tipe', 'T')->sum('total');
		$data['total']		= Folio::where(['jenis' => $unit, 'registrasi_id' => $reg_id])->where('poli_tipe', 'T')->sum('total');
        
        return view('protesa-gigi.cetakTindakan', $data)->with('no', 1);
    }
