...
class NeracaController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (isset($post['excel'])) {
            ...
            $aktivas = AkunCOA::where('akun_code_1', 1)
                ->where('akun_code_2', 0)
                ->where('akun_code_3', 0)
                ->where('akun_code_4', 0)
                ->where('akun_code_5', 0)
            ...
            foreach ($aktivas as $aktiva) {
                $code = implode('', [$aktiva['akun_code_1'], $aktiva['akun_code_2']]);
                $getPerhitungan1   = Helper::getPerhitungan($data['tha'], 1, $code);

                $data['data'][$i]['code']               = $aktiva['code'];
                $data['data'][$i]['nama']               = $aktiva['nama'];
                if ($post['level'] >= 2) {
                    $data['data'][$i]['tag']                = 'b0';
                    ...
                    $aktiva_sub_1 = AkunCOA::where('akun_code_1', $aktiva['akun_code_1'])
                        ->where('akun_code_2', $aktiva['akun_code_2'])
                        ->where('akun_code_3', '!=', 0)
                        ->where('akun_code_4', 0)
                        ->where('akun_code_5', 0)
                        ->where('akun_code_6', 0)
                    ...
                    foreach ($aktiva_sub_1 as $aktiva_sub) {
                        ...
                        if (end($aktiva_sub_1) == $aktiva_sub) {
                            $data['data'][$i]['code']               = '';
                            $data['data'][$i]['nama']               = 'JUMLAH ' . $aktiva['nama'];
                            ...
                            $sumAktiva['anggaran']          = $sumAktiva['anggaran'] + $getPerhitungan1['anggaran'];
                            $sumAktiva['realisasi']         = $sumAktiva['realisasi'] + $getPerhitungan1['realisasi'];
                            $sumAktiva['realisasi_sebelum'] = $sumAktiva['realisasi_sebelum'] + $getPerhitungan1['realisasi_sebelum'];
                        }
                    }
                } else {
                    $data['data'][$i]['anggaran']           = $getPerhitungan1['anggaran'];
                    ...
                    $sumAktiva['anggaran']          = $getPerhitungan1['anggaran'];
                    $sumAktiva['realisasi']         = $getPerhitungan1['realisasi'];
                    $sumAktiva['realisasi_sebelum'] = $getPerhitungan1['realisasi_sebelum'];
                }
            }
            // dd($sumAktiva);
            $sumLiabilitas = [
            ...
            $liabilitass = AkunCOA::where('akun_code_1', 2)
                ->where('akun_code_2', 0)
                ->where('akun_code_3', 0)
                ->where('akun_code_4', 0)
                ->where('akun_code_5', 0)
                ...
                ->where('akun_code_9', 0)
                ->get()->toArray();
            foreach ($liabilitass as $liabilitas) {
                $code = implode('', [$liabilitas['akun_code_1'], $liabilitas['akun_code_2']]);
                $getPerhitungan1   = Helper::getPerhitungan($data['tha'], 1, $code);

                $data['data'][$i]['code']               = $liabilitas['code'];
                $data['data'][$i]['nama']               = $liabilitas['nama'];
                if ($post['level'] >= 2) {
                    $data['data'][$i]['tag']                = 'b0';
                    $data['data'][$i]['anggaran']           = '';
                    $data['data'][$i]['realisasi']          = '';
                    ...
                    $liabilitas_sub_1 = AkunCOA::where('akun_code_1', $liabilitas['akun_code_1'])
                        ->where('akun_code_2', $liabilitas['akun_code_2'])
                        ->where('akun_code_3', '!=', 0)
                        ->where('akun_code_4', 0)
                        ->where('akun_code_5', 0)
                        ->where('akun_code_6', 0)
                        ...
                        ->where('akun_code_9', 0)
                        ->get()->toArray();
                    foreach ($liabilitas_sub_1 as $liabilitas_sub) {
                        $code = implode('', [$liabilitas_sub['akun_code_1'], $liabilitas_sub['akun_code_2'], $liabilitas_sub['akun_code_3']]);
                        $getPerhitungan2   = Helper::getPerhitungan($data['tha'], 2, $code);

                        $data['data'][$i]['code']               = $liabilitas_sub['code'];
                        $data['data'][$i]['nama']               = '&emsp;&emsp;' . $liabilitas_sub['nama'];
                        if ($post['level'] >= 3) {
                            $data['data'][$i]['tag']                = 'b0';
                            $data['data'][$i]['anggaran']           = '';
                            $data['data'][$i]['realisasi']          = '';
                            ...
                            $liabilitas_sub_2 = AkunCOA::where('akun_code_1', $liabilitas_sub['akun_code_1'])
                            ->where('akun_code_2', $liabilitas_sub['akun_code_2'])
                            ->where('akun_code_3', $liabilitas_sub['akun_code_3'])
                            ->where('akun_code_4', '!=', 0)
                            ->where('akun_code_5', 0)
                            ->where('akun_code_6', 0)
                            ->where('akun_code_7', 0)
                            ...
                            ->where('akun_code_9', 0)
                            ->get()->toArray();
                            foreach ($liabilitas_sub_2 as $class) {
                                $code = implode('', [$class['akun_code_1'], $class['akun_code_2'], $class['akun_code_3'], $class['akun_code_4'], $class['akun_code_5']]);
                                $getPerhitungan3   = Helper::getPerhitungan($data['tha'], 3, $code);
                                ...
                                if ($post['level'] >= 4) {
                                    ...
                                    $liabilitas_sub_3 = AkunCOA::where('akun_code_1', $class['akun_code_1'])
                                    ->where('akun_code_2', $class['akun_code_2'])
                                    ->where('akun_code_3', $class['akun_code_3'])
                                    ->where('akun_code_4', $class['akun_code_4'])
                                    ->where('akun_code_5', $class['akun_code_5'])
                                    ...
                                    ->where('akun_code_9', 0)
                                    ->get()->toArray();
                                    foreach ($liabilitas_sub_3 as $sub_class) {
                                        $code = implode('', [$sub_class['akun_code_1'], $sub_class['akun_code_2'], $sub_class['akun_code_3'], $sub_class['akun_code_4'], $sub_class['akun_code_5'], $sub_class['akun_code_6'], $sub_class['akun_code_7']]);
                                        $getPerhitungan4   = Helper::getPerhitungan($data['tha'], 4, $code);
                                        ...
                                        if ($getPerhitungan4['anggaran'] != 0 || $getPerhitungan4['realisasi'] != 0 || $getPerhitungan4['realisasi_sebelum'] != 0) {
                                            ...
                                            if ($post['level'] >= 5) {
                                                ...
                                                $liabilitas_sub_4 = AkunCOA::where('akun_code_1', $sub_class['akun_code_1'])
                                                ->where('akun_code_2', $sub_class['akun_code_2'])
                                                ->where('akun_code_3', $sub_class['akun_code_3'])
                                                ->where('akun_code_4', $sub_class['akun_code_4'])
                                                ->where('akun_code_5', $sub_class['akun_code_5'])
                                                ...
                                                ->where('akun_code_9', '!=', 0)
                                                ->get()->toArray();
                                                foreach ($liabilitas_sub_4 as $coa) {
                                                    $code = implode('', [$coa['akun_code_1'], $coa['akun_code_2'], $coa['akun_code_3'], $coa['akun_code_4'], $coa['akun_code_5'], $coa['akun_code_6'], $coa['akun_code_7'], $coa['akun_code_8'], $coa['akun_code_9']]);
                                                    $getPerhitungan5   = Helper::getPerhitungan($data['tha'], 5, $code);
                                            ...
                                            if (end($liabilitas_sub_3) == $sub_class) {
                                                $data['data'][$i]['code']               = '';
                                                $data['data'][$i]['nama']               = 'JUMLAH ' . $class['nama'];
                                                $data['data'][$i]['anggaran']           = $getPerhitungan3['anggaran'];
                                                $data['data'][$i]['realisasi']          = $getPerhitungan3['realisasi'];
                                ...
                                if (end($liabilitas_sub_2) == $class) {
                                    $data['data'][$i]['code']               = '';
                                    $data['data'][$i]['nama']               = 'JUMLAH ' . $liabilitas_sub['nama'];
                                    $data['data'][$i]['anggaran']           = $getPerhitungan2['anggaran'];
                                    $data['data'][$i]['realisasi']          = $getPerhitungan2['realisasi'];
                                    $data['data'][$i]['percent']            = ($getPerhitungan2['anggaran'] == 0) ? 0 : ($getPerhitungan2['realisasi'] / $getPerhitungan2['anggaran']) * 100;
                                    $data['data'][$i]['realisasi_sebelum']  = $getPerhitungan2['realisasi_sebelum'];
                        ...
                        if (end($liabilitas_sub_1) == $liabilitas_sub) {
                            $data['data'][$i]['code']               = '';
                            $data['data'][$i]['nama']               = 'JUMLAH ' . $liabilitas['nama'];
                            $data['data'][$i]['anggaran']           = $getPerhitungan1['anggaran'];
                            $data['data'][$i]['realisasi']          = $getPerhitungan1['realisasi'];
                            $data['data'][$i]['percent']            = ($getPerhitungan1['anggaran'] == 0) ? 0 : ($getPerhitungan1['realisasi'] / $getPerhitungan1['anggaran']) * 100;
                            $data['data'][$i]['realisasi_sebelum']  = $getPerhitungan1['realisasi_sebelum'];
                            ...
                            $sumLiabilitas['anggaran']          = $sumLiabilitas['anggaran'] + $getPerhitungan1['anggaran'];
                            $sumLiabilitas['realisasi']         = $sumLiabilitas['realisasi'] + $getPerhitungan1['realisasi'];
                            $sumLiabilitas['realisasi_sebelum'] = $sumLiabilitas['realisasi_sebelum'] + $getPerhitungan1['realisasi_sebelum'];
                        }
                    }
                } else {
                    $data['data'][$i]['anggaran']           = $getPerhitungan1['anggaran'];
                    ...
                    $sumLiabilitas['anggaran']          = $getPerhitungan1['anggaran'];
                    $sumLiabilitas['realisasi']         = $getPerhitungan1['realisasi'];
                    $sumLiabilitas['realisasi_sebelum'] = $getPerhitungan1['realisasi_sebelum'];
                }
            }
            ...
            $data['data'][$i]['tag']                = 'b';
            $data['data'][$i]['code']               = '';
            $data['data'][$i]['nama']               = 'JUMLAH AKTIVA';
            $data['data'][$i]['realisasi']          = $sumAktiva['realisasi'];
            $data['data'][$i]['realisasi_sebelum']  = $sumAktiva['realisasi_sebelum'];
            $i++;
            ...
            $data['data'][$i]['tag']                = 'b';
            $data['data'][$i]['code']               = '';
            $data['data'][$i]['nama']               = 'JUMLAH LIABILITAS DAN EKUITAS';
            $data['data'][$i]['realisasi']          = $sumLiabilitas['realisasi'] + $sumEkuitas['realisasi'];
            $data['data'][$i]['realisasi_sebelum']  = $sumLiabilitas['realisasi_sebelum'] + $sumEkuitas['realisasi'];
            $i++;
            ...
            return view('accounting::laporan.neraca.index', $data);
        }
    }
}
